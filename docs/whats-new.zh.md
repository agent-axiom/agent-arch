...
- 检查了搜索索引（search index）、站点地图（sitemap）、robots 文件（robots file）、本地资源（local assets）、锚点（anchors）、图片替代文本（alt text）和外部链接；
...
- 出版材料包（publisher packet）的阻塞项登记表、豁免与决策日志、行长限制与材料包标签（packet labels）现在都适合打印和导出；
