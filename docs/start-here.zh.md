...
这不是某个框架（framework）的指南，也不是 AI 功能目录（AI feature catalog）。这是一本实践型架构书，写给那些需要在真实环境里运行智能体的团队：那里有写入路径（write paths）、人工审批（human approvals）、访问边界、遥测、评测，以及明确的运维负责人机制（operational ownership）。
