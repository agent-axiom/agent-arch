...
如果你第一次来到这本书，先问自己一个问题：你想做的是一个看起来聪明的 demo，还是一个能够承受生产现实的系统？

这本书是写给第二种情况的。你不必从头到尾线性阅读，但最好把它看成一个完整论证，讲的是 agent systems 如何从 prompt-heavy prototypes 演化成 governed operating systems。
...
- 本书承诺主要论证与 reader journey；
- support layers 承诺可复用工件、runnable anchors 与 contract pages；
- 这页的作用，就是帮你尽快找到自己需要的那一种 promise。
...
它是一本面向真实落地团队的实践型架构书和 operating model，讨论的是如何在存在信任边界、审批、遥测、评测与运维责任的环境中部署智能体系统。

它的核心论点很简单：智能体需要平台，而不是魔法。
...
但它们并不能取代本书去解释这些层为什么存在，以及这些层之间的 trade-offs 如何彼此配合。
