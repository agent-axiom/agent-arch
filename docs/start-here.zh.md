...
如果你第一次来到这本书，先问自己一个问题：你需要的是一个看起来厉害的 demo agent，还是一个能承受生产现实的系统？

这本书是写给第二种情况的。最适合把它读成一个完整论证：agent systems 怎样从 prompt-heavy prototypes 长成带有 trust boundaries、policy layer、approvals、observability、evals 和 lifecycle discipline 的受治理系统。
...
这不是某个框架的指南，也不是 AI 功能目录。这是一本实践型架构书，写给那些需要在真实环境里运行 agent 的团队：那里有 write paths、人工审批、访问边界、telemetry、evals，以及明确的运维 ownership。
...
- agent 的真实 trust boundaries 在哪里；
- 安全 tool execution 应该长什么样；
- 为什么没有 traces、SLO 和 evals 时，单靠“聪明模型”不够；
- 第一次认真 rollout 之前到底需要什么。
...
这条路线适合从 agent idea 快速走到 runnable architecture。
...
这条路线适合那些不仅要看 model risk，还要看真实 execution risk 的人。
...
这条路线适合那些不想只交付 demo，而是要把项目放进真实 operational discipline 的人。

## 如果你想先看代码和工件

如果 executable support 比线性阅读更重要，就先看这里：
...
如果你现在就需要 runtime skeleton、policy contracts、memory paths、telemetry 和 rollout artifacts，这条路线会更合适。
...
如果这本书对你来说，比另一页关于“autonomy”的 AI 落地页更接近真实工程，那你来对地方了。
