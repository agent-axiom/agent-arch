...
Ее центральный тезис простой: **агенту нужна платформа, а не магия**. Как только у системы появляются рискованные действия (risky actions), память (memory), подтверждения (approvals), раскатка (rollout) и долгий эксплуатационный след (long operational tail), одной модели и нескольких инструментов (tools) уже недостаточно. Нужны явные границы доверия, слой политик (policy layer), управляемое исполнение (controlled execution), наблюдаемость (observability), оценка качества (quality assessment) и дисциплина жизненного цикла (lifecycle discipline).

Агентов строить скучно, но результат ошеломляющий: вместо эффектного разового трюка (one-off trick) команда получает систему, которую можно ограничивать (constrain), наблюдать (observe), выпускать (ship) и улучшать без гадания (improve without guessing).
