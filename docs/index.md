...
Ее центральный тезис простой: **агенту нужна платформа, а не магия**. Как только у системы появляются рискованные действия (risky actions), память, подтверждения, раскатка (rollout) и долгий эксплуатационный след, одной модели и нескольких инструментов (tools) уже недостаточно. Нужны явные границы доверия, слой политик (policy layer), управляемое исполнение, наблюдаемость, оценка качества и дисциплина жизненного цикла.
