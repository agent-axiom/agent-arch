...
Это книга для тех, кто хочет строить не “магических агентов из презентации”, а спокойные, управляемые и безопасные системы для production.
...
Это практическая книга о безопасной, управляемой и готовой к production архитектуре AI-агентов. Она нужна командам, которым мало промптов и вызовов инструментов и которые хотят спроектировать всю операционную систему вокруг агента: границы доверия, применение политик, подтверждения, сбор доказательной информации, health budgets, eval judgment и управление жизненным циклом.
...
Большинство материалов про агентов оптимизируют путь к быстрому демо. Реальные системы ломаются в другом месте: на границе между рассуждением и действием, в слое памяти, в маршрутах подтверждения, при rollout, в drift и в долгой операционной ответственности. Эта книга нужна, чтобы описать именно эту полную операционную модель.

Ее цель не в том, чтобы помочь собрать “самого автономного агента в комнате”. Ее цель в том, чтобы помочь собрать систему, которая выдерживает реальность production.
...
В первую очередь это практическая архитектурная книга и операционная модель для production agent systems.

Это не руководство по одному фреймворку, не пособие по prompt engineering и не хайповый обзор AI-экосистемы. Справочный слой и исполняемый рантайм здесь нужны для поддержки аргумента книги, а не для замены самой книги.

И это еще и намеренно собранная книга, а не просто набор хороших тем. Операционные главы разведены по ролям так, чтобы читатель чувствовал, как собирается production discipline:
...
И эта форма должна чувствоваться как читательский результат, а не только как схема глав:
- Part V учит читателя захватывать историю выполнения, задавать допустимые budgets и производить reviewable judgments;
- Part VIII учит читателя управлять реакцией жизненного цикла, управляемой lineage, видимостью evidence и accountability всего estate как одним production contour.
...
Если тебе нужны не только главы, но и готовые артефакты, начни с этих вспомогательных страниц. Они нужны, чтобы закреплять аргумент книги, а не заменять ее собственный ход чтения:
