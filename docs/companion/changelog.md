# Companion changelog

Status: skeleton for editor handoff.

## Unreleased

- Added skeleton routes for templates, checklists, changelog and errata.
- Added first placeholder templates for capability contract, release decision
  record and incident record.
- Added production readiness checklist skeleton.

## Release versioning rule

- `v1.0-book`: materials matching the first published book edition.
- `v1.0.x`: typo fixes and small clarifications.
- `v1.1`: new templates, new cases or materially changed guidance.
- `main` or `latest`: current working state, may be ahead of the printed book.
