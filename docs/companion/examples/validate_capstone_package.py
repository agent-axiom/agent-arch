...
def _manifest_integrity(package_dir: Path) -> tuple[bool, str]:
    ...
    if len(artifact_paths) != len(artifacts) or len(set(artifact_paths)) != len(
        artifact_paths
    ):
...
def _validate_raw_unknown_trace(
    ...
    ok = (
        ...
        and session_ids == {summary.get("session_id")}
        and len(tool_events) == 1
        and tool_events[0]["payload"].get("side_effect_status")
        == "side_effect_unknown"
        and len(reconciliation_events) == 1
        and reconciliation_events[0]["payload"].get("failure_reason")
        == "post_dispatch_timeout"
        and len(completion_events) == 1
        and completion_events[0]["payload"].get("status")
...
def _validate_eval(payload: dict[str, Any]) -> tuple[bool, str]:
    ...
    ok = (
        ...
        and "unknown_effect_reconciliation"
        in payload.get("duplicate_ticket_scenarios", [])
        ...
        and run.get("trace_id") in payload.get("trace_ids", [])
        and session_record.get("session_id") in payload.get("session_ids", [])
    )
...
def validate_capstone_package(package_dir: Path) -> dict[str, object]:
    ...
    try:
        ...
        blockers_complete = set(blockers_by_id) == set(REQUIRED_BLOCKERS) and all(
            all(
        ...
        refs_ok = (
            ...
            and all(
                ...
                and _contained_file(package_dir, reference)
    ...
    try:
        unknown_summary = _load_json(
            package_dir / "02-unknown-effect-trace-summary.json"
        )
    ...
    try:
        ...
        record("trace_continuity", trace_ok, trace_reason)
    except (OSError, TypeError, ValueError) as error:
    ...
    try:
        ...
        reconciliation_ok = (
            ...
            and reconciliation.get("required_before_retry")
            == "verification_result=not_found"
            and isinstance(reconciliation.get("owner"), str)
            and bool(reconciliation["owner"].strip())
            and reconciliation.get("business_key")
            == "support-request:onboarding-issue:user-42"
        )
        record(
