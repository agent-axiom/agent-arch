...
def validate_lab8_package(artifacts_dir: Path) -> dict[str, object]:
    ...
    try:
        ...
        blocker_schema_ok = set(blockers_by_id) == set(REQUIRED_BLOCKERS) and all(
            all(
    ...
    try:
        required_ids = tuple(item[1] for item in LAB_ARTIFACTS)
        ...
        record(
            "cumulative_manifest",
            verification.verified and len(verification.artifact_ids) == len(required_ids),
            "; ".join(verification.diagnostics) or "verified",
        )
    except (OSError, TypeError, ValueError) as error:
