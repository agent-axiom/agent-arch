...
Альтернативные описания 56 изображений присутствуют в локальных издательских
DOCX, но отсутствуют в нативном Google Doc и его экспорте. Для переноса
подготовлен `docs/publisher/tools/set_google_doc_image_alt_text.gs`; первый
запуск требует подтверждения разрешений Google.
