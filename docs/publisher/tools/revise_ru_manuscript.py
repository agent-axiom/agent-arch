...
def apply_albumentationsx_mcp_author_case_2026_08_30(text: str) -> str:
    """Apply the disclosed author case once and fail on manuscript drift."""
    ...
    author_case = """**Авторский кейс: AlbumentationsX MCP — возможность, а не отдельный агент.** AlbumentationsX MCP — сторонняя интеграция с открытым исходным кодом, разработанная автором этой книги; она не является официальным продуктом AlbumentationsX и не заменяет Python API (см. источники **S124**, **S125**). Сервер даёт MCP-хосту типизированный путь для проверки конвейеров аугментации, ограниченных локальных превью, сравнения вариантов, обратной связи и воспроизводимого экспорта.

Это возможность, не самостоятельная операционная роль: она не владеет целью или политикой и не обучает модель. Чтение ограничивается настроенным `allowed-root`, запись — отдельным каталогом артефактов; до рендеринга проверяются запрос и лимиты, а начальные значения генератора, манифесты, трассы, профили возможностей, снимки контрактов и эталонные оценки делают поведение наблюдаемым.

Граница доказательств явна: узкий корень настраивается; принятие превью не является жёсткой авторизацией, а молодой проект не доказывает широкое промышленное внедрение."""
