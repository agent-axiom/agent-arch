...
* Входной слой принимает чат, API, вебхуки (HTTP-уведомления) и события. Без него каналы смешиваются с логикой исполнения.
...
Для архитектуры эти приемы выходят за пределы проектирования подсказок. Каждый такой паттерн меняет, где в среде исполнения должны стоять контрольные точки, подтверждения, повторы, границы трассировки и владение.
...
По мере того как слои оценки и проверки становятся частью дисциплины релизов, полезно отслеживать и их качество как отдельное измерение здоровья. Система не вполне здорова, если поведение среды исполнения кажется приемлемым только потому, что проверяющий стал шумным или слишком доверчивым.
...
**Платформа должна давать поддерживаемые стандартные пути, а не набор низкоуровневых деталей.**
...
В нейтральном к поставщикам виде этот паттерн можно назвать долговечным актором агента. Он сочетает стабильную идентичность, локальное долговечное состояние, возобновляемые сессии, пробуждения по расписанию и трассируемую передачу управления к управляемым хранилищам.
