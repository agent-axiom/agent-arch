...
cd agent-arch
git checkout 6bc14de644039c7fea8ef5c521e4b499a7b42982
...
Полезное практическое правило: краткосрочная память должна помогать агенту действовать сейчас, а не становиться источником долговременной истины.
...
Листинг 11 — не «умный» движок извлечения, а специально понятный пример. Он показывает, что ранжирование полезно строить не только по похожести, но и по доверию, свежести и важности.
...
Этот учебный каркас показывает разделение поиска, валидации, выполнения и нормализации результата: поиск, валидацию, выполнение и нормализацию результата.
...
Короткий пример: команда подключила MCP-сервер к документам, но дала ему лишний доступ и не записала происхождение ответа. Правильный контракт фиксирует сервер, версию, область токена, методы, карантин и событие трассы.
...
Материал Google об Agent Sandbox (см. источник **S073**) показывает: для рискованных возможностей очень выгодно проектировать краткоживущие среды исполнения.
...
Листинг 17 — не движок политик эксплуатационного уровня, а понятный каркас. Его задача показать, что повтор должен зависеть от класса исхода, а не быть общим рефлексом.
...
2. Офлайн- и онлайн-оценки должны жить вместе: один контур ловит регрессии до выпуска, другой после выпуска.
...
В главе 23 произошел наблюдаемый обход требования `create_ticket.v2` через оставленный маршрут `v1`: агент сохранил цель и добился того же внешнего эффекта. Здесь этот сигнал становится находкой с владельцем, временным сдерживанием, исправлением и критерием закрытия. Дубль заявки остается регрессионной проверкой пишущего пути: закрытие должно одновременно остановить обход и исключить повтор эффекта.
...
Практический минимальный каркас:
...
Такая схема особенно полезна для чатов, голосовых агентов, рабочих процессов и агентов наблюдения, где пользователь ожидает преемственность, а не обмен запрос-ответ без состояния. Такая схема добавляет риски, которые среда исполнения должна сделать видимыми. Проверьте изоляцию клиентов, утечки между WebSocket-сессиями, продолжение после спящего режима, запланированные эффекты без активного пользователя и миграции состояния при смене версии агента.
...
Конфигурация выпускного шлюза превращает решение в проверяемый контракт: она
