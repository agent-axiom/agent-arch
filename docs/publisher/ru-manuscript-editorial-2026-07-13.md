...
[на той же странице](https://agent-axiom.github.io/agent-arch/companion/#chapter-map).
...
**Полезно разделять политику чтения памяти и политику записи в память.** Одна из самых практичных мыслей из материалов Google состоит в том, что память надо мыслить как управляемую подсистему, а не как «просто хранилище для контекста».
...
**Передовой край памяти идет в сторону адаптивного формирования, но эксплуатация пока требует дисциплины.** Исследования памяти развивают архитектуру дальше: предлагается не просто хранить записи и иногда их сжимать, а постепенно перестраивать слой памяти под реальные подходы использования.
...
**Листинг 12. Контракты инструментов чтения и записи.** Тип: декларативная конфигурация; контракт представлен в листинге.
...
**Ветка восстановления тоже должна проектироваться явно.** Исследования сценариев отказов инструментов усиливают еще одну мысль: путь восстановления почти никогда не стоит оставлять как импровизацию слоя выполнения.
...
**Листинг 16. Операционная семантика `create_ticket` и `send_email`.** Тип: декларативная конфигурация; контракт представлен в листинге.
...
Исследования проектирования проверяющего уточняют, что офлайн-оценки не стоит завязывать только на бинарной метке успеха. Для агентов с длинным горизонтом часто нужен более богатый сигнал оценивания:
...
**Многоходовую согласованность тоже стоит проверять отдельно.** Исследования многоходовой согласованности показывают: агент может выглядеть разумно в коротком сценарии и при этом постепенно входить в противоречие с самим собой в длинном контуре взаимодействия.
...
**Листинг 21. Запись проверки высокорискового изменения.** Тип: декларативная конфигурация; контракт представлен в листинге.
...
**Листинг 25. Политика покрытия наблюдаемостью и блокирующих слепых зон.** Тип: декларативная конфигурация; контракт представлен в листинге.
...
### Политика последовательности проверяет всю траекторию
...
Если нужна самая надежная база для частей I, V и VI, начинайте с нормативного каркаса и слоя `HCI/HITL`. Для инженерной практики смотрите платформенные документы и исследования, всегда учитывая дату публикации.
...
**Листинг 36. Ожидаемое состояние запуска с обязательным подтверждением.** Тип: структурированный результат; контракт представлен в листинге.
...
**Листинг 37. Минимальный профиль песочницы.** Тип: декларативная конфигурация; контракт представлен в листинге.
