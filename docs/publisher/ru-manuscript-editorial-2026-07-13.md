...
**Как читать источники.** Используйте их, чтобы проверить границу между простым рабочим процессом и оправданной агентностью.
...
**Практическая проверка.** Разметьте свой процесс: детерминированные шаги, агентные решения, границы передачи и условия остановки. Глава 3 соберет эти элементы в архитектурные слои и покажет путь одного запроса.

### Источники главы

**Как читать источники.** Используйте их, чтобы проверить выбор оркестрации без преждевременной многоагентности.
...
**Как читать источники.** Используйте их, чтобы проверить связь архитектурных слоев с управлением риском.
...
**Как читать источники.** Используйте их, чтобы проверить границы доверия и устойчивость к внедрению инструкций.
...
**Как читать источники.** Используйте их, чтобы проверить разделение идентичности, полномочия и решения политики.
...
**Как читать источники.** Используйте их, чтобы проверить неизменность подтверждаемого действия и человеческий надзор.
...
**Как читать источники.** Используйте их, чтобы проверить риски памяти, поиска и недоверенного контекста.
...
**Как читать источники.** Используйте их, чтобы проверить жизненный цикл классов памяти и область доступа.
...
**Как читать источники.** Используйте их, чтобы проверить возобновляемую работу и безопасное обновление контекста.
...
**Как читать источники.** Используйте их, чтобы проверить контракт инструмента как ограниченное полномочие.
...
Для MCP полезно держать модель угроз MCP не как общий страх перед интеграциями, а как проверочную матрицу у каждой подключаемой возможности. Официальные материалы MCP отдельно подчеркивают запрет token passthrough, проверку областей доступа, `HTTPS/SSRF-`ограничения и защиту сессий с состоянием; это делает матрицу не опциональным украшением, а частью авторизационного и эксплуатационного контракта. Минимальный вариант выглядит так:
...
**Как читать источники.** Используйте их, чтобы проверить авторизацию MCP, изоляцию и доверие к удаленному серверу.
...
**Как читать источники.** Используйте их, чтобы проверить повторы, неизвестный внешний эффект и долговечное состояние.
...
Повтор трассы сначала проверяет целостность файла: наличие единственного идентификатора трассы, единственного события `run_start` и обязательных непустых полей воспроизведения. Неоднозначные, отсутствующие или отредактированные поля завершают проверку закрытым отказом; точные сообщения диагностики вынесены в сопроводительный справочник.
...
**Как читать источники.** Используйте их, чтобы проверить семантику трасс и пригодность телеметрии для оценки.
...
Роль SLO в этой книге тоже вполне конкретна. Трассы захватывают сырую историю. Оценки производят суждения. Наблюдаемость сохраняет доказательства на масштабе системы. заверение отвечает на findings. SLO задают бюджет здоровья и бюджет риска, которые платформа имеет право тратить в рабочем режиме.
...
**Как читать источники.** Используйте их, чтобы проверить измерение надежности и управленческое действие по SLO.
...
**Как читать источники.** Используйте их, чтобы проверить калибровку оценок и связь офлайн- и онлайн-сигналов.
...
**Как читать источники.** Используйте их, чтобы проверить цепочку от наблюдаемого поведения до решения о выпуске.
...
**Как читать источники.** Используйте их, чтобы проверить распределение ответственности между платформой и продуктом.
...
**Как читать источники.** Используйте их, чтобы проверить управляемый путь по умолчанию и зрелость внедрения.
...
Минимальная запись в реестре для производственной системы с ИИ-агентами обычно должна включать:
...
**Как читать источники.** Используйте их, чтобы проверить полноту инвентаризации и непрерывную сверку реестра.
...
**Как читать источники.** Используйте их, чтобы проверить безопасную разработку и жизненный цикл изменения.
...
* какая версия контракта и схема подтверждения были активны;
...
Сквозной сценарий: происхождение для исправления дубля заявки. После инцидента с дублем заявки последующий разбор должен уметь восстановить не только изменение с исправлением повтора. Ему нужны версии набора для оценки, набора политик для `side_effect_unknown`, контракта возможности `create_ticket`, шлюза поэтапного выпуска, схемы подтверждения и схемы трасс, которые были активны в контрольной волне. Если хотя бы один из этих артефактов «где-то в чате», а не в утвержденном наборе выпуска, команда не сможет доказать, что повторный дубль случился под исправленным контролем или под старым набором правил.
...
Глава 16 «Сквозная цепочка доказательств» показывает, как управляемый каркас остается связан с запросом, политикой, подтверждениями, трассами, оценками, инцидентами и суждением о поэтапном выпуске.
...
**Как читать источники.** Используйте их, чтобы проверить происхождение, целостность и доверие к выпускным артефактам.
...
**Как читать источники.** Используйте их, чтобы проверить телеметрию обнаружения и наблюдаемость разрывов контроля.
...
**Как читать источники.** Используйте их, чтобы проверить поведение при конфликте цели, надзора и ограничения.
...
**Как читать источники.** Используйте их, чтобы проверить заверение, соревновательное тестирование и реагирование.
...
**Как читать источники.** Используйте их, чтобы проверить отзыв полномочий и доказуемое завершение эксплуатации.
...
**Проверка на своей системе.** Сопоставьте модули эталонного пакета с контрольными точками пути одного запуска. Глава 27 покажет фактическое применение каталога возможностей и решений политик.

### Источники главы

**Как читать источники.** Используйте их, чтобы проверить долговечное исполнение и ограничения эталонной среды.

* [Cloudflare, Build Agents on Cloudflare](https://developers.cloudflare.com/agents/).
* [Cloudflare Agents SDK, Durable execution](https://developers.cloudflare.com/agents/runtime/execution/durable-execution/).
...
**Как читать источники.** Используйте их, чтобы проверить принудительное применение политики на границе действия.

* [AWS, Secure AI agents with Policy and Lambda interceptors](https://aws.amazon.com/blogs/machine-learning/secure-ai-agents-with-policy-and-lambda-interceptors-in-amazon-bedrock-agentcore-gateway/).
* [OWASP, AI Agent Security Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/AI_Agent_Security_Cheat_Sheet.html).
...
**Финальный рывок практики.** Выполните лабораторную работу 8 и соберите итоговый аудит готовности по этапам ниже. После практики заключение возвращает всю цепочку к исходному инциденту и формулирует критерий управляемой системы.

### Источники главы

**Как читать источники.** Используйте их, чтобы проверить независимые сигналы готовности и блокирующее решение.
...
* [NIST, AI RMF 1.0](https://www.nist.gov/publications/artificial-intelligence-risk-management-framework-ai-rmf-10), дата обращения: 15 июля 2026 года.
...
* [Дмитрий Викулин, «Архитектура надежных AI-агентов»](https://vikulin.ai/library/tpost/ai\_agent\_architecture), дата обращения: 15 июля 2026 года.
...
* [OpenAI, Agent Builder](https://developers.openai.com/api/docs/guides/agent-builder), дата обращения: 15 июля 2026 года.
* [OpenAI, Safety in building agents](https://developers.openai.com/api/docs/guides/agent-builder-safety), дата обращения: 15 июля 2026 года.
* [Model Context Protocol, Security Best Practices](https://modelcontextprotocol.io/docs/tutorials/security/security\_best\_practices), дата обращения: 15 июля 2026 года.
* [Model Context Protocol, Authorization specification](https://modelcontextprotocol.io/specification/2025-11-25/basic/authorization), дата обращения: 15 июля 2026 года.
* [Agent2Agent Protocol, A2A specification](https://a2a-protocol.org/latest/specification/), дата обращения: 15 июля 2026 года.
...
* [Google Cloud, Vertex AI Agent Builder overview](https://docs.cloud.google.com/gemini-enterprise-agent-platform/overview), дата обращения: 15 июля 2026 года.
...
* Anthropic — [Responsible Scaling Policy 3.4](https://www.anthropic.com/responsible-scaling-policy), дата обращения: 15 июля 2026 года.
* OpenAI — [Running Codex safely at OpenAI](https://openai.com/index/running-codex-safely/), дата обращения: 15 июля 2026 года.
* GitHub Agentic Workflows — [Security Architecture](https://github.github.com/gh-aw/introduction/architecture/), дата обращения: 15 июля 2026 года.
* GitHub Changelog — [Copilot in Visual Studio Code, June 2026 releases](https://github.blog/changelog/2026-07-08-github-copilot-in-visual-studio-code-june-2026-releases/), дата обращения: 15 июля 2026 года.

### Инциденты и сценарии

* [American Bar Association, BC Tribunal Confirms Companies Remain Liable for Information Provided by AI Chatbot](https://www.americanbar.org/groups/business\_law/resources/business-law-today/2024-february/bc-tribunal-confirms-companies-remain-liable-information-provided-ai-chatbot/), дата обращения: 15 июля 2026 года.
...
* [OpenReview (препринт), MemGen: Weaving Generative Latent Memory for Self-Evolving Agents](https://openreview.net/forum?id=vI56m4Iu4e), дата обращения: 15 июля 2026 года.
...
* [OpenReview (препринт), Why Do Multiagent Systems Fail?](https://openreview.net/forum?id=wM521FqPvI), дата обращения: 15 июля 2026 года.
...
Второй запуск намеренно моделирует отказ разрешенной возможности. Он показывает, что неудача становится явным состоянием с трассой и причиной, а не растворяется в общем успешном пути. Параметры командной строки позволяют зафиксировать идентичность, сессию и трассу для воспроизводимого упражнения; полный перечень переключателей и полей ответа находится в сопроводительном справочнике.
