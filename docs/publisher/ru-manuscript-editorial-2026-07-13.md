...
Перед выбором SDK, модели или оркестратора полезно заполнить короткий краткий архитектурный паспорт безопасного агента. Это не бюрократия, а способ заранее увидеть автономность, полномочия, инструменты, память, доказательства, выпуск и владельцев системы.
...
**Как читать источники.** Используйте их, чтобы проверить границу между простым рабочим процессом и оправданной агентностью; платформенный пример не следует читать как универсальную гарантию.
...
**Как читать источники.** Используйте их, чтобы проверить выбор оркестрации без преждевременной многоагентности; платформенный пример не следует читать как универсальную гарантию.
...
**Как читать источники.** Используйте их, чтобы проверить связь архитектурных слоев с управлением риском; платформенный пример не следует читать как универсальную гарантию.
...
**Как читать источники.** Используйте их, чтобы проверить границы доверия и устойчивость к внедрению инструкций; платформенный пример не следует читать как универсальную гарантию.
...
Дальнейшие главы свяжут это решение политики с трассами, подтверждениями, оценками, инцидентами и поэтапным выпуском в единую цепочку доказательств.
...
**Как читать источники.** Используйте их, чтобы проверить разделение идентичности, полномочия и решения политики; платформенный пример не следует читать как универсальную гарантию.
...
**Листинг 8. Запрос на подтверждение.** Тип: декларативная конфигурация; назначение: сделать контракт раздела проверяемым; перед промышленным применением требуется адаптация к схеме и ограничениям организации.
...
**Как читать источники.** Используйте их, чтобы проверить неизменность подтверждаемого действия и человеческий надзор; платформенный пример не следует читать как универсальную гарантию.
...
Критерий приёмки: решение политики предшествует подтверждению; запрос содержит канонический `action_digest`, сводку полезной нагрузки, срок действия и одноразовое значение, а решение сверяется с ожидаемым отпечатком. Ограничение стенда остаётся явным: проверка выполняется только в памяти процесса, срок действия не применяется, а долговечное возобновление исходного действия не реализовано.
...
**Как читать источники.** Используйте их, чтобы проверить риски памяти, поиска и недоверенного контекста; платформенный пример не следует читать как универсальную гарантию.
...
**Как читать источники.** Используйте их, чтобы проверить жизненный цикл классов памяти и область доступа; платформенный пример не следует читать как универсальную гарантию.
...
**Как читать источники.** Используйте их, чтобы проверить возобновляемую работу и безопасное обновление контекста; платформенный пример не следует читать как универсальную гарантию.
...
**Листинг 12. Важно различать инструменты чтения и инструменты записи.** Тип: декларативная конфигурация; назначение: сделать контракт раздела проверяемым; перед промышленным применением требуется адаптация к схеме и ограничениям организации.
...
**Как читать источники.** Используйте их, чтобы проверить контракт инструмента как ограниченное полномочие; платформенный пример не следует читать как универсальную гарантию.
...
Эти поля нужны не ради бюрократии. `schema_hash` и `tool_definition_hash` фиксируют только объявленный интерфейс, но не доказывают неизменность реализации сервера. Для локального сервера нужны подписанный хеш исполняемого артефакта и происхождение зависимостей; для удалённого — проверка идентичности конечной точки, TLS и аудитории ресурса OAuth. Результат инструмента всегда остаётся недоверенным: среда выполнения извлекает только разрешённые типизированные поля и не позволяет ответу менять политику, полномочия или следующий вызов.
...
**Как читать источники.** Используйте их, чтобы проверить авторизацию MCP, изоляцию и доверие к удаленному серверу; платформенный пример не следует читать как универсальную гарантию.
...
**Листинг 16. Хороший контракт выполнения хранит операционную семантику явно.** Тип: декларативная конфигурация; назначение: сделать контракт раздела проверяемым; перед промышленным применением требуется адаптация к схеме и ограничениям организации.
...
**Как читать источники.** Используйте их, чтобы проверить повторы, неизвестный внешний эффект и долговечное состояние; платформенный пример не следует читать как универсальную гарантию.
...
Критерий приёмки: читатель объясняет, почему повтор симулятора не равен сверке внешнего состояния, и называет недостающее доказательство `verification_result`. Эталонная трасса должна совпасть по смыслу с `docs/companion/artifacts/trace-failed-tool-timeout.jsonl`.
...
Тогда трасса остаётся читаемой и при этом показывает, где именно ушло время, деньги и надежность.
...
**Как читать источники.** Используйте их, чтобы проверить семантику трасс и пригодность телеметрии для оценки; платформенный пример не следует читать как универсальную гарантию.
...
**Как читать источники.** Используйте их, чтобы проверить измерение надежности и управленческое действие по SLO; платформенный пример не следует читать как универсальную гарантию.
...
| :---- | :------------- | :-------------------- |
| Требование | Какое поведение обязательно или запрещено и для какого риска? | Версионируемое требование с владельцем, областью и критерием приемки |
...
Переносимость здесь не означает одно универсальное правило для всех сред исполнения. Она означает, что контроль описан как отдельный, версионируемый контракт, а не спрятан в системной подсказке, обработчике конкретного программного каркаса или неявной ветке адаптера. Один и тот же контракт можно привязать к разным техническим границам: до вызова инструмента, после результата инструмента, перед записью состояния или перед внешним действием. Реализация меняется, смысл требования и доказательство его исполнения остаются сопоставимыми.
...
Для того же агента поддержки это означает: если новый адаптер или новая инструкция модели изменяет поведение на сложных сценариях статуса, команда должна увидеть это на контрольном или теневом этапе, а не после широкого выпуска.
...
Для агентных систем один элемент набора должен содержать сценарий, метки, входы, ожидаемые результаты и класс риска.
...
**Как читать источники.** Используйте их, чтобы проверить калибровку оценок и связь офлайн- и онлайн-сигналов; платформенный пример не следует читать как универсальную гарантию.
...
**Листинг 21. Зачем нужен отдельный слой схем.** Тип: декларативная конфигурация; назначение: сделать контракт раздела проверяемым; перед промышленным применением требуется адаптация к схеме и ограничениям организации.
...
**Как читать источники.** Используйте их, чтобы проверить цепочку от наблюдаемого поведения до решения о выпуске; платформенный пример не следует читать как универсальную гарантию.
...
* экспериментальный поэтапный выпуск.
...
**Как читать источники.** Используйте их, чтобы проверить распределение ответственности между платформой и продуктом; платформенный пример не следует читать как универсальную гарантию.
...
**Как читать источники.** Используйте их, чтобы проверить управляемый путь по умолчанию и зрелость внедрения; платформенный пример не следует читать как универсальную гарантию.
...
**Дальше.** Глава 19 введет реестр, который отличает одобренных агентов от фактически обнаруженных сущностей.
...
**Как читать источники.** Используйте их, чтобы проверить полноту инвентаризации и непрерывную сверку реестра; платформенный пример не следует читать как универсальную гарантию.
...
**Дальше.** Глава 20 свяжет владение и реестр с управляемым жизненным циклом ADLC.
...
**Как читать источники.** Используйте их, чтобы проверить безопасную разработку и жизненный цикл изменения; платформенный пример не следует читать как универсальную гарантию.
...
**Дальше.** Глава 21 превратит изменение в проверяемый набор доверенных артефактов.
...
**Как читать источники.** Используйте их, чтобы проверить происхождение, целостность и доверие к выпускным артефактам; платформенный пример не следует читать как универсальную гарантию.

* [Google Research, Securing the AI Software Supply Chain](https://research.google/pubs/securing-the-ai-software-supply-chain/).
* [NIST, SP 800-218A](https://csrc.nist.gov/pubs/sp/800/218/a/final).
* [NIST, SP 800-53 Rev. 5](https://csrc.nist.gov/pubs/sp/800/53/r5/upd1/final).

**Дальше.** Часть VII начнется с телеметрии, необходимой для обнаружения отклонений и разрывов контроля.
...
* несовпадения между подтверждение и возобновление после прерывания;
* неожиданные сдвиги в выборе схемах оркестрации или пересечениях границ исполнителей;
...
**Листинг 25. Куда исследовательская повестка двигает наблюдаемость дальше.** Тип: декларативная конфигурация; назначение: сделать контракт раздела проверяемым; перед промышленным применением требуется адаптация к схеме и ограничениям организации.
...
**Как читать источники.** Используйте их, чтобы проверить телеметрию обнаружения и наблюдаемость разрывов контроля; платформенный пример не следует читать как универсальную гарантию.

* [Microsoft Learn, Observability for Generative AI and agentic AI systems](https://learn.microsoft.com/en-us/security/zero-trust/sfi/observability-ai-systems).
* [OpenAI, Trace grading](https://platform.openai.com/docs/guides/trace-grading).
* [NIST, AI RMF 1.0](https://www.nist.gov/publications/artificial-intelligence-risk-management-framework-ai-rmf-10).

**Дальше.** Глава 23 рассмотрит риск намеренного или возникающего расхождения цели и разрешенного поведения.
...
**Как читать источники.** Используйте их, чтобы проверить поведение при конфликте цели, надзора и ограничения; платформенный пример не следует читать как универсальную гарантию.
...
**Дальше.** Глава 24 превратит такие сценарии в контур заверения и реагирования.
...
В этой главе разбирается, как сигнал риска превращается в сдерживание, исправление и закрытие находки.
...
**Практический шаг.** Проведите короткую тренировку: сохраните доказательства, локализуйте эффект, сформулируйте причинную гипотезу и решение.

### Источники главы

**Как читать источники.** Используйте их, чтобы проверить заверение, соревновательное тестирование и реагирование; платформенный пример не следует читать как универсальную гарантию.

* [Google Research, Security Assurance in the Age of Generative AI](https://research.google/pubs/security-assurance-in-the-age-of-generative-ai/).
* [NIST, AI RMF 1.0](https://www.nist.gov/publications/artificial-intelligence-risk-management-framework-ai-rmf-10).
* [OWASP, Top 10 for Agentic Applications for 2026](https://genai.owasp.org/resource/owasp-top-10-for-agentic-applications-for-2026/).

**Дальше.** Глава 25 задаст контракт прекращения полномочий и сохранения исторических доказательств.
...
**Как читать источники.** Используйте их, чтобы проверить отзыв полномочий и доказуемое завершение эксплуатации; платформенный пример не следует читать как универсальную гарантию.

* [NIST, AI RMF 1.0](https://www.nist.gov/publications/artificial-intelligence-risk-management-framework-ai-rmf-10).
* [NIST, SP 800-218A](https://csrc.nist.gov/pubs/sp/800/218/a/final).
* [NIST, SP 800-53 Rev. 5](https://csrc.nist.gov/pubs/sp/800/53/r5/upd1/final).

**Дальше.** Часть VIII материализует эти требования в эталонной среде исполнения.
...
**Как читать источники.** Используйте их, чтобы проверить долговечное исполнение и ограничения эталонной среды; платформенный пример не следует читать как универсальную гарантию.
...
**Листинг 32. Минимальный контракт возможности.** Тип: декларативная конфигурация; назначение: сделать контракт раздела проверяемым; перед промышленным применением требуется адаптация к схеме и ограничениям организации.
...
**Листинг 33. Принудительный путь от предложения модели к исполнению.** Тип:
...
**Листинг 34. Симметричные перехватчики вокруг решения политики.** Тип:
...
**Листинг 35. Состав отпечатка подтверждаемого действия.** Тип:
...
**Как читать источники.** Используйте их, чтобы проверить принудительное применение политики на границе действия; платформенный пример не следует читать как универсальную гарантию.
...
**Листинг 36. Проверка готовности к поэтапному выпуску.** Тип: синтаксически проверяемый учебный пример; источник: `docs/book/part-vii/chapter-18.md`.
...
**Как читать источники.** Используйте их, чтобы проверить независимые сигналы готовности и блокирующее решение; платформенный пример не следует читать как универсальную гарантию.
...
Эталон не обеспечивает изоляцию процесса и сети, реальный `MCP/OAuth`, аутентификацию подтверждающего, долговечное хранение и применение решения, атомарный аудит, возобновление после перезапуска или проверяемые аттестации выпуска. Прохождение его тестов не является разрешением на промышленный запуск.
...
* глава 24 для ответа контура заверения;
* главы 20, 21 и 24 для связи управляемых артефактов, идентичности выпуска, контракта проверки и происхождения делегированного разрешения;
...
Учебный маршрут разбора обращений поддержки. support-triage-ref показывает идентичность агента, декларации `search_docs/create_ticket`, остановку на подтверждении, идентификаторы трассы и сессии и экспорт спецификации оценки. Он имитирует управляющий путь, но не создаёт внешнюю заявку, не моделирует фиксацию, повтор и сверку и не доказывает отсутствие дубля.

### Матрица доказательств эталонного пакета
...
**Листинг 37. Как запустить.** Тип: структурированный результат; назначение: сделать контракт раздела проверяемым; перед промышленным применением требуется адаптация к схеме и ограничениям организации.
...
Просмотр артефактов жизненного цикла из частей VI и VII, включая связь управления средой выполнения и идентичность выпуска:
...
**Листинг 38. Примерные конфиги.** Тип: декларативная конфигурация; назначение: сделать контракт раздела проверяемым; перед промышленным применением требуется адаптация к схеме и ограничениям организации.
...
Такой пример не делает эталонную среду выполнения полноценным оркестратором песочниц. Он фиксирует поверхность контракта, которую главы 11 и 19 требуют от настоящей среды выполнения с песочницей: манифест, права, материализация рабочей области, состояние сессии и политика снимка/возобновления должны быть видимыми для проверки.

**Паттерн долговечного агентного исполнителя.**

Эталонная среда выполнения должна отдельно показывать границу долговечного агентного исполнителя из главы 26\. Среде выполнения не нужна зависящая от поставщика реализация Durable Object, но ей нужен видимый контракт для стабильной идентичности агента, локального состояния экземпляра, возобновляемых сессий, запланированных пробуждений и передачи в управляемые хранилища.
