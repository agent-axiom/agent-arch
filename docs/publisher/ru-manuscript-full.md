...
**Мини-маршрут на один вечер**
Если у читателя есть только один вечер, достаточно пройти пять файлов и одну проверку. Сначала открыть `docs/companion/examples/capability-contract-support-ticket.md` и назвать действие, которое агенту разрешено. Затем открыть `docs/companion/examples/incident-record-support-ticket-timeout.md` и увидеть, как тайм-аут превращается в расследуемый инцидент. Потом открыть `docs/companion/examples/release-decision-record-support-ticket.md` и проверить, какое доказательство разрешает или останавливает волну. После этого посмотреть `docs/companion/artifacts/trace-failed-tool-timeout.jsonl` и `docs/companion/artifacts/eval-failed-run-timeout.json`: trace и eval должны говорить об одном и том же сбое разными языками. Финальный шаг — запустить `uv run pytest tests/test_agent_runtime_ref.py` и убедиться, что практическая часть книги проверяется кодом, а не только обещанием в тексте.
...
### Досье сквозного кейса

**Продуктовая цель:** сократить время разбора обращений поддержки без потери управляемости: агент может читать контекст, предлагать решение, создавать тикет и готовить ответ клиенту, но не должен незаметно менять внешний мир.

**Главные участники:** пользователь, оператор поддержки, владелец capability `create_ticket`, платформенная команда, security reviewer, release owner и on-call инженер. Важность ролей в том, что ни одно действие агента не остается “ничьим”: у каждого риска есть человек или группа, которые могут разрешить, остановить или расследовать действие.

**Критичные возможности:** чтение тикета, извлечение контекста, запись памяти, создание тикета, подготовка сообщения клиенту, запрос человеческого подтверждения, экспорт trace, формирование eval case и решение о расширении волны запуска.

**Главный риск:** агент выглядит как помощник по тексту, но постепенно получает право на действия с внешними последствиями. Поэтому сквозной вопрос книги звучит так: какое действие сейчас разрешено, чем оно ограничено, какой след останется и кто сможет объяснить результат.

**Доказательства готовности:** capability contract, policy decision, approval record, idempotency key, trace, SLO, regression eval, release decision record, запись в реестре и критерий вывода из эксплуатации.

**Вопрос для читателя:** если заменить кейс поддержки на ваш агентный сценарий, какие поля этого досье вы можете заполнить уже сегодня, а какие пока держатся на устной договоренности?
...
**Сцена, с которой начинается часть**
Демо заканчивается аплодисментами: агент красиво резюмирует обращение и предлагает закрыть тикет. Через час руководитель спрашивает не про качество ответа, а про право на действие: кто разрешил закрытие, где след решения и как отменить ошибку. С этого момента история перестает быть разговором о модели и становится разговором о платформе.
...
**Семь серий сквозного кейса**
Кейс поддержки полезно читать как небольшой производственный сериал, а не как один пример. В первой части команда понимает, что агент не просто отвечает, а получает право на действие. Во второй — выясняет, что память может помочь пользователю и одновременно создать новый риск хранения. В третьей — сталкивается с инструментом, который умеет создать реальный тикет. В четвертой — учится видеть каждый шаг в трассе и оценках. В пятой — превращает найденный сбой в изменение жизненного цикла. В шестой — назначает владельцев, реестр и путь вывода из эксплуатации. В седьмой — решает, можно ли расширять волну запуска. Если читатель удерживает эту линию, книга перестает быть каталогом практик и становится историей взросления одного агента.
...
**Сцена, с которой начинается часть**
Клиент пишет в тикете фразу, которая выглядит как деловая просьба, но фактически пытается стать инструкцией для агента. Модель не “злая” и не “сломанная”: она просто получила недоверенный текст в месте, где система не отличила данные от правил. Эта часть начинается там, где команда понимает, что безопасность агента — это контур разрешенного действия.
...
**Сцена, с которой начинается часть**
Агент запоминает удобную деталь о пользователе и на следующей неделе применяет ее к другому обращению. Пользователь удивлен, оператор не понимает, откуда взялась уверенность, а команда впервые видит, что память переживает запрос и становится частью будущего поведения. Эта часть держит простое напряжение: полезная память должна иметь право на существование.
...
**Сцена, с которой начинается часть**
На панели поддержки появляется новый тикет, созданный агентом после тайм-аута. Никто не уверен, это первый эффект или повтор уже выполненного действия. В этот момент инструмент перестает быть API-вызовом и становится производственным событием, которое должно иметь владельца, ключ идемпотентности, подтверждение и путь сверки.
...
Представь дежурного инженера в момент тайм-аута: пользователь ждет ответ, интеграция молчит, менеджер просит "просто повторить", а в соседней вкладке уже мог появиться первый тикет. Именно здесь книга должна удержать читателя от привычной инженерной реакции. В агентной системе повтор — это не техническая мелочь, а решение о втором возможном побочном эффекте.
...
**Сцена, с которой начинается часть**
На ретроспективе команда спорит, был ли запуск успешным: пользователь получил ответ, но один trace показывает позднее подтверждение, eval помечает регрессию, а release gate не знает, должен ли остановить волну. Эта часть начинается с неприятной истины: без цепочки доказательств качество агента остается впечатлением.
...
Сильная сцена этой главы — не экран с зеленым тестом, а разговор перед расширением волны. Один человек говорит: "пользователи довольны". Другой спрашивает: "какой trace это доказывает, какой eval это проверил и какой gate имеет право остановить выпуск". В этот момент книга переводит спор из настроения в доказательство.
...
**Сцена, с которой начинается часть**
Через полгода в компании уже пять похожих агентов поддержки, каждый со своей памятью, политиками и обходными путями. Все они начинались как разумные локальные инициативы, но теперь никто не может быстро ответить, кого выключать при инциденте. Эта часть показывает, почему владение, золотые пути, ADLC и реестр становятся не бюрократией, а способом сохранить скорость без хаоса.
...
**Сцена, с которой начинается часть**
Перед расширением волны запуска команда видит сильное демо, зеленые проверки и довольных первых пользователей. Но release owner задает последний вопрос: что именно дает нам право пустить агента дальше и какой сигнал остановит волну до ущерба. Финальная часть переводит архитектуру книги в решение: запускать, ограничить или блокировать.
...
### Как применить книгу после чтения

Книга становится полезнее, когда после последней главы у читателя есть не только понимание, но и маршрут действия. Не нужно пытаться внедрить все сразу. Достаточно выбрать один живой агентный сценарий и провести его через несколько коротких проверок.

**За 7 дней**
Выбери один агентный сценарий, который уже существует или планируется в ближайшем квартале. Опиши его как цепочку действий: что читает агент, что он может записать, какие инструменты вызывает, где нужен человек, какой след остается и как остановить запуск. На выходе должна появиться одна страница: capability, владелец, риск, подтверждение, trace, eval и решение о первой волне.

**За 30 дней**
Собери маленький рабочий пакет вокруг этого сценария: capability contract, набор политик, минимальный trace, один regression eval, SLO для управляемости последствий, запись в реестре и черновик launch decision record. Это не финальная платформа, а проверка того, что команда умеет спорить о действиях агента на основании артефактов.

**На архитектурном ревью**
Используй книгу как набор вопросов, а не как список обязательных технологий. Начни с пяти проверок: какое действие агенту разрешено, кто владеет последствием, где политика может сказать "нет", какой trace объяснит запуск и какой gate остановит расширение волны. Если на эти вопросы нельзя ответить без догадок, агент еще не готов к промышленному запуску, даже если демо выглядит сильным.

**Практика в репозитории после чтения**
Открой `docs/companion/examples/capability-contract-support-ticket.md`, `docs/companion/examples/incident-record-support-ticket-timeout.md`, `docs/companion/examples/production-readiness-support-ticket.md` и `docs/companion/examples/release-decision-record-support-ticket.md` как один пакет, а не как отдельные страницы. Затем сопоставь их с `docs/companion/artifacts/trace-failed-tool-timeout.jsonl` и `docs/companion/artifacts/eval-failed-run-timeout.json`: в хорошей системе capability, incident, trace, eval и release decision должны рассказывать одну историю о действии агента. После этого запусти `uv run pytest tests/test_agent_runtime_ref.py` и выбери один тест как основу для собственного командного упражнения.

**Неправильно / промышленно**

- Неправильно: выбирать агента потому, что модель “умеет рассуждать”. Промышленно: сначала назвать действие, владельца, границу полномочий и самый простой workflow, который может заменить агента.
- Неправильно: считать память улучшением UX. Промышленно: оформлять память как состояние с происхождением, сроком жизни, областью видимости и правом удаления.
- Неправильно: давать модели прямой доступ к API. Промышленно: выдавать capability через каталог, шлюз политик, лимиты, идемпотентность и журналируемый результат.
- Неправильно: просить человека “подтвердить намерение”. Промышленно: показывать payload, риск, действие, последствия и способ отказа.
- Неправильно: собирать trace только для отладки. Промышленно: строить trace как доказательство решения, которое связывает контекст, политику, инструмент, подтверждение и результат.
- Неправильно: держать eval как отчет после запуска. Промышленно: подключать regression eval к gate, который может остановить расширение волны.
- Неправильно: запускать потому, что первые пользователи довольны. Промышленно: выпускать только ту волну, для которой есть evidence, owner, rollback и критерий остановки.
- Неправильно: считать ownership организационной формальностью. Промышленно: делать владельца capability, политики, рантайма и инцидентного пути частью архитектуры.

**Пять артефактов, которые удобно переслать команде**

- **Capability review card:** одно действие агента, владелец, вход, выход, риск, политика, подтверждение, лимиты, trace и критерий запрета.
- **Launch review card:** какая волна запуска обсуждается, какие доказательства собраны, какие блокеры остались, кто имеет право сказать "нет".
- **Incident card:** что сделал агент, какой внешний эффект возник, какой trace и eval это подтверждают, какое смягчение включено и кто закрывает исправление.
- **Security review questions:** какие тексты считаются недоверенными, где отделены данные от инструкций, какие policy decisions журналируются и какой обход запрещен.
- **CTO/архитекторская страница:** почему агент нужен именно здесь, какой риск он добавляет, какой золотой путь используется, как он выводится из эксплуатации и что будет измеряться после запуска.

**15 фраз, которые удобно переслать коллегам**

- Агентность начинается не там, где модель рассуждает, а там, где система получает право действовать.
- Самый опасный инструмент — тот, который выглядит как обычная функция, но меняет внешний мир.
- Память агента — это не улучшение UX, пока не ясно, кто разрешил ей существовать.
- Trace нужен не логам, а человеку, который будет объяснять последствия.
- Eval полезен только тогда, когда может остановить выпуск.
- Повтор без идемпотентности превращает сбой в новое действие.
- Политика вне runtime быстро становится документом, который никто не исполняет.
- Launch checklist ценен не галочками, а правом сказать "нет".
- Без владельца capability команда владеет не системой, а надеждой.
- Если обход безопасного пути быстрее золотого пути, команды выберут обход.
- Реестр агентов нужен не для отчетности, а для момента, когда нужно понять, кого выключать.
- Человеческое подтверждение должно подтверждать payload, а не общее намерение.
- Демо показывает возможность; цепочка доказательств показывает готовность.
- Агентный инцидент почти всегда начинается раньше видимого сбоя.
- Хорошая архитектура агента делает автономию наблюдаемой, ограничиваемой и отменяемой.
