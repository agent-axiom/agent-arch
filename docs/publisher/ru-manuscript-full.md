...
Загрузчик YAML для идентичности агента, утвержденного инвентаря, политик, каталога возможностей и политики выпуска.
- memory.py (https://github.com/agent-axiom/agent-arch/blob/main/agent_runtime_ref/memory.py)
...
- rollout.py (https://github.com/agent-axiom/agent-arch/blob/main/agent_runtime_ref/rollout.py)
Минимальный шлюз проверки готовности перед выпуском.
...
Повтор источникового комментария сокращен. Полная логика источников остается в каталоге источников и в основном разделе, где обсуждается обвязка агента.
...
#### 14.1. Практикум: связать capability contract с SLO, шлюз оценки и golden path
...
| --- | --- | --- |
| `capability_name`, `capability_version` | platform/runtime | владелец выпуска |
| `owner_team`, `business_owner` | продукт и платформа | владелец выпуска |
| `risk_tier` | security/platform governance | security проверкаer |
| `tool_principal` | platform/security | infrastructure owner |
| `required_policy_decisions` | policy owner | шлюз выпуска |
| `slo_bindings` | SRE/operations | дежурный владелец |
| `eval_gates` | eval owner | владелец выпуска |
| `rollout_blockers` | совместно | шлюз выпуска |

Такая матрица не заменяет ответственность, а убирает двусмысленность. Если capability меняет риск, меняется не только строка в каталоге. Должны измениться шлюз оценки, условия волны, ожидания по SLO и, возможно, путь подтверждения. Если это не происходит, каталог начинает врать.
...
Повтор подробных сообщений проверки конфигурации средств управления убран из печатного потока. Они сохранены в companion-разделе; в книге остается правило, что controls-policy должен быть проверяемым контрактом, а не неявной договоренностью команды.
...
Повтор связки “каталог возможностей — политики — подтверждения — trace — жизненный цикл” сокращен. В этой версии рукописи она уже раскрыта в главах 5, 6, 16, 17 и 23; приложение оставляет навигационную роль.
...
| --- | --- | --- | --- |
| Все шлюз оценкиs зелёные, трассы полные, SLO в пределах | expand | расширяет волну по заранее заданному лимиту | владелец выпуска |
