...
Это не косметика. Если capability меняет внешний мир, она не должна проходить тот же путь, что и чтение. Если capability меняет граф выполнения, например создает approval request или handoff, она тоже не является обычным вызовом инструмента. У нее свой след, свой владелец и свои failure modes.
