...
`export-events` 和 `inspect-trace` 也会在命令响应层汇总非空 `idempotency_keys`，因此 support duplicate-ticket path 不必手动扫描每个 `approval_requested` 或 `tool_execution` payload 就能检查。`replay-run` 随后会分别返回 `source_idempotency_keys` 和 `replay_idempotency_keys`，明确说明 replay 是带有自身重复写入 key 的新 run，而不是静默复用原始写入。
