# Policy templates и checklists по кейсам

Эта страница нужна для того, чтобы кейсы из книги можно было не только читать, но и использовать как стартовую заготовку.

Здесь нет идеи "вот универсальная политика на все случаи жизни". Наоборот: смысл в том, чтобы показать, как policy layer начинает отличаться в зависимости от сценария.

## Как читать эти шаблоны

Смотри на них не как на готовый продовый YAML, а как на каркас:

- что система считает risky;
- где проходит approval boundary;
- как отделяются read и write actions;
- какие stop conditions и escalation rules нужны;
- какие traces и audit signals стоит считать обязательными.

## Шаблон 1. Support triage agent

### Что должна обеспечивать политика

Для support triage тебе обычно нужно:
...
## Шаблон 2. Internal knowledge agent

### Что должна обеспечивать политика

В knowledge-сценарии главный риск не в side effects, а в доступе и качестве grounding.
...
## Шаблон 3. Incident coordination agent
...
## Универсальный checklist для policy layer
