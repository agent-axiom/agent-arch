...
Очень частая ошибка с памятью выглядит так:
...
Это полезно потому, что потом можно объяснить:
...
Это удобно тем, что книга не только описывает memory contract, но и показывает runnable skeleton.
