...
`check-controls` returns `healthy`, `missing_controls`, `failed_run_controls`, `preserved_failed_run_controls`, `failed_run_controls_healthy`, `blocking_findings`, and `inventory_drift`, so trace/provenance-related control gaps can be reviewed separately from generic control hygiene. Its `controls.yaml` inputs require `registry_reviewed`, `capability_owners_confirmed`, `memory_provenance_enforced`, and `policy_traces_present`, while `block_if` treats `direct_tool_access_present` and `unmanaged_runtime_present` as hard blockers.
