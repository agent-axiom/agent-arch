...
`inspect-lifecycle` now also surfaces `failed_run_archive_targets` and `controls.failed_run_control_expectations`, so operators can see both the retention side and the trace/provenance control side of degraded-path governance in the same lifecycle summary.
