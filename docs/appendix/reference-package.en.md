...
`export-eval-dataset` bundles several built-in session scenarios into one eval-ready JSON artifact, including a dedicated failed-run drill scenario, a profile lookup scenario (`profile_memory`), a multi-run approval-plus-memory scenario (`mixed_session`) with `required_run_count`, and an approval-backed `support_ticket` scenario with a blocking `sandbox_profile_review` grading rule, and its command summary now surfaces aggregate `failed_runs`, `traceable_failed_runs`, and `latest_failure_reason` too.
