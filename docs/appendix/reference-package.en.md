...
Lifecycle list loaders reject malformed, blank, and duplicate entries with `{key} entries must be strings`, `{key} entries must not be empty`, and `{key} entries must be unique`. `check-retirement` returns `system_id`, `ready`, `missing_steps`, `failed_run_archive_targets`, `support_duplicate_archive_targets`, and `replacement_mode`, so operators can confirm which telemetry/session/approval/control-bundle records must survive retirement for later degraded-path and duplicate-ticket review.
