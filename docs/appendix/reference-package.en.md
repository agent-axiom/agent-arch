...
`dump-events` now returns `trace_id`, `status`, `result`, `event_count`, `event_types`, `failure_reason`, `approval_ids`, `approval_capability_names`, `pending_approval_ids`, `pending_approval_capability_names`, `approval_status_counts`, `idempotency_keys`, `events`, and `failure_reason` in its JSON output for degraded-path drills.
