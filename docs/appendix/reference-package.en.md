...
`export-eval-dataset` bundles several built-in session scenarios into one eval-ready JSON artifact, including a dedicated failed-run drill scenario, and its command summary now surfaces aggregate `failed_runs` and `traceable_failed_runs` counts too.
