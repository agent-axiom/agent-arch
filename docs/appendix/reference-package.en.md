...
There are three starter files in [configs](/Users/if/PycharmProjects/agent-axiom/agent-arch/agent_runtime_ref/configs):
...
These are no longer just static examples. `config.py` can load those YAML files into the runtime, the policy engine, and the rollout policy, so the package is now closer to a real operational skeleton.
