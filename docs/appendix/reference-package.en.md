...
`dump-events` now returns `trace_id`, `status`, `result`, `event_count`, `event_types`, `approval_ids`, `approval_capability_names`, `idempotency_keys`, `events`, and `failure_reason` in its JSON output for degraded-path drills.
...
`export-events` returns `output_path`, `trace_id`, `status`, `result`, `event_count`, `event_types`, `redact_fields`, `approval_ids`, `approval_capability_names`, `idempotency_keys`, and optional `failure_reason`, so redaction and degraded-path evidence are visible in the command summary.
...
`inspect-approvals` now returns `trace_id`, `session_id`, `count`, `approval_ids`, `pending_approval_ids`, `approval_capability_names`, `pending_approval_capability_names`, `approval_status_counts`, `idempotency_keys`, and `approvals`, including capability-session lifecycle fields (`capability_session_id`, `capability_session_status`), delegated authorization context such as `authorization_mode`, `delegated_principal_id`, and `delegated_scope`, and `idempotency_key`, so approval review can be compared directly with session evidence and duplicate-write intent. `resolve-approval` returns `approval_id`, `trace_id`, `session_id`, `capability_name`, `requested_by`, `status`, `reviewer`, `resolution_note`, `capability_session_id`, `capability_session_status`, the same delegated context, `idempotency_key`, and `idempotency_keys` after decision, so approval closure does not drop capability-session, acting-identity, or idempotency lineage.
