...
!!! example "重复工单线索的事故记录"
    在支持分流（support-triage）事故里，记录必须把 `idempotency_key` 和 `trace_id`、`session_id`、`approval_id`、`tool_principal`、`bundle_id`、`rollout_wave` 放在一起。否则，复盘就无法可靠地区分一次状态未知的写入和第二次真实的 `create_ticket` 副作用，也无法把事故转成评测/更新门禁。
