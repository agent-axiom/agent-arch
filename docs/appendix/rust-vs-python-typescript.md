...
!!! note "Канонические сценарии выбора языка (Canonical language cases)"
    Выбор языка (Language choice) должен проходить через три канонических сценария (canonical cases), а не через вкус команды. **Триаж обращений поддержки (Support triage)** обычно начинает с Python/TypeScript для итераций поведения (behavior iteration), но выносит шлюз инструментов (tool gateway), сервис подтверждений (approval service), контроль идемпотентности (idempotency control) и аудиторский след (audit trail) в более строгие платформенные сервисы (stricter platform services), где Rust может быть уместен. **Внутренний ассистент знаний (Internal knowledge assistant)** держит эксперименты поиска (retrieval experiments) и цикл оценки (eval loop) ближе к Python, но требует контрактный слой (contract layer) для сервиса памяти/индекса (memory/index service), происхождения источников (source provenance) и доступа с учетом арендатора (tenant-aware access). **Координация инцидентов (Incident coordination)** сильнее давит на надежность рантайма (runtime reliability), конвейер приема трасс (trace ingestion pipeline), безопасность уведомлений (notification safety) и владение ответом (response ownership), поэтому платформенный контроль (platform control) может раньше потребовать Rust или другой строгий сервисный слой.
