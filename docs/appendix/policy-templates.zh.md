...
!!! example "重复工单线索的策略"
    对于贯穿的支持分流（support-triage）案例，`create_ticket` 不能只是一个写工具：它需要受治理的能力边界、强制幂等键（idempotency key）、可追踪的写入意图、针对 `side_effect_unknown` 的停止条件，以及能在变更发布前捕获重复建单的发布/评测门禁（rollout/eval gate）。
