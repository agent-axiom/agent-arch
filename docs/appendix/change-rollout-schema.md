...
Этого уже достаточно, чтобы связать части V, VII и VIII в одну дисциплину эксплуатации.
...
Свежая версия Anthropic Responsible Scaling Policy показывает полезный governance-паттерн: risk posture должен быть версионированным артефактом, а не фоновым настроением команды. В версии 3.4 Anthropic явно фиксирует effective date, меняет порог automated R&D risk, разрешает Risk Report анализировать риски на отдельную coverage date, требует показывать места публичных redactions и уточняет external review для разных частей полного отчета.[^anthropic-rsp][^anthropic-roadmap]

Для агентной системы это полезно переносить в `rollout_gate_record` как отдельный блок:
...
Смысл не в копировании политики Anthropic. Смысл в том, что high-risk agent capability получает допуск только вместе с версией политики, датой покрытия, списком оцененных возможностей, статусом редактирования публичной версии, ревьюерами и явным решением. Risk report тогда перестает быть посмертной документацией и становится частью допуска агента к новым инструментам, автономии или зоне воздействия.
...
- какая `policy_version` и `coverage_date` обосновывают риск-позицию для высокорисковых возможностей;
...
- risk posture для высокорисковой возможности версионирован, имеет coverage date и связан с отчетом риска, а не восстанавливается из памяти команды;
...
- risk report пишется после факта и не влияет на допуск capability к выпуску;
...
- Есть ли `policy_version`, `coverage_date`, redaction status и reviewers для high-risk risk posture?
