...
Соседний `change.yaml` также задаёт reviewed change surface: `change_id` равен `chg-2026-04-07-support-runtime`, `change_type` — `capability_contract_update`, `risk_level` — `high`, а `rollout_strategy` — `staged_canary`. Его `required_signals` перечисляет release evidence — `design_review_passed`, `offline_eval_passed`, `duplicate_ticket_eval_passed`, `policy_diff_reviewed`, `rollback_plan_ready`, `session_expiry_behavior_checked`, `reinit_policy_reviewed`, `sandbox_profile_reviewed` и `failed_run_drill_checked`, а `approval_roles` фиксирует `platform-owner` и `security-reviewer` как required reviewers.
