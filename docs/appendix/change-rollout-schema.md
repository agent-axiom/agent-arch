...
Пока система маленькая, это может выглядеть терпимо. Но как только появляется несколько owners, high-risk actions и staged rollout, такая схема перестает быть управляемой.
...
Это удобно тем, что книга теперь может показывать не только идею gate, но и runnable skeleton этого gate.
