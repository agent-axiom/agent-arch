...
参考 runtime 在 `retirement.yaml` 中映射了这个结构：`system_id` 和 `replacement_mode` 标识 retiring surface，参考包会使用 `staged_replacement` 来表示 controlled handover，`triggers` 说明为什么启动 retirement（`deprecated_runtime`、`replacement_ready` 和 `unsafe_capability_pattern`），`required_steps` 包含 `freeze_rollout`、`disable_risky_capabilities`、`stop_memory_write`、`expire_paused_runs`、`stop_background_routes`、`freeze_reinitialization`、`revoke_egress`、`archive_audit_state` 和 `set_retired_status` 等 controls，而 `session_control_owner`、`emergency_freeze_owner` 与 `archive_targets` 明确保留 ownership 和 retained evidence。Archive list 会列出 `telemetry_jsonl`、`session_exports`、`approval_history`、`paused_run_state`、`capability_session_state` 和 `runtime_control_bundle`，这些正是 retirement 之后仍需可审查的 records。Executable summary `check-retirement` 会返回 `system_id`、`ready`、`triggers`、`missing_steps`、`required_steps`、`archive_targets`、`failed_run_archive_targets`、`support_duplicate_archive_targets` 和 `replacement_mode`，让操作员能看到未完成的 retirement steps，以及为 degraded-path 与 duplicate-ticket review 保留下来的准确 evidence bundles；malformed retirement step overrides 会失败为 `Assessment signals must be a mapping`、`Assessment signal key must be a string`、`Assessment signal key must not be empty`、`Assessment signal keys must be unique` 和 `Assessment signal value must be a boolean: {field}`。
