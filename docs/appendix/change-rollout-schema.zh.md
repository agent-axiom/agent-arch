...
相邻的 `change.yaml` 也定义 reviewed change surface：`change_id` 是 `chg-2026-04-07-support-runtime`，`change_type` 是 `capability_contract_update`，`risk_level` 是 `high`，`rollout_strategy` 是 `staged_canary`。其中 `required_signals` 列出 `design_review_passed`、`offline_eval_passed`、`duplicate_ticket_eval_passed`、`policy_diff_reviewed`、`rollback_plan_ready`、`session_expiry_behavior_checked`、`reinit_policy_reviewed`、`sandbox_profile_reviewed` 和 `failed_run_drill_checked` 等 release evidence，而 `approval_roles` 将 `platform-owner` 与 `security-reviewer` 标记为 required reviewers。
