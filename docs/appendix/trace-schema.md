...
Эта страница нужна для одного практического перехода: от красивого разговора про observability к структуре событий, которую можно реально экспортировать, читать и использовать в eval workflows.
...
И на runnable package:

- [Опорный пакет](reference-package.md)

## Зачем вообще нужна явная схема трасс

Если у команды нет явной trace schema, обычно происходит одно из двух:
