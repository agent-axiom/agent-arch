...
`export-events` и `inspect-trace` также показывают непустые `idempotency_keys` на уровне command response, чтобы support duplicate-ticket path можно было проверить без ручного просмотра каждого `approval_requested` или `tool_execution` payload.
