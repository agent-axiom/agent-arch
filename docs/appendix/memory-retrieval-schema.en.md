...
The bundled `memory.yaml` keeps this concrete through `seed_records`; each seed record carries a stable `memory_id` plus `tenant_id`, `memory_class`, `kind`, `content`, `source`, `confidence`, `provenance`, and `revision`, so the demo can show both retrieval filtering and record lineage. The reference seeds (`mem-001`, `mem-002`, and `mem-003`) intentionally span `trusted_profile`, `trusted_service`, and `session_state` sources, with provenance such as `ephemeral_session_note`, so retrieval examples include different trust and persistence levels.
