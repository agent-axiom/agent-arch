...
`check-change` возвращает `change_id`, `ready`, `missing_signals`, `missing_failed_run_signals`, `rollout_strategy` и `risk_level`; его required signals включают `duplicate_ticket_eval_passed`, чтобы duplicate-ticket regression evidence проверялась и на change readiness, и на rollout readiness.
