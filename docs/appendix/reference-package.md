...
Lifecycle list loaders reject blank entries with `{key} entries must not be empty`. `check-retirement` возвращает `system_id`, `ready`, `missing_steps`, `failed_run_archive_targets` и `replacement_mode`, чтобы оператор видел, какие telemetry/session/approval records должны пережить retirement ради последующего degraded-path review.
