...
`inspect-lifecycle` теперь тоже показывает `failed_run_archive_targets`, `controls.failed_run_control_expectations`, `controls.failed_run_control_domains`, `controls.failed_run_control_count`, `controls.failed_run_control_summary`, `controls.failed_run_control_status`, `controls.failed_run_control_review_required`, `controls.failed_run_control_owner` и `controls.failed_run_control_source`, так что оператор видит в одном lifecycle summary и retention-сторону, и trace/provenance-сторону degraded-path governance.
