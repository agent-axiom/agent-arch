...
`check-controls` возвращает `healthy`, `missing_controls`, `failed_run_controls`, `preserved_failed_run_controls`, `failed_run_controls_healthy`, `blocking_findings` и `inventory_drift`, чтобы trace/provenance-пробелы можно было разбирать отдельно от общей control hygiene.
...
`inspect-approvals` теперь тоже показывает delegated authorization context, включая `authorization_mode`, `delegated_principal_id` и `delegated_scope`, так что review approval path можно напрямую сопоставить с session evidence. `resolve-approval` возвращает тот же контекст и после принятого решения, чтобы acting-identity lineage не терялся на этапе closure.
