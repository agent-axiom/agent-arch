...
`check-controls` возвращает `healthy`, `missing_controls`, `failed_run_controls`, `preserved_failed_run_controls`, `failed_run_controls_healthy`, `blocking_findings` и `inventory_drift`, чтобы trace/provenance-пробелы можно было разбирать отдельно от общей control hygiene. Его inputs в `controls.yaml` требуют `registry_reviewed`, `capability_owners_confirmed`, `memory_provenance_enforced` и `policy_traces_present`, а `block_if` считает `direct_tool_access_present` и `unmanaged_runtime_present` жёсткими blockers.
