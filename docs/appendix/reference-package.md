...
Та же runtime-control summary опирается на `runtime-controls.yaml`, включая `resume_allowed`, `allow_progress_events`, `allow_elicitation`, `expiry_policy` и `expiry_signal_owner`, чтобы resumable capability sessions явно показывали progress, elicitation и expiry assumptions.
