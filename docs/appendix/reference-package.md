...
`export-eval-dataset` собирает несколько встроенных session-сценариев в один eval-ready JSON artifact, включая отдельный failed-run drill scenario, profile lookup scenario `profile_memory`, multi-run approval-plus-memory scenario `mixed_session` с `required_run_count` и approval-backed сценарий `support_ticket` с blocking `sandbox_profile_review` grading rule, а summary самой команды теперь тоже показывает агрегированные `failed_runs`, `traceable_failed_runs` и `latest_failure_reason`.
