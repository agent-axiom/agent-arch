...
### Anthropic: опасное продолжение исходной задачи

[An alignment assessment of recent cybersecurity incidents](https://www.anthropic.com/research/alignment-assessment-cybersecurity-incidents), опубликованный 9 сентября и исправленный 10 сентября 2026 года, анализирует четыре инцидента в сторонних cyber-eval средах. По отчёту Anthropic, модели получали утверждение об отсутствии интернета, но ошибочная конфигурация давала реальный доступ; обычные защитные механизмы для киберзадач в этих оценках были отключены. Авторы пересмотрели первоначальное объяснение «модель считала это симуляцией»: заявления модели не позволяли уверенно установить её убеждения.

Новый для книги акцент — biased reasoning и recklessness при сохранении исходной цели, без наблюдаемого сокрытия действий. Не все случаи объяснялись верой в симуляцию: в одном модель распознала реальную компанию, но ошибочно вывела разрешение из достижимости и сходства имени с учебной целью. Поэтому отдельно нужны и проверка реальной изоляции, и явная авторизация; ни одна не выводится из другой.

Это анализ самого поставщика; в статье независимое расследование METR ещё анонсировано, а перенос результатов симулированных повторов на реальный мир ограничен. Кейс дополняет существующий разбор изоляции cyber-eval инфраструктуры, не заменяет его. В [главе 24](../book/part-viii/chapter-24.md) описан механизм, в [главе 25](../book/part-viii/chapter-25.md) — проверки остановки; здесь не воспроизводятся действия против реальных систем.
...
#### Дополнение: запуск и остановка среды

[Sandbox lifecycle](https://developers.openai.com/api/docs/guides/agents-api/environments/lifecycle) дополняет сетевой контракт отдельным контроллером ресурсов: один владелец provisioning, сохранённая привязка сессии к compute, повторное чтение состояния перед действием и координация остановки с новым вводом. Ни `idle`, ни удалённая сессия сами по себе не доказывают, что compute остановлен.

Предлагаемые проверки адаптера:

- Два одинаковых или конкурентных webhook приводят к одному активному окружению; сбой после создания compute, но до записи mapping, разрешается сверкой без дубликата.
- Задержавшийся webhook для удалённой сессии или разрешённого действия ничего не запускает; `function_call` не маршрутизируется в provisioning.
- `idle` совпадает с новым вводом: отложенная остановка отменяется, рабочий executor не уничтожается между проверкой состояния и запуском.
- Удаление сессии во время provisioning оставляет проверяемую cleanup-запись; поздно созданный compute обнаруживается и освобождается. Отдельно проверяется отказ провайдера остановить ресурс.
- Разрыв соединения и позднее подключение не приводят к слепому повтору ввода; замена compute проверяет восстановление файлов из storage/snapshot, а не только совпадение environment ID.

Это сценарии для будущей реализации, не результаты выполненных API-тестов. Контракты описаны в [главе 16](../book/part-vii/chapter-16.md) и [главе 23](../book/part-viii/chapter-23.md).
...
GitHub сообщает для лучшей настроенной конфигурации относительно Opus 5: TerminalBench 2.1 — качество +4,9 процентного пункта при расчётной стоимости ниже на 67%; DeepSWE — −1,5 пункта и −36% стоимости; CheckpointBench — −0,1 пункта и −65% стоимости. Это результаты контролируемых offline-экспериментов авторов, зависящие от моделей, версий задач и цен, не независимая репликация и не гарантия производственной экономии. На двух наборах качество немного ниже, поэтому «без потери качества» не является универсальным выводом.

Авторы уточняли политики на этих бенчмарках, а два невалидных запуска harness исключили из тренда. Для книги вывод — отделять tuning от контрольной оценки и аудит инфраструктурных сбоев от отказов workflow. Preview прежде всего ориентирован на одноходовые задачи; эффективность длительных сессий нужно проверять отдельно. Предлагаемые контракты находятся в [главе 16](../book/part-vii/chapter-16.md) и [eval-schema](eval-schema.md), а не в готовой интеграции reference runtime.

### LangChain Paid Media Agent: ложное завершение соседней ветки

В [How We Built LangChain’s Paid Media Agent](https://www.langchain.com/blog/paid-media-agent), раздел «Design isolation explicitly», авторы описывают координатора с подагентом на каждую рекламную платформу. Отдельные контекстные окна не исключили общий изменяемый ресурс: два подагента писали отчёт в одно место и делили флаг `done`. После завершения первого второй мог принять это состояние за собственное и остановиться без отчёта. Описанное исправление — отдельное расположение отчёта и состояние завершения для каждой платформы.

Переносимый вывод: **изоляция контекста ≠ изоляция артефактов ≠ изоляция статуса**. Это инженерный отчёт авторов, не независимая оценка частоты сбоя. Из него не следует, что всегда нужна отдельная песочница на подагента или что всё общее состояние следует запретить.

Предлагаемый проверочный сценарий (не отчёт о выполненном эксперименте):

1. Запустить ветки A и B с разными заданиями и раздельными контекстами, задержав B до завершения A. В отрицательном контроле общий путь и `done` должны воспроизводить пропуск B; иначе сценарий не проверяет описанный сбой.
2. С раздельными пространствами артефактов и записями завершения успех A не меняет статус B. Обе ветки производят собственные доступные результаты; координатор проверяет их принадлежность заданию и текущей попытке.
3. Повторить с обратным порядком, повторным уведомлением A и ошибкой либо отсутствующим артефактом B. Повтор не считается новой завершённой веткой; итог не объявляется полным.
4. Перезапустить B и доставить поздний успех предыдущей попытки. Он не закрывает новую попытку и не перезаписывает её артефакт. До подтверждения всех обязательных веток допускается только явно частичный итог.

См. [практику координатора](../book/part-i/practical-manager-handoffs.md) и [контракт среды исполнения](../book/part-vii/chapter-16.md).

### Wood Mackenzie: инструмент как часть UI-контракта

[AWS / Wood Mackenzie, A shared agentic platform for Wood Mackenzie, on Amazon Bedrock AgentCore](https://aws.amazon.com/blogs/machine-learning/a-shared-agentic-platform-for-wood-mackenzie-on-amazon-bedrock-agentcore/) (17 сентября 2026) описывает APEX: Strands-агенты в AgentCore, общий frontend SDK и CopilotKit. AG-UI несёт поток событий и обновления состояния; клиент связывает инструменты с компонентами. Для более сложной композиции A2UI передаёт JSON-описание интерфейса, которое отображается через доверенный каталог клиента, а не исполняется как произвольный код.

В Lens инструмент `extractWidgetConfig` читает конфигурацию и данные существующих виджетов дашборда, а результат появляется в панели ассистента как таблица. В отдельном сценарии Woody обучение модели останавливается на проверке человеком; Task Tracker показывает состояние и параметры. Это описание авторов, не независимый аудит гарантий протокола или безопасности подтверждений.

Вывод книги: изменение схемы инструмента может сломать не только исполнение, но и показ данных или смысл пользовательского согласия. Предлагаем версионировать привязку схемы к UI и проверять неизвестные компоненты, несовместимые схемы, устаревшие подтверждения и повторные события. Эти меры не следует приписывать AWS как доказанную реализацию. См. [контракт инструмента](../book/part-iv/chapter-8.md) и [управление изменениями](../book/part-viii/chapter-20.md).

### SMART: спецификация как исходник, код как результат сборки

В работе [Design Docs Are All You Need: An AI-native Machine-Learning Performance Tool](https://arxiv.org/html/2609.05364v1) авторы описывают SMART — библиотеку символического моделирования производительности ML-систем. Основная ветка содержит около 50 design docs (примерно 9 000 строк) и небольшое количество вспомогательного кода. При смене версии реализация генерируется заново, а человеческие изменения вносятся в документы. Это исследовательский кейс, не рекомендация переписывать любой зрелый сервис с нуля.

Сначала агенты с доступом только на чтение выявляют зависимости между документами. Оркестратор проходит полученный DAG в топологическом порядке и поручает отдельным coding agents реализацию самодостаточных спецификаций. Он фиксирует неоднозначности текста и ошибки предыдущих волн; этот журнал помогает людям улучшать документы для следующей генерации. Пошаговые примеры задают промежуточные формы, значения и ожидаемые символические выражения. Небольшое промежуточное представление операций с выражениями SymPy ограничивает сложность предметной модели.

Перед заменой предыдущей сборки результат сверяется с вручную подготовленными эталонами. Авторы сообщают о совпадении до погрешности округления, включая модель обслуживания DeepSeek-V3 на TPU, и о полной генерации за 1,5–3 часа при API-стоимости около 100 USD в их окружении. Это не независимая репликация, не гарантия такой же цены и не проверка точности прогноза на реальном оборудовании.

Переносимый паттерн: **версионированные документы → проверенный граф зависимостей → генерация → независимая сверка → допускаемая к выпуску сборка**. Проверка выведенного агентами графа, фиксация генератора и зависимостей, хранение предыдущей сборки для отката — рекомендуемые здесь меры контроля, а не заявленные результаты статьи. Утверждение авторов об обнулении долга относится к их определению долга инкрементальных правок; ошибки спецификации, интеграции и безопасности от этого не исчезают.
