...
Lifecycle list loaders reject blank entries with `{key} entries must not be empty`. `check-retirement` 会返回 `system_id`、`ready`、`missing_steps`、`failed_run_archive_targets` 和 `replacement_mode`，这样操作员就能看到哪些遥测/会话/审批记录必须在退役之后继续保留下来，供后续退化路径评审使用。
