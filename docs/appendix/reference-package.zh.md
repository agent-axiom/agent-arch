...
`export-eval-dataset` 会把几个内置会话场景打包成一个可直接用于评测的 JSON 工件，其中包括一个单独的失败运行演练场景、profile lookup 场景 `profile_memory`、带有 `required_run_count` 的 multi-run approval-plus-memory 场景 `mixed_session`，以及带有阻断型 `sandbox_profile_review` 评分规则的带审批路径 `support_ticket` 场景，而命令摘要现在也会直接显示聚合后的 `failed_runs`、`traceable_failed_runs` 与 `latest_failure_reason`。
