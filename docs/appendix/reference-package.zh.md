...
`check-controls` 会返回 `healthy`、`missing_controls`、`failed_run_controls`、`preserved_failed_run_controls`、`failed_run_controls_healthy`、`blocking_findings` 和 `inventory_drift`，这样追踪/来源证明相关缺口就能和普通控制卫生分开审阅。
