...
`inspect-lifecycle` 现在也会显示 `failed_run_archive_targets`、`controls.failed_run_control_expectations` 和 `controls.failed_run_control_domains`，这样操作员在同一个 lifecycle summary 里就能同时看到 degraded-path governance 的 retention 侧面与 trace/provenance control 侧面。
