...
`inspect-agent` 会返回 `agent_id`、`display_name`、`owner_team`、`runtime_principal`、`approved_capabilities` 和 `catalog_capabilities`，让 inventory review 可以把 configured identity 与 capability catalog 对照起来。在内置的 `agent.yaml` 中，该 identity 归 `agent_platform` 所有，以 `svc-support-triage-ref` 运行，并且只 approved `search_docs` 与 `create_ticket`；随后 capability catalog 将 `search_docs` 标记为由 `knowledge_platform` 拥有并绑定到 `svc-knowledge-reader`，将 `create_ticket` 标记为由 `support_platform` 拥有并绑定到 `svc-ticket-writer`。每条 `catalog_capabilities` entry 也会携带 `name`、`risk_tier`、`network_access` 和 `allowed_egress`，让 reviewers 在同一个 response 中看到 capability identity 与 egress posture。Identity/catalog loaders 会用这些错误校验 shape：`'agent' must be a mapping`、`agent.id is required`、`agent.display_name is required`、`agent.owner_team is required`、`agent.runtime_principal is required`、`'approved_capabilities' must be a list`、`'capabilities' must be a mapping`、`Capability spec for {name!r} must be a mapping`、`capabilities.{capability_name}.{key} must be a string`、`capabilities.{capability_name}.{key} is required`、`{label} must be a string` 和 `capabilities.{capability_name}.timeout_seconds must be positive`、`'{label}.{key}' must be an integer` 和 `'{label}.{key}' must be a boolean`、`{label}.approval must be a string`、`{label}.approval must not be empty` 和 `{label}.approval is not supported: {approval}`。
