...
现在，运行时也会把工具路径中的失败类结果，例如验证失败，当成一等运行结局来处理。它不再假装这次运行仍然成功，而是记录失败运行、为 failed 或 denied runs 发出明确携带 `failure_reason` 的 `run_failed` 事件以及 terminal `run_complete.failure_reason`，并在会话导出、trace inspection、replay summaries 与 CLI 输出中通过 `failure_reason` 字段同时保留这个状态以及具体失败原因。
