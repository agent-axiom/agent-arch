...
`check-change` 会返回 `change_id`、`ready`、`missing_signals`、`missing_failed_run_signals`、`rollout_strategy` 和 `risk_level`；它的 required signals 包括 `duplicate_ticket_eval_passed`，因此重复工单回归证据会同时进入 change readiness 和 rollout readiness。
