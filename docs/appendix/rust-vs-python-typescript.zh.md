...
[GitHub Copilot 的 Rust 迁移](https://github.blog/ai-and-ml/generative-ai/migrating-the-github-copilot-runtime-to-rust-using-copilot/)说明语言和进程边界是不同选择。原生核心既可嵌入 Python/TypeScript 应用，也可保持独立服务器，不必因此重写面向用户的层。

| 标准 | 通过 FFI 在进程内运行 | 通过 IPC/RPC 在独立进程运行 |
| --- | --- | --- |
| 启动与内存 | 无需强制启动第二个进程；仍需加载库、初始化并占用核心内存 | 独立启动和进程内存；复用服务器可分摊成本 |
| 数据传输 | 减少必须的 IPC 往返；复制、序列化和回调仍可能昂贵 | 显式协议、序列化、队列与背压 |
| 故障边界 | 原生崩溃可能终止宿主应用 | 可独立重启进程；会话状态仍需恢复 |
| 生命周期 | 句柄、缓冲区、线程、回调及释放顺序的归属 | 子进程启动、监督、关闭与清理的归属 |
| 兼容性 | ABI 版本、平台构建、错误及内存传递规则 | 协议版本、消息兼容性和状态格式 |

这并非自动推荐 FFI。集成前应规定缓冲区与句柄所有权、禁止释放后的回调、明确取消传播、错误转换以及跨语言边界的 panic 处理。应确定哪些调用需要异步执行，不能阻塞宿主事件循环。Safe Rust 不会自动让外部代码或 FFI 契约变得安全。

独立进程有助于故障分离，但本身不是沙箱：仍需权限、文件系统与网络限制。嵌入式运行时也不意味着不可信代码必须在宿主进程中执行；工具仍可放在单独的执行边界之后。

应在相同任务与策略下比较冷/热启动、整个进程树的内存、会话创建与释放、p50/p95 延迟、取消、回调负载和崩溃恢复。将无真实模型的开销测试与使用模型的用户任务分开测量；运行时节省不等于整个智能体获得相同比例的加速。渐进迁移见 [Copilot 案例](rust-agent-platforms.zh.md)。
