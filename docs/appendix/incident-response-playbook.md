...
!!! note "Канонические сценарии реагирования (Canonical response cases)"
    Реагирование на инцидент (incident response) должно выбирать разные пути сдерживания (containment paths) для трех канонических сценариев (canonical cases). **Триаж обращений поддержки (Support triage)** сначала замораживает записывающую возможность (write capability), сохраняет доказательства подтверждения (approval evidence), `idempotency_key`, статус побочного эффекта (side-effect status) и волну раскатки (rollout wave). **Внутренний ассистент знаний (Internal knowledge assistant)** сначала ограничивает область поиска (retrieval scope), приостанавливает записи в память (pauses memory writes), сохраняет происхождение источников (source provenance), доказательства границы арендатора (tenant boundary evidence) и решение контроля доступа (access-control decision). **Координация инцидентов (Incident coordination)** сначала фиксирует статус эскалации (escalation status), побочные эффекты уведомлений (notification side effects), владение ответом (response ownership), состояние передачи управления (handoff state) и владельца экстренного отката (emergency rollback owner).
