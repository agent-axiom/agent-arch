...
## Политика траектории или последовательности

Политика, которая оценивает не только текущее действие, но и доверенный снимок
предшествующей последовательности: ранее связанные fingerprints, накопленные
счётчики в заданном окне, обязательные предшественники и состояние
подтверждений. Такая политика не должна восстанавливать историю из model
context или compaction summary и закрыто отказывает при отсутствующем,
повреждённом, устаревшем или непроверенном снимке. Подтверждение связывается с
каноническим fingerprint всего запроса, включая действие, tenant/subject,
ожидаемую историю, последовательность, окно, версию политики, значимые
fingerprints и дельты счётчиков. Накопительная арифметика должна иметь явные
границы scale/range и не зависеть от глобального numeric context.

Читать дальше:

- [Схема набора политик и контракта подтверждения](policy-bundle-schema.md)
- [Схема трасс и каталог событий](trace-schema.md)
- [Исполняемые сценарии политики траектории](../companion/examples/trajectory-policy-scenarios.md)
