...
!!! note "Обновление safe-agent схем"
    Обновления 17-19 мая 2026 года связали прозу, приложения и guards для safe-agent архитектуры: модель угроз MCP и контракт `mcp_server`, контракт доверия для A2A handoff и артефакт trust-delegation, карта defense-in-depth controls, запись verifier verdict, запись governance action, сопоставление телеметрии NIST AI RMF, memory poisoning review fields и unified agent threat evidence теперь отражены в [trace schema](appendix/trace-schema.md), [eval schema](appendix/eval-schema.md) и [memory/retrieval schema](appendix/memory-retrieval-schema.md).
