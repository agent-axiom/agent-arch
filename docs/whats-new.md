...
!!! note "Обновление схем safe-agent"
    Обновления 17-19 мая 2026 года связали прозу, приложения и защитные проверки для архитектуры safe-agent: модель угроз MCP и контракт `mcp_server`, контракт доверия для передачи A2A (handoff) и артефакт делегирования доверия (trust-delegation), карта defense-in-depth-контролей, запись вердикта проверяющего (verifier verdict), запись управленческого действия (governance action), сопоставление телеметрии NIST AI RMF, поля проверки отравления памяти (memory poisoning) и единая evidence-модель угроз агентам теперь отражены в [trace schema](appendix/trace-schema.md), [eval schema](appendix/eval-schema.md) и [memory/retrieval schema](appendix/memory-retrieval-schema.md).
