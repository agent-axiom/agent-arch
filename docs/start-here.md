...
Если ты впервые попал в эту книгу, начни с одного вопроса: тебе нужен впечатляющий демо-агент или система, которая выдержит производственную реальность (production reality)?

Эта книга написана для второго случая. Ее полезно читать как один аргумент о том, как агентные системы взрослеют: от прототипов, перегруженных промптами (prompt-heavy prototypes), к управляемым системам с границами доверия (trust boundaries), слоем политик (policy layer), подтверждениями (approvals), наблюдаемостью (observability), оценками (evals) и жизненным циклом.

Агентов строить скучно, но результат ошеломляющий: дисциплина вокруг границ доверия (trust boundaries), трасс (traces), подтверждений (approvals) и раскатки (rollout) превращает демо в систему, которую можно безопасно улучшать.
