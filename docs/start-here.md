...
## Что это за книга (what kind of book)
...
## Маршрут на 30 минут (30-minute route)

Если времени мало, пройди этот путь (path):
...
После этого маршрута у тебя уже должна сложиться рабочая рамка (working frame):

- где у агента проходят реальные границы доверия (trust boundaries);
- как выглядит безопасное исполнение инструментов (safe tool execution);
...
## Маршруты по ролям (role-based routes)

### Если ты инженер продукта (product engineer)
...
Этот путь нужен, чтобы быстро перейти от идеи агента (agent idea) к рабочей архитектуре (working architecture).

### Если ты платформенный инженер (platform engineer)
...
Этот путь нужен, если ты собираешь платформенный каркас (platform skeleton), а не локальную обвязку вокруг одной модели (model wrapper).

### Если ты инженер по безопасности (security engineer)
...
Этот путь полезен, если тебе нужно видеть не только риск модели (model risk), но и риск реального исполнения (real execution risk).

### Если ты руководитель или архитектор (leader or architect)
...
Этот путь нужен, если главная задача не собрать демо (demo), а удержать инициативу в рабочей эксплуатационной дисциплине (operating discipline).

## Если хочешь сразу код и артефакты (code and artifacts)

Если важнее не линейное чтение (linear reading), а исполняемые опоры (executable supports), начни с этого:
...
## Если нужно быстро решить конкретную задачу (specific task)

### Безопасное выполнение инструментов (safe tool execution)
...
### Память и извлечение (memory and retrieval)
...
### Главная линия схем безопасного агента (Safe-agent schema spine)
...
## Что открыть рядом с книгой (companion references)
...
Если книга кажется тебе ближе к реальной инженерии (real engineering), чем очередной AI-лендинг (AI landing page) про “автономность”, значит ты попал по адресу.
