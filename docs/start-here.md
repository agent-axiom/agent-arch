...
Это не руководство по одному фреймворку (framework) и не каталог AI-возможностей (AI feature catalog). Это практическая архитектурная книга для команд, которым нужно запускать агентов в реальной среде, где есть пути записи (write paths), человеческие подтверждения (human approvals), границы доступа, телеметрия, оценки и явная эксплуатационная ответственность (operational ownership).
