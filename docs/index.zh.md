...
它不是某个框架的手册，不是提示技巧合集（prompt tricks），也不是 AI 市场巡礼。本书站在具体 SDK 和平台文档（platform docs）之上，去回答更棘手的问题：智能体到底该被允许做什么，写入路径（write path）应该怎样受限，应该观察什么，变更应该怎样发布，系统上线后到底谁负责。
