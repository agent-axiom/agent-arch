...
!!! example "Сквозной кейс: кто чинит общий слой"
    После duplicate-ticket инцидента продуктовая команда должна владеть тем, как support workflow отвечает пользователю и когда эскалирует. Но platform team должна владеть runtime retry policy, idempotency contract, trace schema и rollout gate, потому что эти решения нужны не одному агенту, а всем write-capability сценариям. Если это не разделить заранее, следующий инцидент снова превратится в спор о владельце.
...
## 11. Быстрый тест зрелости для platform operating model

Команде не стоит думать, что она уже решила проблему platform ownership, только потому, что создала platform team и задокументировала несколько review rules.

Более сильная планка такая:

- platform и product ownership явны именно на том слое, где реально происходят инциденты;
- golden paths убирают хаос, а не просто публикуют reusable pieces;
...
- платформа измеряется снижением duplication и bypasses, а не театром вокруг adoption.

Если большинство этих условий не выполняется, у организации уже может быть platform label, но реального platform operating model у нее пока нет.
