...
## 1. Почему вызов инструментов это не просто “модель выбрала функцию”
...
Проблема в том, что вызов инструментов это не только вопрос “что модель захотела вызвать”. Это вопрос:

- какие действия вообще допустимы;
- в каком контексте они разрешены;
- кто отвечает за контракт вызова;
- где происходят валидация, повторы и побочные эффекты;
- как система ведет себя при частичном сбое.

Поэтому модель выполнения нужно проектировать как слой платформы, а не как тонкий helper вокруг LLM API.
...
Вместо этого у тебя должен быть слой выполнения, который:
...
## 3. Каталог инструментов это интерфейс платформы, а не список случайных функций

Если смотреть на каталог как на “папку с вызовами”, он быстро превращается в мусорную свалку интеграций. Гораздо полезнее считать его публичным интерфейсом слоя выполнения.

Хороший каталог инструментов обычно хранит:
