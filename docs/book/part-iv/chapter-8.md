...
Чем больше tools одновременно видит модель:

- тем больше токенов уходит на их описания;
- тем длиннее становятся похожие друг на друга contracts;
- тем труднее различать почти одинаковые capabilities;
- тем сильнее размывается внимание на этапе выбора.

Именно поэтому "давайте просто покажем модели все инструменты" обычно плохо заканчивается. Качество выбора падает не потому, что модель "стала глупее", а потому, что candidate set стал слишком шумным.

Хороший practical pattern здесь обычно называется semantic tool filtering:

- полный registry продолжает жить в platform layer;
- но конкретному run показывают только узкий релевантный поднабор;
- часто это 3-5 tools, а не несколько десятков.
...
## 9. Практические правила для execution layer

Если нужен короткий набор правил, который реально помогает в production, он обычно такой:

1. У каждого инструмента должны быть owner, schema, risk class и contract lifecycle.
2. Read и write paths нужно различать до первого rollout, а не после первого инцидента.
3. Все write tools должны иметь idempotency strategy до того, как их увидит реальный трафик.
4. Все tool outcomes нужно нормализовать до передачи обратно модели.
5. Если side effect уже мог произойти, безопаснее остановиться, чем делать слепой retry.

## 10. Что команды чаще всего делают неправильно

Execution layer почти всегда ломают одинаково:
...
## 11. Простой кодовый шаблон для execution layer
...
## 12. Tool results тоже нужно проектировать, а не просто возвращать как есть
...
## 14. Быстрый тест зрелости для tool layer

Команде не стоит считать execution layer зрелым только потому, что tools уже можно вызывать.

Более сильный стандарт такой:

- каталог явный и у него есть owner;
- различие между read, write и orchestration семантикой видно сразу;
- idempotency спроектирована до того, как на это укажет инцидент;
- неопределенность не прячется за фальшивым success;
- модель не становится прямой интеграционной поверхностью.

Если одного-двух пунктов не хватает, система еще может работать. Если не хватает большинства, tool layer пока остается только прототипной обвязкой.
