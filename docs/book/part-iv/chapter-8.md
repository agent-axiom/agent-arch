...
### Схема инструмента может быть контрактом с интерфейсом

В [платформе Wood Mackenzie на AgentCore](https://aws.amazon.com/blogs/machine-learning/a-shared-agentic-platform-for-wood-mackenzie-on-amazon-bedrock-agentcore/) AG-UI передаёт события между агентом и приложением, а клиент связывает вызов инструмента и его аргументы с UI-компонентом. Для более сложных интерфейсов агент возвращает декларативный A2UI-план, который клиент отображает через собственный каталог доверенных виджетов. Это два связанных пути, а не утверждение, что любой вызов инструмента автоматически является A2UI.

Следовательно, схема аргументов и результата может иметь ещё одного потребителя — renderer. Backend может принять новое поле или единицу измерения, а старый интерфейс — скрыть её, неверно показать сумму либо подтвердить не то действие. Предлагаемый контракт книги связывает `tool_id`, `tool_schema_version`, `render_binding_version` и версию каталога компонентов; это условные поля, не обязательные поля AG-UI/A2UI и не уже реализованная функция эталонного рантайма.

Клиент должен проверять схему и выбирать компоненты только из разрешённого каталога. Неизвестный виджет или несовместимая привязка дают явное состояние «не поддерживается», а не загрузку произвольного кода. Допустим проверенный текстовый fallback для чтения; чувствительное действие нельзя молча подтверждать через упрощённый интерфейс, скрывающий значимые поля. Доверенный виджет не делает его данные достоверными и не заменяет серверную авторизацию.

Нажатие кнопки не является самодостаточным approval. Сервер связывает подтверждение с проверенным субъектом, конкретным действием, ресурсом, точными значимыми аргументами и ревизией состояния. Если после показа формы агент или пользователь меняет эти данные, старое подтверждение недействительно: нужен актуальный просмотр и новое решение. Отображение, получение события и успешное выполнение инструмента — разные состояния. Проверки обновлений описаны в [главе 20](../part-viii/chapter-20.md).
