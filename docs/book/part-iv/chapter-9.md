...
### Частичное OAuth-согласие: запрос не равен выданным правам

Cloudflare показывает конкретный случай: клиент помечает scopes как обязательные или необязательные, а пользователь может снять необязательные scopes в текущем запросе авторизации. Эти категории применяются только к scopes, запрошенным в данном потоке; они не заставляют каждый запрос включать весь каталог прав клиента. После обмена authorization code приложение должно проверить фактически выданный набор, а не считать весь запрос одобренным. Источник: [Cloudflare, From all-or-nothing to task-based OAuth consent](https://blog.cloudflare.com/task-based-oauth-consent/) (20 августа 2026).

Для адаптера полезно разделять `requested_scopes` (запрос авторизации), `granted_scopes` (подтвержденные права текущего credential) и `required_scopes` (минимальные права конкретной операции). Последнее поле — контракт инструмента, а не список обязательных scopes на экране согласия. Проверка `required_scopes ⊆ granted_scopes` необходима, но недостаточна: отдельно проверяются субъект, ресурс, audience, срок действия, политика задачи и нужное подтверждение действия.

Например, условный сервис получил запрос `tickets.read` и `tickets.write`, но выдал только `tickets.read`. Агент может подготовить сводку, если чтение само по себе входит в задачу; обновление обращения блокируется **до вызова инструмента**. Это не повод переключаться на более привилегированную учетную запись или повторять запрос расширенного доступа автоматически. Если запись обязательна для результата, задача остается незавершенной: агент сообщает о недостающем праве. Формат решения и проверочные сценарии см. в [контракте набора политик](../../appendix/policy-bundle-schema.md).

## 6. Ядро MCP 2026-07-28 не хранит сессию

Спецификация MCP 2026-07-28 сделала базовый протокол самодостаточным на уровне каждого запроса: ядро не хранит протокольную сессию, а согласование версии выполняется для каждого сообщения.[^mcp-2026-07-28][^mcp-2026-07-28-release] Из ядра удалены прежняя последовательность `initialize`/`initialized` и протокольный заголовок с идентификатором сессии. Это упрощает горизонтальное масштабирование и восстановление после сбоя, но не отменяет прикладного состояния.
...
Расширенная поддержка MCP в AgentCore Gateway остается полезным примером агрегации `tools/list`, `prompts/list`, `resources/list` и шаблонов ресурсов, переноса `outputSchema`, динамического списка и аннотаций инструментов.[^aws-agentcore-extended-mcp] Для трассы полезны `listing_mode`, `listed_under_principal`, `output_schema_hash` и `tool_annotations`. Однако описанное AWS поведение от 2025-11-25 зависит от поставщика и версии сервиса, а не задает универсальную модель актуального ядра MCP.[^aws-stateful-mcp] Платформа должна датировать такой профиль совместимости и не переносить старую семантику продолжения сессии в общий протокол.
...
### Self-hosted executor не означает локальный harness

В [OpenAI Agents API](https://developers.openai.com/api/docs/guides/agents-api/environments/self-hosted) harness работает у OpenAI, а `codex exec-server` запускается в выбранном владельцем окружении: выполняет shell-команды, читает и записывает файлы, использует локальные MCP-серверы. Executor регистрируется через API и сам устанавливает исходящий WebSocket для команд и результатов; после разрыва он переподключается. Это не полностью локальное исполнение управляющего контура.

Исходящее соединение не означает, что данные остаются внутри: результаты команд, включая прочитанное содержимое, могут возвращаться облачному harness. Владелец обязан определить допустимые mounts, локальные credentials, сеть и передаваемые результаты, а также обеспечить изоляцию выбранной среды. Название self-hosted и установка executor сами по себе не создают sandbox. Не смешивайте этот API с отдельно описанным Agents SDK Sandbox Agents.

Разделяйте окружения по пользователю или workload: совместно размещённые агенты видят общие файлы, credentials и ресурсы. Документация требует для каждой сессии собственный environment ID и executor; повторное использование образа не означает безопасное совместное использование живого окружения. Ограниченный ключ подключения отличается от основного ключа приложения, который должен оставаться снаружи; см. [§12.1 главы 16](../part-vii/chapter-16.md).

Переподключение восстанавливает транспорт, но не доказывает exactly-once исполнение прерванной команды. Сверяйте статус операции и evidence до повтора побочного эффекта. Connection events `pending`, `connected`, `failed` не заменяют финальный результат задачи. Это предлагаемые эксплуатационные проверки поверх документированного подключения, не доказательство реализованной идемпотентности API.
