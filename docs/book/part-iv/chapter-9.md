...
!!! info "Актуальность главы"
    ...
    Что изменилось после предыдущей проверки: MCP security boundary, tool-poisoning surfaces, A2A trust model и замечания по готовности к публикации теперь имеют конкретное покрытие контрактами и проверки поверхности документации.
...
Для MCP полезно держать [MCP threat model](../../appendix/trace-schema.md) не как общий страх перед интеграциями, а как проверочную матрицу у каждой подключаемой возможности. Минимальный вариант выглядит так:
