...
- **tool poisoning** — описание инструмента или результат пытаются изменить поведение модели; контроль: отделять результаты инструментов от инструкций и держать список разрешенных контрактов.
- **rug pull attack** — ранее одобренный MCP-сервер меняет инструменты, области доступа или поведение после ревью; контроль: фиксация версии, повторная аттестация, проверка различий и быстрый путь карантина.
- **tool shadowing** — новый инструмент маскируется под похожий одобренный инструмент и перехватывает намерение модели; контроль: уникальные имена возможностей, владение реестром и семантическое ревью перед публикацией.
- **confused deputy** — агент вызывает действие с чужими или слишком широкими делегированными полномочиями; контроль: проверка субъекта, привязки к цели, состояния подтверждения и решения политики прямо перед побочным эффектом.
- **over-scoped tokens** — MCP-сервер получает больше OAuth-областей доступа, чем нужно конкретной операции; контроль: короткоживущие токены с ограниченными областями доступа, раздельные области доступа по инструментам и запрет широких постоянных секретов.
- **data exfiltration through legitimate channels** — данные уходят через разрешенный результат инструмента, уведомление или комментарий в тикете; контроль: проверки DLP, классификация результатов, границы арендатора и ревью для рискованных записей.
- **supply-chain attack** — скомпрометированный сервер, пакет или адаптер становится доверенной возможностью; контроль: происхождение, подписанные артефакты, ревью зависимостей и закрепленная ответственность владельца.
- **replay/tampering** — запрос, ответ или сеанс с сохранением состояния переигрываются или меняются между шагами; контроль: подпись запросов, nonce/ключи идемпотентности, корреляция трасс и срок жизни сессии.
- **sandbox escape** — инструмент или адаптер выходит за пределы сетевой, файловой или процессной границы; контроль: эфемерная песочница, минимальные правила выхода во внешнюю сеть, изоляция секретов и сдерживание на уровне среды исполнения.
...
Эти поля нужны не ради бюрократии. `schema_hash` и `tool_definition_hash` ловят внедрение схемы инструмента и подмену после подтверждения. `token_scope`, `token_ttl` и `user_delegation_required` ограничивают confused-deputy сценарии. `return_value_filtering` считает результаты инструментов недоверенным содержимым, включая внедрение инструкций через результаты инструментов. `server_isolation_profile` и `replay_protection` делают выход из песочницы, повтор воспроизведения и подмену достаточно видимыми для сдерживания.
...
- `host` (`узел`) — это приложение или рантайм, который управляет сессией и решает, к каким возможностям вообще подключаться;
- `client` (`клиент`) — это протокольный компонент, который узел создает для связи с конкретным MCP-сервером;
- `server` (`сервер`) — это граница, которая публикует инструменты, ресурсы и другие поверхности возможностей, а затем возвращает структурированный результат.

Из этого следуют две очень практичные вещи:

- один узел может одновременно держать несколько клиентов;
- один рантайм агента может работать сразу с несколькими MCP-серверами, не смешивая их в один неразличимый комок интеграций.
