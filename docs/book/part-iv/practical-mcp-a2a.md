...
Расширенный [контракт доверия при передаче управления A2A (A2A handoff trust contract)](../../appendix/trace-schema.md) должен быть еще конкретнее:
...
- наследование политик: downstream agent наследует ограничения исходного запроса, а не получает более широкую свободу;
...
- атрибуция сбоев: telemetry различает ошибку инициатора, ошибку исполнителя, policy denial, tool failure и недоступность внешней среды.
