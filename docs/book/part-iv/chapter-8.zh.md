...
OpenAI 的实践指南里还有一个很有用的简化：工具不只适合按 `read` 和 `write` 分，也适合按它们在系统中的角色来分。[^openai-practical]

- `data tools` 读取并返回上下文：状态检查、检索、CRM 读取；
- `action tools` 改变外部世界：创建工单、发送邮件、更新记录；
- `orchestration tools` 帮助运行时自身工作：请求审批、交接、调用规划器。

这两条分类轴可以一起使用：

- `data tools` 通常更接近 `read`；
- `action tools` 通常更接近 `write`；
- `orchestration tools` 可能两者都有，但它们有单独的运营含义。

Anthropic 的工作流分类在这里又补上了一层很有用的纪律。[^anthropic] 目录不应该只告诉模型“有哪些工具存在”，还应该说明“这些工具在哪些编排模式里才是安全可用的”。

例如：

- `data tool` 也许适合放进 `routing`、`prompt chaining` 或 `parallelization`；
- `write action tool` 也许只适合出现在审批中断之后，或者只适合放进边界非常紧的工作流；
- `orchestration tool`，比如 `request_human_approval` 或 `handoff_to_specialist`，会直接改变执行图，因此需要更严格的追踪和负责人规则；
- `orchestrator-workers` 模式可能需要一个显式的 worker-safe 目录子集，而不是把父级全部工具表面都暴露出去。
...
在 [Wood Mackenzie 的 AgentCore 平台](https://aws.amazon.com/blogs/machine-learning/a-shared-agentic-platform-for-wood-mackenzie-on-amazon-bedrock-agentcore/)中，AG-UI 在智能体与应用之间传递事件，客户端将工具调用及参数绑定到 UI 组件。对于更复杂的界面，智能体返回声明式 A2UI 蓝图，由客户端可信组件目录中的原生组件渲染。这是两条相关路径，不代表每次工具调用都自动属于 A2UI。

因此，参数与结果模式可能还有一个消费者：渲染器。后端接受了新字段或单位，旧界面却可能隐藏它、显示错误金额或批准错误操作。本书建议将 `tool_id`、`tool_schema_version`、`render_binding_version` 与组件目录版本关联。这些是示意字段，不是 AG-UI/A2UI 的必需字段，也不是参考运行时已实现的功能。

客户端应验证模式，只从获准目录选择组件。未知组件或不兼容绑定应产生明确的不支持状态，而不是加载任意代码。只读结果可采用已验证的文本回退；敏感操作不得悄悄退回隐藏关键字段的简化审批界面。可信组件不代表其数据可信，也不能替代服务端授权。

点击按钮不是充分的审批证据。服务端应将确认绑定到已验证主体、具体操作、资源、准确的关键参数及状态修订号。展示表单后，如果智能体或用户修改这些值，旧审批应失效：必须重新展示当前预览并作出新决定。渲染、收到事件和工具执行成功是不同状态。更新检查见[第 20 章](../part-viii/chapter-20.zh.md)。
...
1. 每个工具都应该有负责人、schema、风险等级和契约生命周期。
2. 读路径和写路径必须在第一次 rollout 之前就分开，而不是等第一次事故之后再补。
3. 每个写工具在接触真实流量前都必须有幂等策略。
4. 所有工具结果都应该先被规范化，再返回给模型。
5. 只要副作用可能已经发生，停下来通常比盲目重试更安全。
