...
Anthropic 的 工作流分类 在这里又补上了一层很有用的纪律。[^anthropic] 目录不应该只告诉模型“有哪些工具存在”，还应该说明“这些工具在哪些编排模式里才是安全可用的”。
