...
## 2. Идемпотентность это не nice-to-have, а базовая страховка
...
!!! example "Сквозной кейс: неизвестный итог тикета"
    В support-triage кейсе самый опасный момент — не ошибка в рассуждении, а таймаут после вызова `create_support_ticket`. Если helpdesk мог уже создать тикет, повторный вызов без idempotency key превратит один клиентский запрос в два инцидента. Правильная ветка сначала ищет тикет по correlation ID, затем либо привязывает найденный результат к trace, либо останавливает run и просит оператора подтвердить состояние.
...
Именно поэтому limit полезно задавать не только на уровне всего сервиса, но и:
...
То есть граница отката это не “потом посмотрим”. Это часть контракта инструмента и workflow.
