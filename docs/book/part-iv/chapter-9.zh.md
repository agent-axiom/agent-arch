...
!!! info "时效说明"
    ...
    自上次审查以来的变化：MCP 安全边界、工具投毒攻击面、A2A 信任模型和出版就绪问题，现在都有具体的 contract coverage 与 docs-surface guards。
...
对 MCP 来说，[MCP threat model](../../appendix/trace-schema.zh.md) 不应该只是“外部集成有风险”这种笼统提醒，而应该成为每个接入能力的审查矩阵。一个最小版本可以这样看：
