...
- 一个智能体运行时也可以同时和多个 MCP servers 工作，而不是把它们揉成一个分不清边界的集成大泥团。
...
- 用于实验的本地临时 MCP servers；
- 用于共享生产能力的受治理的远程 MCP servers；
- 用于已批准服务器的发现 / 门户层；
- 位于访问边界的身份执行；
...
很多团队都会在这里犯同一种成熟度错误。他们把 MCP 标准化成协议，却仍然用非正式方式接入服务器：有人把端点发进群聊，另一个团队把它复制进本地配置，很快就没人说得清哪些 MCP servers 是已批准的，哪些只是实验性的，哪些则悄悄绕过了正常评审。
...
### 5.2. Shadow MCP 是 影子 API 问题的新版本

当 MCP 变得非常容易接入时，团队也会得到一种新的 shadow IT 形式：未登记的 MCP servers 已经承载真实业务动作，但其负责人、评审和控制模型却没有被正式化。[^cloudflare-mcp]
...
- 认证依赖长期本地密钥；
- 没有统一审计轨迹可以说明哪个智能体调用了哪个 MCP endpoint；
- 平台团队往往要到 事故之后才知道它存在。
...
如果这些问题答不上来，问题就已经不只是“集成文档不完整”，而是平台在自己的控制模型之外制造了一条 影子能力路径。
...
AWS 最近的另一个信号也很有价值：一旦 MCP clients 和 servers 开始支持更强的有状态交互模式，MCP 就不再只是一个无状态工具封套，而会更像一种带会话的运行时协议。[^aws-stateful-mcp]
...
- 传输是否显式：direct、MCP、sandboxed exec？
- 系统是否区分 可信结果和部分可信结果？
- 业务载荷之外是否保留执行事实？
- 对高风险执行是否使用了 短生命周期沙箱？
- 能否解释为什么某项能力会在这次运行中被允许？

如果这些答案很模糊，那说明 能力层 还只是“一堆好用的集成”，而不是受管理的平台层。
