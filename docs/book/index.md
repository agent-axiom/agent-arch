...
Это главная входная страница (main entry page) самой книги. Если нужен самый короткий путь (shortest path) в основной текст (main text), начинай с [Части I. Основания](part-i/index.md). Если сначала хочется увидеть структуру и статус публикации (structure and publication status), открой [План книги (Book Plan)](plan.md).

## Что обещает эта книга (book promise)

У книги один главный тезис (main thesis): агенту нужна платформа (platform), а не магия.

Агентов строить скучно, но результат ошеломляющий: вместо эффектного разового трюка (one-off trick) появляется система, которую можно ограничивать (constrain), наблюдать (observe), выпускать (ship) и улучшать без гадания (improve without guessing).
...
- видеть минимальный набор платформенных слоев (minimum platform layers), без которых нельзя пускать систему к рискованным действиям (risky actions);
...
Если нужен самый короткий полезный маршрут (shortest useful path), иди так:
...
У книги есть два практических слоя (practical layers):

- `Стабильное ядро` (stable core): части I-VII, особенно главы 1-12 и 18;
- `Быстро меняющийся слой` (fast-moving layer): глава 13, часть VIII и исследовательские страницы приложений (research appendix pages).

Если ты читаешь книгу впервые (first-time reader), сначала лучше пройти стабильное ядро (stable core), а потом вернуться к более подвижному слою (fast-moving layer).
