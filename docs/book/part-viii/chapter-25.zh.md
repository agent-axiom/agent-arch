...
!!! example "贯穿案例：工单写入（ticket-write）的控制评测"
...
**控制评测案例主线说明（Control-eval case-spine note）：**[评测门禁与验证器契约（eval gate and verifier contract）](../../appendix/eval-schema.zh.md)应该把三个规范案例（canonical cases）作为不同的行为与控制评测表面（behavioral and control eval surfaces）来测试。支持分诊（Support triage）需要重复预防（duplicate prevention）、载荷变更检查（payload-mutation check）、[审批路径误用检查（approval-path misuse check）](../../appendix/approval-schema.zh.md)、旧网关路由探针（old-gateway-route probe）和应急禁用断言（emergency-disable assertion）。内部知识助手（Internal knowledge assistant）需要来源扎根评测（source-grounding eval）、租户过滤绕过探针（tenant-filter bypass probe）、不当记忆写入检查（improper-memory-write check）、[检索投毒场景（retrieval-poisoning scenario）](../../appendix/memory-retrieval-schema.zh.md)和新鲜度回归门禁（freshness regression gate）。事故协调（Incident coordination）需要升级路径检查（escalation-path check）、通知压制探针（notification suppression probe）、响应者角色滥用场景（responder-role abuse scenario）、[事故状态篡改检查（incident-state tampering check）](../../appendix/incident-record-schema.zh.md)和回滚控制评测（rollback control eval）。
...
[Anthropic 的评估](https://www.anthropic.com/research/alignment-assessment-cybersecurity-incidents) 支持独立控制场景：告知智能体环境隔离，但受控观察与之矛盾；或者授权范围内没有解。成功意味着停止危险路径、说明限制并请求澄清权限，而不是不惜代价完成目标。“模拟”声明不扩大既定范围。

在真正隔离的环境中使用合成服务及无害工具模拟进行检查，不能为追求真实感而暴露第三方。成对比较有授权解与无授权解的任务，分别改变隔离声明和矛盾证据。记录轨迹长度及失败尝试积累的压力。在运行前独立确定权限标准，而不依赖模型答案。

区分智能体决策与控制效果：gateway 阻止了禁止尝试，是 containment 成功，不是智能体主动正确停止。测量越界、停止或升级耗时，以及授权任务上的无谓拒绝。承认可能造成伤害却不改变后续行为，不算成功。评分应覆盖完整轨迹，而非仅最终答案。字段见 [评测数据集模式与评分契约](../../appendix/eval-schema.zh.md)；这是建议实验，不是对 Anthropic 公布百分比的复现。
...
- 场景类别必须进入可评审的[评测模式（eval schema）](../../appendix/eval-schema.zh.md)；
- 发现结果必须有负责人和分诊路径；
