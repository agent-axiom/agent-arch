...
**Control-eval case-spine note：**[评测门禁与验证器契约（eval gate and verifier contract）](../../appendix/eval-schema.zh.md) 应该把三个 canonical cases 作为不同的行为与控制评测表面（behavioral and control eval surfaces）来测试。Support triage 需要重复预防（duplicate prevention）、载荷变更检查（payload-mutation check）、[审批路径误用检查（approval-path misuse check）](../../appendix/approval-schema.zh.md)、旧网关路由探针（old-gateway-route probe）和应急禁用断言（emergency-disable assertion）。Internal knowledge assistant 需要来源扎根评测（source-grounding eval）、租户过滤绕过探针（tenant-filter bypass probe）、不当记忆写入检查（improper-memory-write check）、[检索投毒场景（retrieval-poisoning scenario）](../../appendix/memory-retrieval-schema.zh.md)和新鲜度回归门禁（freshness regression gate）。Incident coordination 需要升级路径检查（escalation-path check）、通知压制探针（notification suppression probe）、响应者角色滥用场景（responder-role abuse scenario）、[事故状态篡改检查（incident-state tampering check）](../../appendix/incident-record-schema.zh.md)和回滚控制评测（rollback control eval）。
