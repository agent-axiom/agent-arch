...
!!! example "贯穿案例：替换后的旧工单写入器"
    如果 support-triage v2 替换了曾经制造重复工单的旧路径，退役必须证明旧的 `create_support_ticket` 路径已经不能再行动。仅仅移除 prompt route 不够：团队还必须[关闭工具主体](../../appendix/lifecycle-artifact-schema.zh.md)、[撤销 gateway exposure](../../appendix/registry-operations-handbook.zh.md)、[让 paused approvals 过期](../../appendix/approval-schema.zh.md)、[停止后台重试](../../appendix/lifecycle-artifact-schema.zh.md)，并[保留审计轨迹](../../appendix/trace-schema.zh.md)，这样未来的重复工单才不会被归因成“来源不明”的旧智能体。
