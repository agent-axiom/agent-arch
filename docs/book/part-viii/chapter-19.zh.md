...
**ADLC case-spine note：**lifecycle state model 应该把三个 canonical cases 作为不同的 release-bearing surfaces 来跟踪。Support triage 连接 code change、[策略包（policy bundle）](../../appendix/policy-bundle-schema.zh.md)、write-capability contract、duplicate-ticket evals 和 [发布门禁（rollout gate）](../../appendix/change-rollout-schema.zh.md)。Internal knowledge assistant 连接 [检索语料（retrieval corpus）](../../appendix/memory-retrieval-schema.zh.md)、source-grounding evals、memory-write rules、tenant filters 和 freshness review。Incident coordination 把 escalation policy、responder-role map、notification contract、[事故状态模式（incident-state schema）](../../appendix/incident-record-schema.zh.md) 和 post-incident lessons 作为一个 governed change set 连接起来。
