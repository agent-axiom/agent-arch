...
也就是说，新的生命周期不是因为“智能体很神秘”，而是因为现在有更多可变部件需要被发布、评估和治理。本章的主要工件是 智能体开发生命周期状态模型（ADLC state model）：一张状态与转换地图，而不是又一个泛泛的治理清单。
...
**ADLC case-spine note：**生命周期状态模型（lifecycle state model）应该把三个 canonical cases 作为不同的发布承载表面（release-bearing surfaces）来跟踪。Support triage 连接代码变更（code change）、[策略包（policy bundle）](../../appendix/policy-bundle-schema.zh.md)、写能力契约（write-capability contract）、重复工单评测（duplicate-ticket evals）和[发布门禁（rollout gate）](../../appendix/change-rollout-schema.zh.md)。Internal knowledge assistant 连接[检索语料（retrieval corpus）](../../appendix/memory-retrieval-schema.zh.md)、来源扎根评测（source-grounding evals）、记忆写入规则（memory-write rules）、租户过滤器（tenant filters）和新鲜度评审（freshness review）。Incident coordination 把升级策略（escalation policy）、响应者角色映射（responder-role map）、通知契约（notification contract）、[事故状态模式（incident-state schema）](../../appendix/incident-record-schema.zh.md)和事故后经验（post-incident lessons）作为一个受治理变更集（governed change set）连接起来。
