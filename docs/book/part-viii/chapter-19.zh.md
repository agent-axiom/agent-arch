...
!!! note "第 VIII 部分中的章节角色"
    **核心问题：** 如何把智能体系统从普通软件生命周期推进到 ADLC。

    **独特工件：** ADLC 状态模型。

    **相邻边界：** 生命周期定义状态；变更管理决定哪些状态转换需要审查。

    **本章不覆盖：** 发布规则、发现响应或所有权注册表。

    **案例延续：** 重复支持工单变成生命周期变更，而不是局部补丁。
...
!!! example "贯穿案例：把重复工单当作 ADLC 变更"
    在支持分诊系统里，修复重复工单事故并不会随着重试代码补丁结束。现在的发布承载工件还包括新的[评测数据集（eval dataset）](../../appendix/eval-schema.zh.md)、面向 `side_effect_unknown` 的策略包、`create_support_ticket` 的能力契约、金丝雀发布门，以及后续调查结果所需的[追踪模式（trace schema）](../../appendix/trace-schema.zh.md)。在 ADLC 里，这些变化应该作为一个可审阅的变更集一起移动；否则团队只是发布了代码，却把生命周期继续留在坏掉的状态。

**ADLC 案例主线说明（ADLC case-spine note）：**生命周期状态模型（lifecycle state model）应该把三个规范案例（canonical cases）作为不同的发布承载表面（release-bearing surfaces）来跟踪。支持分诊（Support triage）连接代码变更（code change）、[策略包（policy bundle）](../../appendix/policy-bundle-schema.zh.md)、写能力契约（write-capability contract）、重复工单评测（duplicate-ticket evals）和[发布门禁（rollout gate）](../../appendix/change-rollout-schema.zh.md)。内部知识助手（Internal knowledge assistant）连接[检索语料（retrieval corpus）](../../appendix/memory-retrieval-schema.zh.md)、来源扎根评测（source-grounding evals）、记忆写入规则（memory-write rules）、租户过滤器（tenant filters）和新鲜度评审（freshness review）。事故协调（Incident coordination）把升级策略（escalation policy）、响应者角色映射（responder-role map）、通知契约（notification contract）、[事故状态模式（incident-state schema）](../../appendix/incident-record-schema.zh.md)和事故后经验（post-incident lessons）作为一个受治理变更集（governed change set）连接起来。
