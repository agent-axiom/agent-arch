...
### Совместимость tool schema и пользовательского интерфейса

[Кейс Wood Mackenzie](https://aws.amazon.com/blogs/machine-learning/a-shared-agentic-platform-for-wood-mackenzie-on-amazon-bedrock-agentcore/) добавляет к контракту инструмента клиентскую привязку к компоненту. Следующая дисциплина — рекомендация книги, не заявленная гарантия AWS, AG-UI или A2UI. Версионируйте и проверяйте совместимость всей связки: схема инструмента → привязка отображения → каталог компонентов → формат подтверждения. Успешного backend-теста недостаточно.

Релиз должен задавать поддерживаемые сочетания старого/нового клиента и backend, обработку уже открытых форм и восстановленных сессий. Проверьте не только синтаксис, но и смысл: валюты, единицы, значения по умолчанию, скрытые поля и целевые ресурсы. Даже добавление необязательного поля может стать несовместимым изменением, если оно меняет последствия действия, но не показывается старым UI. Неизвестная версия блокирует зависимое чувствительное действие до обновления интерфейса; откат backend не восстанавливает автоматически валидность старой формы.

Предлагаемые приёмочные сценарии:

1. Неизвестный компонент: явная ошибка либо безопасное представление для чтения, без выполнения переданного кода и без доступной опасной кнопки.
2. Новая схема со старой привязкой: несовпадение обнаружено до подтверждения; значимые поля не теряются молча.
3. После показа формы изменены сумма, ресурс или ревизия состояния: сервер отклоняет старое подтверждение и требует актуальный просмотр.
4. Повторное или запоздалое UI-событие: оно не подтверждает другую операцию и не повторяет побочный эффект; действуют привязка к операции и идемпотентность.
5. Откат или переподключение с открытой формой: версии и серверное состояние сверяются заново; отображённый успех не подменяет подтверждённый результат выполнения.

Это сценарии проверки, не отчёт об исполненных тестах платформы Wood Mackenzie. Архитектурная граница — в [главе 8](../part-iv/chapter-8.md).
