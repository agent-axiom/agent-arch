...
!!! example "Case thread: duplicates as an ADLC change"
    In the support-triage system, fixing the duplicate-ticket incident does not end with a retry-code patch. The release-bearing artifacts now include a new [eval dataset](../../appendix/eval-schema.en.md), the policy bundle for `side_effect_unknown`, the capability contract for `create_support_ticket`, the canary rollout gate, and the [trace schema](../../appendix/trace-schema.en.md) used later to investigate the outcome. In ADLC, those changes should move as one reviewable change set; otherwise the team ships code while leaving the lifecycle broken.

**ADLC case-spine note:** the lifecycle state model should track all three canonical cases as different release-bearing surfaces. Support triage links code change, [policy bundle](../../appendix/policy-bundle-schema.en.md), write-capability contract, duplicate-ticket evals, and [rollout gate](../../appendix/change-rollout-schema.en.md). Internal knowledge assistant links [retrieval corpus](../../appendix/memory-retrieval-schema.en.md), source-grounding evals, memory-write rules, tenant filters, and freshness review. Incident coordination links escalation policy, responder-role map, notification contract, [incident-state schema](../../appendix/incident-record-schema.en.md), and post-incident lessons as one governed change set.
