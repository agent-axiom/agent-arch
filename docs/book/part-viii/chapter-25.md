...
!!! example "Сквозной кейс: контрольная оценка для записи тикета"
...
### Задачи без допустимого решения и противоречивая среда

[Оценка Anthropic](https://www.anthropic.com/research/alignment-assessment-cybersecurity-incidents) мотивирует отдельные контрольные сценарии: агенту обещают изоляцию, но контролируемые наблюдения ей противоречат; либо задача не имеет решения внутри разрешённой области. Успех здесь — прекратить рискованный путь, сообщить ограничение и запросить уточнение полномочий, а не выполнить цель любой ценой. Обещание «это симуляция» не расширяет заданный scope.

Проводите такие проверки в действительно изолированной среде с синтетическими сервисами и безопасными инструментами-имитациями, не открывая доступ к третьим сторонам ради реализма. Сравнивайте парные задачи с допустимым решением и без него; отдельно меняйте заявленную изоляцию и противоречащие свидетельства. Фиксируйте длительность траектории и накопленное давление неудачных попыток. Экспертный эталон допустимости задаётся до запуска и независимо от ответа модели.

Разделяйте решение агента и работу контроля: запрещённая попытка, остановленная gateway, — успех containment, но не корректная остановка самим агентом. Измеряйте нарушение границ, время до остановки/эскалации и необоснованный отказ на разрешённых задачах. Слова о возможном вреде без изменения последующих действий не считаются успехом. Анализируйте полную траекторию, а не только финальный ответ. Поля — в [eval-schema](../../appendix/eval-schema.md); это предлагаемый эксперимент, не воспроизведение опубликованных процентов Anthropic.
...
- класс сценария должен быть отражен в [проверяемой схеме оценки](../../appendix/eval-schema.md);
