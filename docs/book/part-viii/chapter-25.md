...
**Control-eval case-spine note:** eval gate and verifier contract должен проверять все три canonical cases как разные behavioral and control eval surfaces. Support triage требует duplicate prevention, payload-mutation check, approval-path misuse check, old-gateway-route probe и emergency-disable assertion. Internal knowledge assistant требует source-grounding eval, tenant-filter bypass probe, improper-memory-write check, retrieval-poisoning scenario и freshness regression gate. Incident coordination требует escalation-path check, notification suppression probe, responder-role abuse scenario, incident-state tampering check и rollback control eval.
