...
**Control-eval case-spine note:** [eval gate and verifier contract](../../appendix/eval-schema.md) должен проверять все три canonical cases как разные behavioral and control eval surfaces. Support triage требует duplicate prevention, payload-mutation check, [approval-path misuse check](../../appendix/approval-schema.md), old-gateway-route probe и emergency-disable assertion. Internal knowledge assistant требует source-grounding eval, tenant-filter bypass probe, improper-memory-write check, [retrieval-poisoning scenario](../../appendix/memory-retrieval-schema.md) и freshness regression gate. Incident coordination требует escalation-path check, notification suppression probe, responder-role abuse scenario, [incident-state tampering check](../../appendix/incident-record-schema.md) и rollback control eval.
