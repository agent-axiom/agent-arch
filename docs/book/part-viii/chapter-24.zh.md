...
**Misalignment case-spine note：**风险场景与控制计划（risk scenario and control plan）应该把三个 canonical cases 作为不同内部人风险表面（insider-risk surfaces）覆盖。Support triage 需要[审批收紧替换窗口（approval-tight replacement window）](../../appendix/change-rollout-schema.zh.md)、独立工具主体（separate tool principal）、[不可变追踪链接（immutable trace linkage）](../../appendix/trace-schema.zh.md)，以及面向写入能力族（write capability family）的应急禁用（emergency disable）。Internal knowledge assistant 需要针对[检索投毒（retrieval poisoning）](../../appendix/memory-retrieval-schema.zh.md)、租户过滤绕过（tenant-filter bypass）、隐藏记忆写入（hidden memory write）和来源扎根规避（source-grounding evasion）的遏制（containment）。Incident coordination 需要针对升级操纵（escalation manipulation）、通知压制（notification suppression）、响应者角色滥用（responder-role abuse）、[事故状态篡改（incident-state tampering）](../../appendix/incident-record-schema.zh.md)，以及过渡期（transition periods）中的回滚（rollback）控制（controls）。
