...
В этом и состоит главный смысл этой главы. Она должна показать, где доказательная база перестает быть просто общей телеметрией и превращается в управляемый опорный слой: место, которое сохраняет, на какой именно проверенный набор артефактов, доверенную версию контракта и утвержденную идентичность выпуска потом опирается разбор инцидента или управленческое решение.

Если тебе нужна связующая страница, которая показывает, как этот управляемый опорный слой остается связанным с request, policy, approvals, traces, evals, incidents и rollout judgment, открой [Сквозную цепочку доказательств](../part-v/evidence-spine.md).
