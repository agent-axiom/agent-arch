...
!!! info "时效说明"
    ...
    - 智能体系统的托管发布控制、审批流与分阶段发布（rollout）能力；
    - 不同平台如何界定承载发布意义的表面；
...
!!! note "第 VIII 部分中的章节角色"
    **核心问题：** 哪些智能体系统变更需要正式发布控制、评测和回滚。

    **独特工件：** 变更包。

    **相邻边界：** 变更管理决定什么需要发布控制；保障闭环从风险信号之后开始。

    **本章不覆盖：** 工件来源追踪、事故响应或整个智能体资产的所有权。

    **案例延续：** 重复工单防护通过受治理的变更包推进。
...
[Wood Mackenzie 案例](https://aws.amazon.com/blogs/machine-learning/a-shared-agentic-platform-for-wood-mackenzie-on-amazon-bedrock-agentcore/)为工具契约增加了客户端组件绑定。以下规则是本书建议，不是 AWS、AG-UI 或 A2UI 已声明的保证。应对整条链进行版本管理与兼容性验证：工具模式 → 渲染绑定 → 组件目录 → 审批格式。仅通过后端测试并不足够。

发布应声明新旧客户端与后端的支持组合，并处理已打开表单和恢复会话。除语法外，还应验证币种、单位、默认值、隐藏字段与目标资源等语义。即使新增的是可选字段，如果它改变操作后果却不在旧 UI 显示，也可能是不兼容变更。未知版本应阻止依赖该界面的敏感操作，直到界面更新；后端回滚不会自动恢复旧表单的有效性。

建议的验收场景：

1. 未知组件：明确报错或提供安全的只读表示，不执行传入代码，也不启用危险操作按钮。
2. 新模式配旧绑定：审批前发现不匹配，不得悄悄丢弃关键字段。
3. 预览后金额、资源或状态修订号变化：服务端拒绝旧审批，要求新的当前预览。
4. 重复或迟到的 UI 事件：不能批准其他操作，也不能重复产生副作用；仍需操作绑定与幂等性。
5. 表单打开期间回滚或重连：重新核对版本与服务端状态；界面显示成功不能替代已确认的执行结果。

这些是建议检查，不是对 Wood Mackenzie 平台已执行测试的报告。架构边界见[第 8 章](../part-iv/chapter-8.zh.md)。
...
- 离线评测通过；
- 受影响路径的失败运行演练覆盖；
- 明确确认失败运行仍然能通过发布身份、追踪链接、会话级证据、导出字段（例如 `failure_reason`）、像 `latest_failure_reason` 这样的面向操作员的摘要，以及会话评审中的 `traceable_failed_runs` 保持可追踪；
- 受限 rollout；
- 第一波监控；
- 清晰回滚路径。
...
- 审批阈值；
- 受审批约束的能力会话的中断与过期语义；
- 编排模式选择、工作者安全目录（worker-safe catalog）暴露与委派工作者（delegated worker）评审边界。
...
- 变更风险是被显式分类的，而不是靠感觉猜；
- 评测和门禁是按变更类型匹配的；
- 影响半径在发布（rollout）前就被限制，而不是事后再解释；
- 回滚能在真正承载风险的层面上工作。
