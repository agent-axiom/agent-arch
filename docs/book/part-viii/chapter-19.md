...
Классический software development lifecycle по-прежнему нужен, потому что именно он дает базовую инженерную дисциплину:
...
Именно поэтому NIST AI RMF GenAI Profile подчеркивает, что trustworthiness considerations должны жить не только в момент релиза, а на всем lifecycle: design, development, use и evaluation.[^nist-genai]
...
То есть новый lifecycle появляется не потому, что “агенты особенные”, а потому, что у них больше подвижных поверхностей, которые нужно выпускать и контролировать.
...
- approval and rollout gates для ограничения blast radius.
...
## 7. Отдельный lifecycle нужен и для security assurance
...
- какой prompt/routine изменился между двумя rollout waves.
...
Полезно иметь ранние lifecycle-stage вопросы:
...
Если назвать это lifecycle model, появляются более взрослые вопросы:
...
- думать, что rollout checklist автоматически покрывает весь lifecycle;
...
После этой переходной главы логично идти в change management, assurance loop и supply chain discipline. Но уже сейчас полезно связать ее с тем, что в книге написано раньше:
