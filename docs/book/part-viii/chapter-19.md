...
**ADLC case-spine note:** lifecycle state model должен отслеживать все три canonical cases как разные release-bearing surfaces. Support triage связывает code change, [policy bundle](../../appendix/policy-bundle-schema.md), write-capability contract, duplicate-ticket evals и [rollout gate](../../appendix/change-rollout-schema.md). Internal knowledge assistant связывает [retrieval corpus](../../appendix/memory-retrieval-schema.md), source-grounding evals, memory-write rules, tenant filters и freshness review. Incident coordination связывает escalation policy, responder-role map, notification contract, [incident-state schema](../../appendix/incident-record-schema.md) и post-incident lessons как один governed change set.
