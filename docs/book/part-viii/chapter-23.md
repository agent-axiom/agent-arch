...
### Удаление сессии не освобождает compute

В [OpenAI Agents API](https://developers.openai.com/api/docs/guides/agents-api/environments/lifecycle) удаление сессии не останавливает self-hosted compute и не порождает webhook удаления. Поэтому retirement нельзя завершать по одному успешному ответу delete-session.

Предлагаемый порядок для адаптера: прекратить новый ввод; под общей координацией запретить новые provisioning и согласовать уже начатые запуски; сохранить несекретные ссылки и нужные аудиторские данные; отдельно удалить сессию и остановить compute через провайдера; подтвердить оба результата. Сохраняйте запись незавершённой очистки для повторов и сверки с провайдером. Если compute появился после запроса удаления, контроллер должен обнаружить и освободить его, а не потерять вместе с mapping. Ошибка остановки означает незавершённый retirement, даже если сессия уже удалена. Политика хранения файлов и snapshots остаётся отдельным решением, а не побочным эффектом удаления сессии.
