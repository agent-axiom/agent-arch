...
The same rule matters for failed runs. If a capability timed out, an approval path failed validation, or an upstream dependency collapsed, later reviewers still need to know which approved artifact set and release identity governed that failure, and which exported failure field such as `failure_reason` preserved the concrete condition. Otherwise the organization preserves the happy path as provenance, but treats degraded behavior as unowned residue.
