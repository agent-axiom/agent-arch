...
### Кейс Google: быстрый triage и проверка взаимодействий изменений

В [кейсе Google Cloud](https://cloud.google.com/blog/topics/systems/using-ai-agents-to-secure-google-infrastructure) поток выглядит так: находка → структурное подтверждение достижимости → предложение исправления → человеческое ревью в исходном запросе на изменение. Быстрый presubmit-контур проверяет отдельное изменение; ночное интеграционное сканирование после внесения изменений ищет уязвимости, возникающие при их взаимодействии. Второй контур не делает первый полным и не предотвращает автоматически попадание дефекта в production до ночной проверки.

Как рекомендацию книги, сохраняй для каждого контура проверенную ревизию и конфигурацию, владельца находки и ссылки на доказательства. Для исправления требуй целевой тест, который выявляет дефект до правки и проходит после, плюс повторную проверку достижимости на новой ревизии. Результат ночного сканирования должен возвращаться в тот же процесс triage, исправления и регрессий, с отдельным решением о сдерживании и выпуске. Не разрешай автоматическое применение патча только потому, что агент сгенерировал убедительное объяснение. Это предлагаемые условия приёмки, а не заявленные гарантии Mantis или реализованная интеграция эталонной среды исполнения.
