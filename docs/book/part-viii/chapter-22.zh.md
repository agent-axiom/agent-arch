...
!!! example "贯穿案例：重复工单修复的 provenance"
    在重复工单事故之后，后续复盘需要能重建的不只是重试补丁对应的 commit。它还需要知道金丝雀期间生效的 [eval dataset](../../appendix/eval-schema.zh.md) 版本、`side_effect_unknown` 策略包、`create_support_ticket` 能力契约、[rollout gate](../../appendix/change-rollout-schema.zh.md)、审批模式和 [trace schema](../../appendix/trace-schema.zh.md)。如果其中任何一个工件只是“在聊天里某处”，而不是已批准发布包的一部分，团队就无法证明再次出现的重复工单到底发生在修复后的控制之下，还是旧规则集之下。
