...
!!! info "这一部分的快速路线"
    ...
    - [第 21 章](chapter-21.zh.md)：把 assurance 理解成围绕 drift、findings 与 response 的 operational loop；
    - [第 22 章](chapter-22.zh.md)：锁定 provenance、approved artifacts 与 contract lineage；
    - [第 23 章](chapter-23.zh.md)：再用 replacement、retirement 与 runtime-control shutdown 把生命周期收束起来；
    - [第 24 到 27 章](chapter-24.zh.md)：沿着同一条线继续扩展到 misalignment、adversarial evaluation、observability 作为 evidence substrate，以及整个 agent estate 的治理。
...
- 最后把同一套纪律扩展到 misalignment、adversarial evaluation、observability 作为 evidence substrate，以及整个 agent estate 的治理。
...
- 一种更成熟的变更评审与发布门禁视角；
- 更清楚地区分 eval loop、assurance response、observability evidence 与 estate governance；
