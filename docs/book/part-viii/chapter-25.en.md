...
**Control-eval case-spine note:** the [eval gate and verifier contract](../../appendix/eval-schema.en.md) should test all three canonical cases as different behavioral and control eval surfaces. Support triage needs duplicate prevention, payload-mutation check, [approval-path misuse check](../../appendix/approval-schema.en.md), old-gateway-route probe, and emergency-disable assertion. Internal knowledge assistant needs source-grounding eval, tenant-filter bypass probe, improper-memory-write check, [retrieval-poisoning scenario](../../appendix/memory-retrieval-schema.en.md), and freshness regression gate. Incident coordination needs escalation-path check, notification suppression probe, responder-role abuse scenario, [incident-state tampering check](../../appendix/incident-record-schema.en.md), and rollback control eval.
