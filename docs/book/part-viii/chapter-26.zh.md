...
**Observability case-spine note：**[追踪与遥测覆盖记录（trace and telemetry coverage record）](../../appendix/trace-schema.zh.md) 应该展示三个 canonical cases 的 observability coverage。Support triage 需要覆盖 ticket-write paths、[审批链接（approval linkage）](../../appendix/approval-schema.zh.md)、`tool_principal`、[`policy_bundle`](../../appendix/policy-bundle-schema.zh.md)、`contract_version`、duplicate outcome 和 bypass blind spots。Internal knowledge assistant 需要覆盖 [检索来源追踪（retrieval provenance）](../../appendix/memory-retrieval-schema.zh.md)、source-grounding verdicts、tenant-filter decisions、[记忆写入事件（memory-write events）](../../appendix/memory-retrieval-schema.zh.md) 和 freshness drift。Incident coordination 需要覆盖 escalation path、notification delivery、responder-role identity、[事故状态转换（incident-state transitions）](../../appendix/incident-record-schema.zh.md)、rollback events 和 [事故后控制变更（post-incident control changes）](../../appendix/lifecycle-artifact-schema.zh.md)。
