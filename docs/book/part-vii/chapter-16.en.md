...
At the narrow end of this architectural spectrum sits Laconian, an open-source project by the author of this book: its `if` is a Markdown-only behavioral skill whose installable artifact contains no executable or tool-specific logic and performs no external effects itself; any tool use or side effects belong to the host task and runtime, while the benchmark machinery stays outside the artifact loaded into that runtime.[^laconian-runtime-boundary] The Cloudflare `security-audit` example below occupies the opposite end: a skill-shaped entry point backed by a durable multi-stage workflow.
...
GitHub Copilot code review adds a separate lesson: "better tools" without task shape can make a reviewer worse. GitHub's case study says broad Unix-style tool access first gave the review agent too many ways to read the repository and spend budget, but not enough structure for a useful finding.[^github-copilot-review-tools] The portable contract is **workflow-constrained review**: first narrow the review through pull request evidence, diff-anchored review questions, and narrow-before-read; then allow targeted tool use and preserve the `tool_trace`, `review_cost`, evidence ref, and `quality_gate`. A review agent should prove which PR fact or diff hypothesis a tool checked, rather than presenting a long log as confidence.
