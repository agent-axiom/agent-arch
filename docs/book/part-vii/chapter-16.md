...
!!! info "Как читать эту главу"
    ...
    - где именно должен жить основной цикл того же support-агента;
...
В нашем сквозном support-кейсе это уже не теоретический вопрос. Агент умеет проверять статус пользователя, обращаться к памяти, открывать тикет через gateway и оставлять traces. Но без явной схемы рантайма все эти шаги очень быстро расползаются по локальным хендлерам, ad hoc retries и случайным интеграционным обходам.
...
Поэтому эту главу полезно читать не только как главу про границы модулей, но и как главу про работающую структуру под давлением изменений. Настоящий вопрос в том, появилась ли у рантайма такая форма, которая выдержит новые политики, новые инструменты, длинные запуски, interrupts и давление rollout, не распадаясь снова на хендлеры и исключения.
...
1. принять request и построить контекст run;
...
<p>У базового рантайма уже есть несколько обязательных контрольных точек</p>
...
!!! example "Сквозной кейс: где живет защита от дублей"
    В support-triage рантайме защита от duplicate ticket не должна быть спрятана в helpdesk adapter. `runtime.py` должен управлять run context и retry веткой, `execution.py` — выполнять write tool через идемпотентный контракт, `telemetry.py` — фиксировать `side_effect_unknown`, а `policy.py` и rollout gate — решать, можно ли продолжать. Тогда один и тот же инцидент не расползается по обработчикам.
