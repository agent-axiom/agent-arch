...
На узком конце этой архитектурной шкалы находится Laconian — открытый проект автора этой книги: его `if` — поведенческий навык в одном Markdown-файле. Сам устанавливаемый артефакт не содержит исполняемой или завязанной на инструменты логики и не производит внешних эффектов; любое использование инструментов и любые побочные эффекты принадлежат задаче и среде исполнения, а механика бенчмарка остается за пределами загружаемого в неё артефакта.[^laconian-runtime-boundary] Рассмотренный ниже Cloudflare `security-audit` показывает противоположный край: вход через навык, за которым стоит долговечный многостадийный рабочий процесс.
...
Материал OpenAI Responses API computer-environment описывает тот же слой как агентный компьютер: модель предлагает действие, платформа исполняет shell command в изолированном контейнере, возвращает streamed observation, а следующий model turn решает, продолжать ли работу.[^openai-computer-environment] Важная архитектурная граница здесь в том, что model decision и command execution не одно и то же. Модель предлагает, но runtime отвечает за isolation, filesystem/artifact persistence, optional structured storage, restricted network access, timeout/cancellation и наблюдаемый вывод инструмента.

Более сильная версия такого контракта также фиксирует каждый **bounded tool-output cap** и каждую **concurrent tool session**. Ограничение вывода — это не косметическое усечение, а управление budget контекста: сохранить полезные evidence, не позволяя сырым логам затопить следующий ход модели. Параллельные сессии — это не только ускорение: им нужны отдельные session ids, состояние timeout/cancellation, output envelopes и атрибуция ошибок, чтобы одна ветка shell, browser или data processing не перезаписала наблюдение другой ветки.
...
### Снимок инициализации не является checkpoint задачи

[AWS, The new AgentCore Runtime](https://aws.amazon.com/blogs/machine-learning/the-new-agentcore-runtime-elastic-optimized-and-consistently-fast-starts/) (18 сентября 2026) описывает запуск контейнера до успешной проверки здоровья, затем снимок уже инициализированного окружения. Новые экземпляры восстанавливаются из него вместо повторной загрузки и инициализации. Важно различать три назначения:

| Артефакт | Что сохраняет | Чего сам по себе не гарантирует |
| --- | --- | --- |
| Снимок инициализации | Подготовленное окружение, загруженные зависимости и стартовую конфигурацию | Продолжение конкретной пользовательской задачи |
| Checkpoint агента | Состояние конкретного запуска, курсор шага и ссылки на свидетельства | Восстановление памяти процесса, сокетов и файлов без отдельного контракта |
| Снимок активной сессии | Состояние исполнения в границах механизма провайдера | Актуальность полномочий, сохранность внешних соединений и однократность побочных эффектов |

В статье новый runtime включается через `platformVersion: V2`. Снимки инициализации и возврат неиспользуемой памяти описаны как доступные; suspend/resume активных сессий с memory snapshotting и hooks перед завершением находятся в разделе «coming soon». Там же перечислены baseline pricing, увеличение ресурсов, x86 и session context keys. Это статус на дату публикации, а не обещание доступности во всех будущих конфигурациях.

Предлагаемое правило книги: указывать назначение снимка, версию кода и конфигурации, владельца и границу данных. Общий стартовый снимок не должен переносить состояние одного пользователя другому; идентичность и чувствительные данные запуска привязываются отдельно. После восстановления проверяйте актуальность конфигурации и полномочий, необходимость обновить соединения и корректность уникальных идентификаторов. Быстрое восстановление окружения не заменяет журнал задачи, checkpoint и сверку исхода прерванных внешних операций. Это требования к адаптеру, не подтверждённые гарантии AWS или реализованная интеграция эталонного рантайма.
...
Отдельный урок GitHub Copilot code review состоит в том, что "более мощные tools" без формы задачи могут ухудшить результат: общий Unix-style tool access сначала дал review agent слишком много способов читать репозиторий и тратить бюджет, но недостаточно структуры для полезного замечания.[^github-copilot-review-tools] Переносимый контракт — **workflow-constrained review**: сначала сузить review через pull request evidence, diff-anchored review questions и narrow-before-read, затем разрешить targeted tool use, сохранить `tool_trace`, `review_cost`, найденный evidence ref и `quality_gate`. Review agent должен доказывать, какой PR-факт или diff-гипотезу проверял инструмент, а не просто предъявлять длинный лог как уверенность.
...
### 8.4. Восстанавливаемые внутренние задачи и легковесные потоки (fibers)
...
### 8.5. Оболочка агента и долговечный стержень рабочего процесса (Agent shell + durable workflow spine)
...
- secret boundary: основные ключи приложения и бизнес-полномочия остаются вне sandbox; доступ к бизнес-сервисам предоставляется через brokered capability. Если протокол требует ключ идентичности executor внутри окружения, его ограниченные полномочия и возможная доступность агентному коду описываются отдельно.
...
[OpenAI Agents API: self-hosted sandboxes](https://developers.openai.com/api/docs/guides/agents-api/environments/self-hosted) показывает такое исключение явно: облачный harness управляет собственным executor, а ограниченный ключ внутри окружения разрешает только подключение environments, не другие действия API. Документация допускает чтение этого ключа агентным кодом, но требует отдельный ключ с остальными разрешениями `None`, принадлежащий той же организации, проекту и пользователю либо service account, что и сессия. Основной ключ приложения остаётся снаружи. Ключ подключения — всё ещё секрет, не публичный идентификатор и не гарантия изоляции: его нельзя включать в исходники, образы или логи; нужны ротация и отзыв. Не следует приписывать ему недокументированную привязку только к одному environment.

Для промышленного адаптера фиксируйте классы credential, фактические разрешения, привязку владельца сессии, несекретную ссылку на ключ и процедуру отзыва. Это уточнение модели угроз, не ослабление границы бизнес-полномочий и не реализованная интеграция reference runtime. Сетевые и пользовательские границы разобраны в [главе 9](../part-iv/chapter-9.md).

### Жизненный цикл сессии и compute — разные контракты

[OpenAI Agents API: Sandbox lifecycle](https://developers.openai.com/api/docs/guides/agents-api/environments/lifecycle) уточняет: сессия может пережить своё self-hosted окружение, но приложение управляет compute и файлами. Один компонент должен владеть provisioning сессии и долговечной привязкой `session → environment → provider compute`; повторные и конкурентные запросы не должны создавать дубликаты. В промышленном адаптере это требует атомарной координации по сессии, идемпотентного запуска и сверки с провайдером после неопределённого исхода, а не только дедупликации одного webhook ID.

Webhook handler проверяет подпись, ставит запрос в очередь и отвечает успехом только после успешной постановки. Запуск инициирует `agent.session.action_required` с `required_action.type: environment_connection`; stream называет запрос `agent.session.requires_action`. `function_call` требует результата функции, не запуска среды. Worker заново читает сессию: удалённую сессию или уже разрешённое действие пропускает, актуальную потребность обслуживает запуском либо подключением существующего executor. Подтверждённый текущий `failed` требует освобождения compute. Signing secret и credential чтения сессии остаются отдельно от executor key.

Остановка использует ту же координацию с новым вводом и provisioning. Событие `idle` может прийти после снятия запроса подключения, но до начала ожидающего turn: оно не является разрешением остановить среду. Отложенную остановку отменяют при новом запросе подключения или начале исполнения; перед остановкой повторно проверяют состояние. Если приложение не умеет координировать этот переход, документация рекомендует сохранять среду работающей. Проверка состояния без защиты от конкурентного запуска оставляет гонку.

Connection events показывают соединение, но не запрашивают compute. Разрыв посреди turn не гарантирует webhook переподключения или перезапуск убитой команды. Позднее подключение не воспроизводит ввод, уже завершившийся тайм-аутом; перед повтором сверяют исход запроса. Тот же environment ID не восстанавливает файлы на новом compute: нужны storage/snapshots провайдера. Это предлагаемый контракт адаптера поверх документации API, не реализованный контроллер reference runtime. Завершение — в [главе 23](../part-viii/chapter-23.md).
...
### Выборочное сжатие вывода: меньше текста не всегда дешевле

В экспериментах GitHub ранние варианты сжатия заставляли агента перечитывать сохраненный вывод или повторять команды; локальная экономия токенов увеличивала стоимость и задержку всей задачи. В частности, фильтр `git diff` убрали после таких регрессий. Это результат конкретной интеграции и нагрузок, а не доказательство, что RTK или любое сжатие вредны. Источник: [GitHub Engineering, How we make AI coding more cost efficient without sacrificing task quality](https://github.blog/ai-and-ml/github-copilot/how-we-make-ai-coding-more-cost-efficient-without-sacrificing-task-quality/) (2 сентября 2026).

Предлагаемый контракт обработки вывода отделен от сжатия истории разговора и пока не является реализованной возможностью эталонного runtime:

- `preserve`: код, `cat`, `git diff`, `git show` и произвольный неизвестный вывод сохраняются без потери содержимого.
- `lossless_group`: результаты поиска можно перегруппировать, сохранив все совпадения, пути и связь с исходным результатом.
- `selective_compress`: повторяющиеся install/build/test/progress-логи сжимаются, только если экономия существенна. Диагностика, exit code и уникальные ошибки не исчезают вместе с шумом. Смешанный вывод классифицируется по содержимому; одно имя команды не доказывает, что весь вывод безопасно сокращать.

До преобразования сохрани разрешенный политикой оригинал и свяжи его с `tool_call_id`, `original_output_ref`, версией преобразования и признаком полноты. Сжатое представление явно сообщает, что сокращено, и дает прямой путь чтения оригинала без повторного выполнения команды. Если оригинал был обрезан upstream или недоступен, не называй восстановление полным. Ограничения секретов, доступа и хранения действуют и для оригинала; его чтение не расширяет права и не повышает доверие к внешнему тексту.

Восстановление — не только запасной путь, но и сигнал оценки: частые обращения к оригиналу, повторные поиски и команды могут означать, что удалено полезное. Не повторяй действие с побочным эффектом ради восстановления текста. Метрики и парное сравнение описаны в [схеме оценок](../../appendix/eval-schema.md).

### Регенерация из документов как отдельный режим сборки

Кейс SMART из [Design Docs Are All You Need: An AI-native Machine-Learning Performance Tool](https://arxiv.org/html/2609.05364v1) предлагает хранить устойчивую спецификацию, а реализацию считать пересобираемым артефактом. Здесь важен не только DAG оркестрации: исправление должно возвращаться в документ, иначе следующая чистая генерация может его потерять. Практический тест — восстановить систему из документов и разрешенных зависимостей без доступа к прежнему коду.

Для такого режима мы рекомендуем отдельный манифест сборки: версии документов и графа, модель и конфигурация генератора, зависимости, журнал неоднозначностей, полученный артефакт и независимые результаты проверки. Обнаруженный агентами граф проверяется на циклы, пропущенные зависимости и совместимость интерфейсов. Повторяемая семантика не означает побайтово одинаковый код. Новая генерация не должна автоматически заменять рабочую сборку: нужны проверка, управляемый выпуск и откат. Это предлагаемый контракт, не реализованная возможность эталонного runtime. Подробности и ограничения — в [кейсе SMART](../../appendix/case-studies.md).

### Наследование контекста не является делегированием прав

[LangChain, Organizing Context in a Multi-Agent Harness](https://www.langchain.com/blog/organizing-context-in-a-multi-agent-harness) показывает fork как передачу состояния координатора и истории с новым поручением. Экономия возможна за счет меньшего числа повторных чтений и prompt caching, но зависит от задачи, модели и сохранения подходящего кэширующего префикса. Это не обещание бесплатной передачи большой истории.

Предлагаемый внутренний контракт запуска: `subagent_role`, `context_mode`, `context_source_ref`, `context_snapshot_ref`, `task_constraints_ref`, `delegated_capabilities_ref`, `result_contract_ref`. Это не утверждение, что такие поля уже реализованы в эталонном runtime. Снимок делает границу наследования явной: последующие изменения родителя не следует считать автоматически доставленными ребенку.

Полномочия определяются отдельно политикой задачи: fork не копирует доступ к инструментам, действительность approvals или право раскрывать историю другому субъекту. `isolated` означает свежий контекст модели, а не отдельную файловую систему, credential или sandbox. Проверяющему нужны исходные требования и свидетельства, но не заключение автора как готовый ответ. Какой режим использовать по роли — см. в [практике координатора](../part-i/practical-manager-handoffs.md).

### Выбирать схему выполнения, а не только модель

[GitHub Project HydraFusion](https://github.blog/ai-and-ml/github-copilot/project-hydrafusion-frontier-quality-via-multi-model-orchestration/) выбирает между `single`, `cascade` и `critique`. Это отдельный контракт runtime: решение определяет не только исполнителя, но и число этапов, условия эскалации и способ проверки результата.

| Схема | Когда рассматривать | Граница выполнения |
| --- | --- | --- |
| `single` | Один исполнитель ожидаемо достигает требуемого качества | Один solver, затем проверка результата |
| `cascade` | Недорогая первая попытка полезна, а gate умеет обнаруживать недостаточный результат | Черновик → gate → принятие либо ограниченная эскалация |
| `critique` | Независимое ревью ожидаемо полезнее ещё одной самостоятельной попытки | Черновик → изолированный критик → одна доработка автором → проверка |

В HydraFusion критик — из другой семьи моделей, без инструментов и с read-only ролью; это снижает связанность, но не доказывает независимость ошибок. Приведённые критерии выбора следует калибровать на задачах, а не превращать в универсальное правило «сложное всегда отправлять нескольким моделям».

Предлагаемый контракт для промышленного адаптера: до запуска проверить `routing_policy_version`, `workflow_ref`, привязки моделей и доступность, `quality_gate_ref`, общий бюджет/дедлайн, предел числа этапов и разрешённый fallback. Каждый этап имеет роль, входной snapshot, timeout, отмену и выходной контракт. Эскалация или fallback не обнуляют бюджет и не расширяют права; недоступный критик не означает успешное ревью. Если безопасный маршрут не укладывается в лимиты, остановить выполнение с явным статусом, а не продолжать бесконечную цепочку моделей.

Рабочие изменения solver хранит в промежуточной изолированной копии. Только валидированный кандидат допускается к применению через обычную границу approval; отменённый или невалидный workflow ничего не применяет к целевой копии. Это не откатывает уже совершённые внешние побочные эффекты: их по-прежнему контролируют gateway, подтверждения и идемпотентность. Учитывайте стоимость и задержку маршрутизатора, draft, gate, critique, revision, escalation, retry и fallback. Это предлагаемый контракт, не реализованный HydraFusion-router reference runtime; сравнение схем — в [главе 13](../part-v/chapter-13.md).

### Артефакты и завершение принадлежат конкретной ветке

[LangChain Paid Media Agent](https://www.langchain.com/blog/paid-media-agent) иллюстрирует ложное завершение соседнего подагента через общий путь отчёта и `done`. Следующий контракт — рекомендация книги, а не заявление о реализованной поддержке в эталонном рантайме или точной внутренней схеме LangChain:

- Координатор назначает `run_id`, `branch_id` и `attempt_id`; рабочий путь, например `runs/<run_id>/<branch_id>/<attempt_id>/report.pdf`, принадлежит этой попытке. Общие исходные данные могут быть доступны только для чтения. Уникальное имя само по себе не ограничивает доступ: границы записи проверяются инструментом или файловой политикой, включая разрешение путей и ссылок.
- Запись завершения содержит те же идентификаторы, статус, ссылку на артефакт и свидетельство проверки. Один общий `done` или само наличие файла не подтверждают работу всех веток. Успешный рендер подтверждает лишь согласованный критерий рендера, не достоверность анализа.
- Уполномоченный компонент публикует подтверждение успеха только после доступности и проверки артефакта. Координатор принимает его для текущей попытки; повторная доставка не увеличивает число завершённых веток, а запоздалый результат отменённой или заменённой попытки не заменяет текущий.
- Итог собирается по заранее определённому набору обязательных веток. Отсутствие, ошибка и отмена остаются отдельными состояниями; частичный итог допустим только с явным указанием пропусков, а не как полный успех.

Проверяйте порядок A → B и B → A, повторную доставку завершения и поздний результат старой попытки. Конкретный сценарий приведён в [практических кейсах](../../appendix/case-studies.md).
...
[^laconian-runtime-boundary]: Laconian, зафиксированная ревизия `669c45e849f75c99f81af19561d09cf24664e935`: [`skills/if/SKILL.md`](https://github.com/agent-axiom/laconian/blob/669c45e849f75c99f81af19561d09cf24664e935/skills/if/SKILL.md) и [`docs/design.md`](https://github.com/agent-axiom/laconian/blob/669c45e849f75c99f81af19561d09cf24664e935/docs/design.md).
