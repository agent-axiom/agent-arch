...
На узком конце этой архитектурной шкалы находится Laconian — открытый проект автора этой книги: его `if` — поведенческий навык в одном Markdown-файле, который не вызывает инструменты и не создает побочных эффектов, а механика бенчмарка остается за пределами артефакта, загружаемого в среду исполнения.[^laconian-runtime-boundary] Рассмотренный ниже Cloudflare `security-audit` показывает противоположный край: вход через навык, за которым стоит долговечный многостадийный рабочий процесс.
