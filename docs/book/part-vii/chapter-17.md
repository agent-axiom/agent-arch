...
### Частичный grant не должен превращаться в расширение полномочий

Успешный OAuth flow еще не означает, что все `requested_scopes` вошли в `granted_scopes`: пользователь может отклонить необязательную часть запроса. Это поведение описано в [Cloudflare, From all-or-nothing to task-based OAuth consent](https://blog.cloudflare.com/task-based-oauth-consent/). Для каждой операции каталог задает отдельные `required_scopes`; названия scopes в примерах книги условные, а не универсальный словарь OAuth.

Перед исполнением шлюз вычисляет `missing_scopes = required_scopes - granted_scopes` по проверенному контексту авторизации. При непустом наборе он запрещает именно эту операцию. Независимая разрешенная часть задачи может продолжиться, если это сохраняет смысл задачи и не нарушает ее атомарность; частичный результат нельзя выдавать за полный. Подтверждение действия не создает недостающий OAuth scope. Новое согласие на доступ — отдельное решение пользователя, после которого политика и границы задачи проверяются заново, без автоматического расширения активного run.

После обновления credential, отзыва доступа или возобновления задачи нельзя полагаться на старый снимок grant. Неизвестные или неподтвержденные права не восстанавливаются из исходного запроса. Формат evidence и сценарии отказа приведены в [контракте набора политик](../../appendix/policy-bundle-schema.md).

### Владелец доступа и тип credential — независимые оси

[LangChain Connections](https://www.langchain.com/blog/connections-managed-credentials-and-per-caller-identity-for-managed-deep-agents) в prerelease Managed Deep Agents разделяет две настройки: кому принадлежит credential и каким способом получен доступ. `agent-owned` означает credential развёртывания, общий для вызывающих; `user-owned` — выбор credential конкретного пользователя во время выполнения. Это не то же самое, что учётная запись, которую видит внешний сервис.

| Владелец | Статический секрет | OAuth grant |
| --- | --- | --- |
| Агент / развёртывание | Общий ключ поискового сервиса | Grant выделенной общей учётной записи |
| Пользователь | Личный секрет, выбираемый по проверенной идентичности | Grant конкретного вызывающего пользователя |

Следовательно, OAuth сам по себе не доказывает действие от имени пользователя, а статический секрет не обязательно общий. В Connections владелец фиксируется при создании connection; runtime lookup выбирает существующий credential, а не меняет владельца. Имя connection — ссылка на настройку, не доказательство прав.

Для промышленного контракта связывайте проверенного инициатора, tenant, развёртывание, connection и фактического субъекта внешнего сервиса. При отсутствии пользовательского grant остановите зависимую операцию и запросите доступ через защищённый поток; нельзя незаметно подставлять credential агента или другого пользователя. После возобновления заново проверьте идентичность, ресурс, права и отдельное подтверждение действия. Это расширяет уже описанный контракт частичного OAuth grant, а не заменяет его.

Проверяйте и чтение: одинаковый запрос к приватным репозиториям должен учитывать права каждого вызывающего. Для общей учётной записи аудит должен различать «кто попросил» и «от чьего имени исполнено», не выдавая её действие за личное действие пользователя. Поля и сценарии ниже — [предлагаемое расширение policy bundle](../../appendix/policy-bundle-schema.md), не реализованная интеграция Connections в reference runtime.

### Операция, ресурс и класс доступа — три разных измерения

[Гранулярная авторизация Cloudflare Workers](https://blog.cloudflare.com/workers-granular-authorization/) и [документация ролей](https://developers.cloudflare.com/workers/authorization/) дают конкретную матрицу вместо одного разрешения «доступ к Worker». Ниже — минимальные права для указанных операций; более широкие роли могут включать их, но не расширяют область ресурса автоматически.

| Операция | Ресурс | Требуемый класс доступа |
| --- | --- | --- |
| Читать настройки, метрики, логи и трассы | Конкретный Worker | `Metadata Read-Only` |
| Читать исходный код | Тот же Worker | `Content Read-Only` |
| Обновить код существующего Worker без изменения маршрутов | Тот же Worker | `Editor` |
| Удалить Worker | Удаляемый Worker | `Admin` в подходящей области |
| Изменить Route или Custom Domain | Worker и каждая затронутая зона | `Editor` для Worker **и** `Workers Routes Write` для каждой зоны |

Имя команды не определяет полномочия: `wrangler deploy` может обновлять существующий Worker, создавать новый или менять маршруты. Создание требует `Admin` на уровне продукта Workers; обычный деплой с неизменными маршрутами не требует права на зону. Шлюз должен получать перечень действий и целевых ресурсов из проверенного адаптера и фактической конфигурации, а не из заверения модели «это только деплой». Неопределённый набор целей не даёт разрешения.

Не превращайте это в правило «для деплоя нужны права на все связанные ресурсы». По документации Cloudflare, деплой Worker с bindings к KV/R2/D1 не требует отдельных прав на эти ресурсы; прямой доступ к ним проверяется отдельно. Но право менять код может позволить использовать уже существующие bindings через Worker. Поэтому запрет удаления не гарантирует безопасный деплой: нужны проверка кода, ограничение runtime-доступа и отдельное подтверждение высокорискового изменения. Аналогично, разрешение на телеметрию не делает логи нечувствительными: секреты и персональные данные требуют фильтрации и контроля доступа.

Для составного изменения проверяйте все необходимые пары «действие — ресурс», не выдавая Worker-role за право управлять зоной. Формат свидетельств и отрицательные проверки — в [предлагаемом расширении набора политик](../../appendix/policy-bundle-schema.md).

Ограничение реализации: [подробная документация Workers](https://developers.cloudflare.com/workers/authorization/workers/#limitations) отдельно указывает, что Custom Domains пока не поддерживают роли на уровне отдельного Worker. Строка матрицы описывает необходимые классы прав, но не гарантирует поддержку любой комбинации scope и операции. Проверяйте поддержку целевого API; отсутствие поддержки не является основанием автоматически расширять доступ до уровня продукта.
...
### Кэш каталога MCP: свежесть не является разрешением

LangChain описывает кэширование списка инструментов с серверным TTL; FastMCP уточняет `ttlMs`, `cacheScope` и разделение общего хранилища по вызывающему субъекту. Это поведение конкретного клиента: кэш включается явно и работает с поддерживающими подсказки modern-era серверами; legacy-соединение не получает эту возможность автоматически. Источники: [LangChain: MCP in LangChain](https://www.langchain.com/blog/mcp-in-langchain-stateless-protocol-elicitation-and-more) / [FastMCP: Response caching](https://gofastmcp.com/clients/client#response-caching).

Для изоляции нужен отдельный клиент на вызывающего пользователя либо partition общего кэша, выведенная из проверенной идентичности. Один tenant недостаточен, если у двух пользователей разные каталоги; переданный пользователем `user_id` и URL сервера сами по себе не являются доверенной partition. В FastMCP только ответы с серверной отметкой `public` могут пересекать partitions. Локальная политика может запретить и такое разделение; нельзя самостоятельно объявлять пользовательский каталог публичным.

**Попадание в кэш — это повторное использование описания, а не решения allow.** Даже до истечения TTL отзыв scope должен блокировать вызов при новой проверке полномочий. Каталог не является ни credential, ни списком гарантированно разрешенных действий. При смене субъекта создай новый клиент; при изменении grant или политики сделай старую видимость недоступной до повторной проверки. Не показывай другому пользователю даже названия и схемы скрытых инструментов.

Если определение инструмента изменилось, прежний план и подтверждение могут больше не соответствовать действию. При обнаружении новой версии или digest перечитай каталог и проверь аргументы, риск и применимость подтверждения заново; TTL сам по себе не обнаруживает изменение мгновенно. Для чувствительных операций используй принудительное обновление или проверяемую привязку к версии. Клиентский digest обнаруживает различия между снимками, но не заставляет сервер исполнить старую версию: такая гарантия требует серверного version binding. Поля и проверочные сценарии см. в [контракте набора политик](../../appendix/policy-bundle-schema.md).
