...
## 2.1. Semantic gap между пользователем и knowledge layer реален

Еще одна частая проблема здесь почти не видна на демо: пользователь формулирует запрос разговорно, а документы и knowledge records обычно написаны формально, технически или в терминах внутренних систем.

Из-за этого retrieval может дать слабый результат не потому, что данных нет, а потому, что между user query и corpus есть semantic gap.

Практически это означает, что retrieval query иногда полезно дополнительно обрабатывать:

- нормализовать названия сущностей и внутренних статусов;
- переписывать запрос в более "документный" стиль;
- добавлять controlled query expansion;
...
Но здесь важна одна дисциплина: такой промежуточный query aid не должен превращаться в новый "факт". HyDE или query rewriting полезны как инструмент retrieval, а не как замена grounded answer.

## 2.2. Обычно стоит начинать с RAG, а не с training

Если проблема в том, что агенту не хватает свежих знаний или доступа к внутренним документам, самый практичный первый шаг обычно не training, а нормальный retrieval layer.

Причина простая:

- RAG быстрее обновлять;
- retrieval проще аудировать и ограничивать;
- knowledge drift проще чинить обновлением corpus, чем переобучением модели;
- changing documents и mutable knowledge sources лучше ложатся в retrieval, чем в веса модели.

При этом полезно различать две разные задачи:

- continued pretraining в первую очередь помогает адаптировать knowledge distribution;
- SFT в первую очередь помогает адаптировать поведение, стиль и decision pattern.

Практическое правило обычно такое: сначала доведи retrieval до внятного уровня, а уже потом решай, уперлась ли система в потолок и нужен ли training.

Хороший operational сигнал тоже довольно простой: если support-агент долго работал нормально, а потом просел без заметных изменений prompt и model routing, сначала стоит подозревать stale retrieval corpus, indexing drift или data freshness problem, а не мистическую "деградацию модели".

!!! example "Сквозной кейс: что доставать обратно"
    В support-triage кейсе retrieval не должен просто поднимать все прошлые обращения клиента. Для текущего run полезны последние открытые тикеты, проверенный профильный факт вроде предпочитаемого языка и актуальная выдержка из support playbook. Старые черновики, непроверенные жалобы и устаревшие summary должны либо уйти в фоновой compaction, либо вообще не попадать в prompt.
...
Но именно здесь нельзя перепрыгивать через operational discipline.
...
На этом базовая часть про память уже складывается. Дальше можно либо углубиться в retention и deletion, либо перейти к части про инструменты и выполнение.
