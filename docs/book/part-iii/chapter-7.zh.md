...
压缩也会形成安全边界。摘要只是持久会话状态的派生、不受信任的视图，不能携带权限。身份、策略与能力版本、审批绑定、幂等键、外部副作用状态、检查点、预算以及用户约束的精确引用，都必须在摘要文本之外保存，并在继续执行前重新验证。[上下文连续性信封](../../appendix/continuity-envelope-schema.zh.md)给出了这一契约及其失败关闭的恢复协议。

在这条边界上，应明确区分两个数据平面：

- **可丢弃的上下文视图：** 对话历史、检索片段、工作笔记和压缩摘要。它可以重建或删除。
- **持久控制状态：** 租户与主体身份、委派范围、策略与能力版本、`approval_id` 及其 `action_digest` 和有效期、`idempotency_key`、`side_effect_status`、`checkpoint_ref`、剩余预算、未完成义务、约束的精确引用以及证据沿袭。它必须以结构化形式保存，不能依赖模型生成的摘要。

这个区分能避免一种常见的类别错误：高质量摘要可以帮助重新定位，但无论文字多流畅，它都不会变成策略数据库、审批注册表、副作用账本或事件日志。
