...
## 9. Практические правила для memory design

Если нужен короткий каркас для первых решений, он обычно выглядит так:

1. Сначала разделяй session context и persistent memory, а потом уже спорь о richer memory features.
2. Лучше писать меньше, но понятнее: validated facts почти всегда ценнее сырого текста.
3. Правила записи должны быть жестче правил чтения.
4. Каждая долговременная запись должна нести provenance, tenant metadata и writer identity.
5. Если запись может пережить run, по умолчанию безопаснее вынести ее в background path.
...
- смешивают profile memory, retrieval store и execution artifacts;
- позволяют summaries жить без provenance и validation state;
- используют непроверенную память в policy decisions;
- долго не проектируют удаление, пересмотр и устаревание записей.

## 11. Что production-команда должна уметь быстро ответить
...
Для нашего support-кейса это и есть следующий шаг: научиться различать рабочий контекст, профиль пользователя и долгоживущую память так, чтобы потом execution layer не действовал на грязном state.
