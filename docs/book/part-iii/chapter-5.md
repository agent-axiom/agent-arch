...
### 8.3. Memory poisoning — это сценарий безопасности, а не только качество данных

Самый неприятный отказ памяти часто выглядит не как взлом инструмента, а как тихая запись вредного или ошибочного факта в доверенный слой. Например, пользовательская фраза, результат непроверенного retrieval или summary из инцидента попадает в долговременную память, получает вид стабильного знания и через несколько запусков начинает влиять на решения.

Минимальный сценарий memory poisoning должен проверять те же memory poisoning review fields, которые потом попадают в [memory/retrieval schema](../../appendix/memory-retrieval-schema.md) и [trace schema](../../appendix/trace-schema.md):
