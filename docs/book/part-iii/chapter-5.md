...
- `untrusted write`: может ли непроверенный источник попасть в устойчивую память;
- `delayed activation`: влияет ли запись на поведение не сразу, а через следующий запуск или другую сессию;
- `cross-tenant contamination`: может ли запись пересечь границу арендатора или роль доступа;
- `policy influence`: используется ли непроверенная память в решении политики, подтверждении или выборе инструмента;
- `provenance check`: видит ли оператор источник, идентичность записавшего компонента, состояние проверки и время записи;
- `quarantine and rollback`: можно ли отключить запись, переиграть затронутые трассы и объяснить, какие ответы она изменила.
