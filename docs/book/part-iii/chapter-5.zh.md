...
最小 memory poisoning 场景应该检查同一组 memory poisoning review fields，之后它们会进入 [记忆/检索模式（memory/retrieval schema）](../../appendix/memory-retrieval-schema.zh.md) 和 [追踪模式（trace schema）](../../appendix/trace-schema.zh.md)：
