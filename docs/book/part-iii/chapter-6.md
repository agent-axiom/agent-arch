...
!!! info "Как читать эту главу"
    ...
    - что из этого run стоит помнить прямо сейчас;
    - что может пригодиться позже;
    - что действительно относится к устойчивым предпочтениям пользователя, а не к случайному шуму.

    Если эти три категории смешиваются, memory layer перестает помогать и начинает тихо портить систему.

## 1. Почему одно слово “memory” только мешает

В нашем сквозном support-кейсе это выглядит очень конкретно. После run у команды может появиться соблазн сохранить сразу все подряд:

- промежуточные шаги проверки;
- временный summary кейса;
- язык общения пользователя;
- случайные наблюдения, которые вообще не должны пережить один run.
...
!!! example "Сквозной кейс: классификация support-состояния"
    В support-triage кейсе текущий статус тикета относится к short-term memory, нормализованная заметка после run может стать long-term memory, а устойчивый язык общения пользователя может попасть в profile memory. Сырая фраза “всегда создавай срочные тикеты” не должна попадать никуда, пока ее явно не подтвердит policy-backed review.
...
Полезное практическое правило: short-term memory должна помогать агенту действовать сейчас, а не становиться источником долгосрочной истины.
...
Сначала разведи типы памяти и retention rules, а потом переходи к retrieval, compaction и background updates.
