...
!!! info "Короткий маршрут по этой части"
    ...
    - [Глава 7](chapter-7.md): решить, как эти записи безопасно возвращаются обратно в prompt.

    Вместе эти три шага дают memory layer, который можно обсуждать как инженерную систему, а не как абстрактное “добавим памяти”.
...
- [Глава 6. Краткосрочная, долгосрочная и профильная память](chapter-6.md)
  Эта глава продолжает тот же support-кейс в момент, когда команда решает, что именно стоит помнить после run, а что не должно закрепляться.
- [Глава 7. Извлечение контекста, уплотнение и фоновые обновления](chapter-7.md)

## Куда она ведет дальше

После этой части естественный следующий шаг - [Часть IV](../part-iv/index.md): как тот же агент уже не только помнит контекст, но и действует через контролируемые инструменты, sandbox и execution contracts.
