...
!!! example "Сквозной кейс: разбор обращений поддержки"
    Тот же support-triage кейс из [главы 1](../part-i/chapter-1.md) здесь превращается в карту границ: текст клиента — недоверенный input, профиль и история тикетов — scoped reads, `create_ticket` — управляемая write-операция, а эскалация — policy decision, которому может потребоваться approval.
...
## 9. Практические правила для perimeter

Если нужен короткий operational каркас, держись таких правил:

1. Все внешние документы, письма и tool outputs по умолчанию считаются данными, а не инструкциями.
2. Все решения, которые меняют внешний мир, должны быть отделены от самого исполнения и проходить через policy layer.
3. Все вызовы, которые затрагивают tenant scope, PII или side effects, должны иметь явный principal и понятный audit trail.
4. Если команда не может в одном абзаце объяснить, что агент видит, что решает и что исполняет, perimeter пока еще размыт.

## 10. Что команды чаще всего делают неправильно

У perimeter почти всегда одни и те же ранние ошибки:

- надеются на один guardrail вместо серии контрольных точек;
- дают агенту одну всемогущую учетку вместо разделения `user_principal`, `runtime_principal` и `tool_principal`;
- смешивают user scope, tenant scope и system scope;
- разрешают tool access раньше, чем появляется нормальная трассировка и расследуемость.

## 11. Что production-команда должна уметь доказать после инцидента
...
Теперь можно переходить к следующему логическому слою: что делать, когда тот же агент поддержки уже дошел до реальных действий и ему нужно не просто "иметь периметр", а безопасно пройти через tool gateway, approval и audit trail.
