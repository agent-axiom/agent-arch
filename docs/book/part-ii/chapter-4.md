...
- gateway обходят ради “временной” интеграции;
- approval запрашивают слишком поздно, когда опасное действие уже почти готово к запуску;
- egress rules живут в голове команды, а не в явном contract surface;
- audit trail не хранит policy decision, principal или approval context;
- write-actions и read-actions описаны одинаково, хотя их риск разный.
...
Сначала зафиксируй реальные execution boundaries и approval points, а потом перенеси тот же запрос в memory layer и дальше по системе.
