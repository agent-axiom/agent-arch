...
在 [LangChain 付费媒体智能体案例](https://www.langchain.com/blog/paid-media-agent)中，两个子智能体共用了报告路径和 `done` 标志：第一个完成后，第二个可能跳过自己的工作。作者按平台分离了报告位置和完成状态。

应分别设计三个边界：**上下文**决定模型看到什么；**产物范围**决定分支可以读取和修改什么；**执行状态**决定结果证明的是谁的工作。`fork`/`isolated` 只控制第一项，并不保证另外两项。协调器声明预期分支，分别接收各分支的确认，并明确报告缺项。详细的[归属契约](../part-vii/chapter-16.md)不要求每个专家都有独立虚拟机，但要求记录与产物的分离可被验证。
