...
## 4. Когда агент действительно нужен, а когда нет

В практическом гайде OpenAI есть очень здоровая мысль: агент не нужен только потому, что задача “звучит современно”.[^openai-practical] Его стоит вводить там, где у тебя правда есть хотя бы один из трех признаков:

- система должна принимать серию неочевидных решений по ходу задачи;
...
А вот если у тебя ситуация другого типа, лучше честно остаться в мире workflow:

- шаги почти всегда одинаковые;
- условия переходов легко формализуются;
- требования к объяснимости и повторяемости очень высокие;
- цена ошибки на write path заметная, а ценность “свободы агента” сомнительна.

Практическое правило здесь простое:

- `workflow` хорош там, где путь известен заранее;
- `single-agent loop` хорош там, где путь нефиксированный, но контур узкий;
- `multi-agent` нужен только тогда, когда одна петля уже перестает быть понятной по контексту, ответственности или параллелизму.

Если кратко: агентность должна покупать тебе полезную гибкость, а не просто заменять понятную схему на эффектную.

## 5. Почему “магия” ломается раньше, чем кажется
...
## 6. Четыре принципа, на которые стоит опираться
...
## 7. Что production-команда должна видеть всегда
