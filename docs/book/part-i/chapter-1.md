...
!!! info "Быстрый выбор: workflow, single-agent или multi-agent"
    | Если задача выглядит так | Что брать первым | Почему |
    | --- | --- | --- |
    | Путь почти всегда известен заранее | `workflow` | Дешевле сопровождать, проще тестировать, легче объяснять |
    | Нужен ограниченный выбор инструмента или следующего шага | `single-agent loop` | Дает гибкость без раннего взрыва сложности |
    | Есть независимые подзадачи, разные контексты и разные владельцы | `multi-agent` | Разделяет ответственность и контекст |
