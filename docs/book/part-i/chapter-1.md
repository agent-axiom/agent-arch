...
**Заметка о сквозных сценариях платформы:** аргумент «платформа, а не магия» должен с первого шага держаться на трех канонических сценариях. Триаж обращений поддержки показывает, как повторы, согласования, запись тикетов и восстановление хода инцидента ломают демо-мышление. Внутренний ассистент знаний показывает, как область поиска, опора на источники, границы арендаторов и запись памяти требуют платформенных контролей. Координация инцидентов показывает, как эскалация, побочные эффекты уведомлений, роли ответственных и доказательства отката превращают «умного помощника» в управляемую систему исполнения.
...
Ниже самая полезная короткая рамка для старта. Она дана обычной таблицей, а не только визуальным блоком: так ее легче читать в печати, PDF и текстовом извлечении.

| Если в задаче видно это | Начинай с этого | Почему |
| --- | --- | --- |
| Путь почти всегда известен заранее | Рабочий процесс | Его дешевле сопровождать, проще тестировать и легче объяснять. |
| Нужен ограниченный выбор инструмента или следующего шага | Одиночный агентный цикл | Он дает гибкость без раннего взрыва сложности. |
| Есть независимые подзадачи, разные контексты и разные владельцы | Многоагентная схема | Она разделяет ответственность и контекст только там, где это разделение действительно нужно. |
...
Короткая текстовая формула: известный путь — рабочий процесс; ограниченный выбор следующего шага — одиночный агентный цикл; независимые подзадачи с разными владельцами — многоагентная схема только после доказанной необходимости.
...
!!! summary "Шаблон завершения главы"
    ...
    **Что проверить в своей системе:** где достаточно рабочего процесса; какие побочные эффекты требуют согласования; какие трассы и оценки нужны в первый день.

    **Сопутствующие артефакты:** используй эталонную архитектуру, проверочный список производственной готовности и эталонную среду исполнения как проверяемые артефакты, а не как обязательный программный каркас.
...
- **Практика среды исполнения:** устойчивое выполнение, подтверждения и воспроизводимые трассы — инженерные механизмы, а не метафоры.
- **Устойчивые утверждения:** рискованные побочные эффекты требуют явных точек контроля; трассы и оценочные сигналы нужны для производственной ответственности.
- **Быстро меняющаяся область:** агентные программные каркасы, SDK и шаблоны оркестрации будут меняться быстрее, чем сам принцип управления.
