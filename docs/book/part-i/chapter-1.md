...
!!! info "Быстрый выбор: workflow, single-agent или multi-agent"
    ...
    - **Нужен ограниченный выбор инструмента или следующего шага** -> начинай с `single-agent loop`: он дает гибкость без раннего взрыва сложности.
    - **Есть независимые подзадачи, разные контексты и разные владельцы** -> только тогда рассматривай `multi-agent`: он разделяет ответственность и контекст.
