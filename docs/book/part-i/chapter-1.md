...
- **Вендорская практика:** OpenAI и Anthropic сходятся в том, что начинать лучше с более простой исполнимой формы и добавлять агентность только там, где она дает полезную гибкость.
- **Практика рантайма:** устойчивое выполнение, подтверждения и воспроизводимые трассы — инженерные механизмы, а не метафоры.
- **Авторская интерпретация:** формула `platform, not magic` — синтез этих практик в одно проектное правило этой книги.
- **Быстро меняющаяся область:** агентные фреймворки, SDK и паттерны оркестрации будут меняться быстрее, чем сам принцип управления.
