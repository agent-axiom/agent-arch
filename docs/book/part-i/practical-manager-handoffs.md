...
### Контекст подагента выбирается по роли

[LangChain, Organizing Context in a Multi-Agent Harness](https://www.langchain.com/blog/organizing-context-in-a-multi-agent-harness) (8 сентября 2026) различает `fork` — продолжение с историей координатора — и `isolated` — новый контекст с описанием задачи. В описанной версии Deep Agents `isolated` является режимом по умолчанию. Это варианты передачи контекста, не новые модели оркестрации.

| Роль | Предпочтительный режим | Что передать |
| --- | --- | --- |
| Исполнитель продолжает уже разобранную задачу | `fork`, если история релевантна и разрешена | Собранные свидетельства, ограничения, конкретное поручение |
| Проверяющий оценивает готовую работу | `isolated` | Сам артефакт, требования и критерии проверки, без убеждающего пересказа автора |
| Исследователь решает самостоятельный вопрос | `isolated` | Вопрос, границы поиска, формат свидетельств |
| Агент памяти анализирует разговор | `fork`, если доступ к истории допустим | Разговор как данные и узкое поручение; запись памяти разрешается отдельно |

Это исходные рекомендации, не жесткое правило «каждый исполнитель наследует всё». Большая, нерелевантная или чувствительная история — причина передать минимальный проверенный пакет вместо полного fork. В fork вместе с фактами приходят ошибки и недоверенный текст. В isolated нельзя выкидывать обязательные ограничения задачи. Результат подагента возвращается координатору как проверяемый итог; полный внутренний ход работы не обязан попадать в его контекст.

### Отдельный контекст не изолирует файлы и завершение

В [кейсе LangChain Paid Media Agent](https://www.langchain.com/blog/paid-media-agent) два подагента использовали общий путь отчёта и флаг `done`: завершение первого могло заставить второго пропустить свою работу. Авторы разделили расположение отчётов и состояние завершения по платформам.

Проектируйте три границы отдельно: **контекст** определяет, что видит модель; **пространство артефактов** — что ветка может читать и менять; **состояние выполнения** — чью работу подтверждает результат. `fork`/`isolated` управляет первой границей и не обеспечивает две остальные. Координатор задаёт ожидаемые ветки, принимает подтверждение каждой отдельно и явно сообщает о пропусках. Подробный [контракт владения](../part-vii/chapter-16.md) не требует отдельной виртуальной машины для каждого специалиста, но требует проверяемого разделения записей и артефактов.
