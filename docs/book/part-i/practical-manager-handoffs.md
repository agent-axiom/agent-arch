...
Разбор Anthropic про multi-agent research system полезен именно тем, что показывает не демо, а production-экономику такого выбора.[^anthropic-multi-agent-research] Их система выигрывает там, где задача похожа на исследование вширь: есть много независимых направлений поиска, каждое направление можно отдать отдельному агенту со своим context window, а координатор потом сжимает результаты в общий ответ.

Хороший практический фильтр такой:

- **да:** breadth-first research, независимые ветки, высокая ценность результата, терпимый рост стоимости;
...
Отсюда следует неприятный, но полезный вывод: multi-agent часто повышает качество не магией кооперации, а тем, что разрешает системе потратить больше токенов, инструментальных вызовов и параллельных контекстов. Поэтому manager pattern должен держать бюджеты, лимиты, stop conditions и eval-gates так же явно, как он держит план работы. Иначе “исследовательская глубина” быстро превращается в неконтролируемую стоимость и рваную трассу.
