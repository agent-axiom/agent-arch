...
Именно из этих вопросов и рождается архитектура платформы. Не из любви к слоям как таковым, а из необходимости закрыть конкретные failure questions до того, как в системе появится risky write path.
...
Здесь тезис из первой главы получает первое прямое доказательство. Формулы `model + tools + instructions` достаточно, чтобы прототип выглядел убедительно. Но ее уже недостаточно, чтобы объяснять права, side effects, accountability и recovery, как только система касается реальности.
...
Иными словами, в систему входит не "текст", а управляемый execution context. Этим слоем закрывается первая группа провалов из главы 1: неясный actor, неясный scope и невозможность потом восстановить, что именно система вообще обрабатывала.
...
Это и есть control plane. Он отвечает не за "ум", а за право системы действовать. Именно здесь снимается один из самых дорогих рисков демо-мышления: путаница между “модель предложила” и “система имеет право сделать”.
...
Хороший runtime выглядит скучно. Он не поражает "магией", а делает ход выполнения предсказуемым. Здесь закрываются провалы вокруг retries, pauses, restarts и длинных путей исполнения.
...
Этот слой отвечает за два самых неприятных класса сбоев: неконтролируемые side effects и невозможность понять, что именно произошло после частичного отказа внешней системы.
...
Без этого у тебя не платформа, а сложный черный ящик. Этот слой нужен не для красоты дашбордов, а чтобы дубль тикета, странный retry или слабый policy path можно было потом не пересказывать, а реконструировать.
...
| Orchestration runtime | Workflow graph, planner, checkpoints | Иначе выполнение разваливается при первом retry, pause или restart |
...
Архитектура полезна не потому, что делает схему красивой. Она полезна потому, что отвечает на те самые failure questions из первой главы до того, как они станут инцидентами.
