...
!!! info "快速判断：工作流、单智能体还是多智能体"
    ...
    **路径基本已知。** 从 `workflow` 开始：它更容易维护、更容易测试，也更容易解释。

    **系统只需要受限地选择下一步或某个工具。** 使用 `single-agent loop`：它增加灵活性，但不会过早引爆复杂度。

    **存在独立子任务、不同上下文和不同责任边界。** 只有这时才考虑 `multi-agent`：它只在拆分真实存在时才分离责任和上下文。
