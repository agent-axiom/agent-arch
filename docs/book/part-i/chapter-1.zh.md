...
!!! info "如何阅读本章"
    **章节导向：** 先判断普通工作流是否已经足够。然后再增加智能体循环、记忆、工具和委托。本章给出一条判断规则：它应该能放在一页纸上，而且不依赖网站实时导航也能读懂。
...
这是最值得先记住的一组判断规则。这里不用密集表格，而用独立小节表达，这样在印刷、PDF、搜索索引和纯文本提取里都能保留含义。
...
当路径基本已知、状态转移容易描述、重复性比灵活推理更重要时，先用 `workflow`。它更容易维护、更容易测试，也更容易解释。

### 单智能体循环

当系统只需要在窄边界内受限地选择下一步或某个工具时，使用 `single-agent loop`。它增加灵活性，但不会过早引爆复杂度，并且仍然保留一个明确负责人。
...
图示的文本补充：请求先被转换成执行上下文；策略和审批限制行动权；运行路径随后调用模型、记忆和工具；每一步都留下追踪与评测证据，用于发布、调查和生命周期变更。
...
!!! note "适合印刷的章节结尾"
    在一页纸上，本章应该留下三个判断：先从工作流开始；只有为了必要灵活性才加入智能体循环；只有在上下文、权限和责任真正分开时，才引入多智能体形态。
...
- **主张：** 团队应该先从最简单的可执行形态开始；**来源支撑：** Anthropic 与 OpenAI 都独立提出了这条智能体系统的实践起点。[^anthropic][^openai-practical]
- **主张：** 当自治触及访问、记忆、写路径或事故时，它就变成架构问题；**来源支撑：** durable execution、Agents SDK 与 agent evals 的实践都要求受控执行路径。[^langgraph-durable][^openai-sdk][^openai-evals]
- **主张：** 追踪和评测信号不是装饰，而是问责机制；**来源支撑：** Agents SDK 与 eval guidance 都把步骤证据与质量检查、发布决策联系起来。[^openai-sdk][^openai-evals]
