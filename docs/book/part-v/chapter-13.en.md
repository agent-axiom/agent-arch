...
!!! example "Open-source project by the book's author: Laconian as an inspectable skill"

    Laconian is an open-source project by the author of this book, and `if` is the portable skill inside it.[^laconian-case] The complete installable artifact is one behavioral Markdown file, `skills/if/SKILL.md`, with no scripts, dependencies, permissions, assets, network calls, or tool-specific instructions. This is a transparently disclosed author case, not independent third-party validation.

    The `if` contract does not demand brevity at any cost. Its priority order is correctness and safety; the user's requirements, requested detail, format, and tone; material facts, constraints, warnings, and uncertainty; practical sufficiency; clarity and natural language; and only then brevity. Code, commands, errors, numbers, versions, URLs, identifiers, schemas, required keys, order, and format remain exact whenever their exact form carries meaning. Compression stops before the next deletion would weaken correctness, safety, requirement coverage, completeness, usefulness, clarity, or tone.
