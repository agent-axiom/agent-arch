...
[GitHub Engineering, How we make AI coding more cost efficient without sacrificing task quality](https://github.blog/ai-and-ml/github-copilot/how-we-make-ai-coding-more-cost-efficient-without-sacrificing-task-quality/) 展示了每次工具调用 token 指标的陷阱：更短的响应可能引发重读、重复命令和额外轮次。GitHub 报告，在最终压缩器触发的已测试离线任务中，没有检测到统计显著的任务成功率回归；这不保证在所有负载上等效。

在相同任务和仓库快照上，对比完整的策略允许输出与选择性压缩，固定模型、harness、预算和 verifier。重复实验或随机化顺序有助于估计波动。主要结果是完整任务的成功率、总成本和延迟，而非单条消息大小。成本应包括压缩器、存储、原文读取、重复调用和失败尝试。每个成功任务的成本等于全部尝试总成本除以成功数；成功数为零时该指标未定义。

另外测量被读取原文的压缩输出比例、重复命令、重复探索和额外轮次。重复本身不是因果证据：将事件关联到原始输出和恢复原因，并通过组间比较判断回归。同时报告整个测试集的效果以及压缩实际触发任务的诊断切片，明确分母。预先规定可接受的质量回归与节省标准，估计不确定性，再通过受限在线实验验证。建议的产物字段见[评估 schema](../../appendix/eval-schema.md)。

### 为规格生成系统提供独立参考

SMART 将重新生成的库与人工构建模型核对；文档也包含用于生成测试的精确示例。来源：[Design Docs Are All You Need: An AI-native Machine-Learning Performance Tool](https://arxiv.org/html/2609.05364v1)。不要混淆这两类检查：从同一错误前提产生的代码与测试可以一致地重复错误。

保留独立参考及预期结果的来源。生成器可见的示例用于检查规格一致性，但不是留出测试集。补充由独立人员编写且生成器不可见的案例、不变量和接口检查，在固定条件下比较多次干净生成。参考结果的变化需要单独审查，不能为迁就新代码而调整。若声称性能模型符合物理现实，就需要硬件测量；与另一分析模型一致不能证明这一点。这些是评估建议，不是 SMART 作者开展的额外实验。

### 评估 fork 与 isolated：效率及独立性

对于继续执行工作，在匹配的任务、产物和权限上比较 `fork` 与 `isolated`，固定模型、harness 和预算。测量成功率、重复读取、工具调用、整个任务成本和延迟；分别报告 cached/uncached 输入与缓存预热条件。读取减少不证明总成本降低。动机来源：[LangChain, Organizing Context in a Multi-Agent Harness](https://www.langchain.com/blog/organizing-context-in-a-multi-agent-harness)。

对 verifier 增加独立测试：同一个含已知缺陷的 diff，分别配以中性证据包和协调者错误但自信的解释。比较缺陷检出、误报和接受误导解释的情况。`isolated` 这个名称不能证明独立性：偏差可能来自任务、共享文件或选择性证据；相同模型也可能共享错误。参考结果和标准应独立于被审查方案。这是建议实验，并非文章实测结果；字段见 [eval-schema](../../appendix/eval-schema.md)。

### 评估执行模式选择

选择 `single`/`cascade`/`critique` 本身就是评估对象，而不是多智能体有效性的证明。[HydraFusion](https://github.blog/ai-and-ml/github-copilot/project-hydrafusion-frontier-quality-via-multi-model-orchestration/) 多次在其基准上调整策略；不能把公开的最佳配置当作已证明完全未接触的留出集结果。

对于自己的系统，应在相同任务与仓库快照上，将自适应路由与固定模式比较，保持可用模型池、权限、限制、价格和独立最终 grader 一致。按任务或仓库分离调参与留出集；控制实验前冻结 router 与阈值，不得将隐藏答案或评分输入在线路由。级联 gate 是被测系统的一部分，不是其独立质量标准。

测量验证后的成功率、所有尝试的完整成本、包含 p95 的延迟，以及升级、fallback、失败、gate 错误接受率。报告总体结果与任务切片，而不只报告触发 critique 的案例。重复运行与不确定性区间用于区分改善与波动；提前规定允许的质量退化。按预先声明的规则处理缺失与取消结果，不得为提高节省比例而删除它们。离线验证后还需有限的在线发布实验，尤其针对多轮任务；证据字段见 [eval schema](../../appendix/eval-schema.zh.md)。
...
Google 为这一验证漏斗增加了局部威胁模型：它们来自代码库的实时元数据和依赖关系；独立的分诊代理使用 AST 解析、调用图遍历和预先索引的领域安全规则验证发现。[Google Cloud](https://cloud.google.com/blog/topics/systems/using-ai-agents-to-secure-google-infrastructure) 结果应包含从攻击者可访问入口到危险操作的可检查路径、访问条件，以及展示问题的代码片段或测试，而不只是第二个大语言模型的赞同。

本书建议的契约将证据绑定到代码快照、构建配置、威胁模型版本和调用图修订版。图中的路径本身不能证明条件可满足或漏洞可被利用：反射、动态分派和外部依赖都会限制分析覆盖范围。在不完整或过期的图中找不到路径意味着“尚未证实”，而非“安全”。代码或假设变化后必须重新验证。开发、发现和分诊的规则、上下文及运行框架应相互分离；结构化检查补充而不是替代独立验证。

Google 报告分诊精确率超过 92%、耗时不到一分钟，某些局部威胁模型的误报率为 3%。这些是作者在其环境中的结果：缺少共同数据集和分母时，不能将其合并为一个指标、迁移到其他代码库或视为完整检测的保证。字段和验证场景见[评估模式](../../appendix/eval-schema.md)。
