...
### 7.1. Multi-agent качество должно иметь отдельный бюджет

У многоагентных систем есть особая ловушка: они могут честно стать качественнее и одновременно перестать быть экономически приемлемыми. Практический урок Anthropic про multi-agent research system здесь очень прямой: прирост качества часто связан с тем, что система тратит больше токенов, больше tool calls и больше независимых context windows.[^anthropic-multi-agent-research]
...
- tool calls на ветку и на весь запуск;
- latency p95 с учетом fan-out/fan-in;
- доля запусков, где координатор остановил исследование по budget или stop condition;
- качество финального сжатия: не потерялись ли важные findings при сборке ответа.

Если задача не breadth-first, ветки не независимы или ценность результата не покрывает дополнительный бюджет, SLO должны подталкивать систему обратно к single-agent или manager-led форме, а не поощрять дорогую оркестрацию ради архитектурной красоты.
