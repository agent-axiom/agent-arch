...
### Запуск окружения и память требуют отдельного бюджета

[Новый AgentCore Runtime](https://aws.amazon.com/blogs/machine-learning/the-new-agentcore-runtime-elastic-optimized-and-consistently-fast-starts/) показывает, почему задержку старта среды нельзя смешивать со временем работы модели. AWS измеряет echo-агент без модели и инструментов: 5 000 холодных вызовов на агента, пять размеров образа от 200 MB до 2 GB, в пределах стандартных квот. Клиент в us-west-2 обращается к us-east-1 через публичную сеть. Сообщаемые около двух секунд P75 включают межрегиональный round trip; это не чистое время восстановления microVM и не универсальный SLO пользовательского ответа.

Предлагаемая декомпозиция бюджета: очередь/выделение среды → готовность окружения → первый полезный ответ → завершение задачи. Фиксируйте границы интервалов, чтобы не суммировать пересекающиеся этапы. Отдельно измеряйте cold start, восстановление из стартового снимка, повторное использование уже готовой среды и resume задачи — названия провайдера могут относить восстановление снимка к cold start. Для каждого пути нужны частота, p50/p95/p99, ошибки, тайм-ауты и контекст нагрузки: регион, размер образа, конкуренция, квоты и версия runtime. Не скрывайте неудачные старты из знаменателя.

В стоимости различайте лимит памяти, фактическую резидентную память и оплачиваемые GB-hours. AWS сообщает о загрузке страниц по требованию и возврате освобождённой или остывшей памяти вместо удержания пикового объёма до конца сессии; при этом ставка за память повышена. Снижение GB-hours не означает такое же снижение счёта. Сверяйте интеграл оплачиваемой памяти по времени с действующим тарифом и биллингом; отдельно учитывайте CPU, хранение и прочие оплачиваемые ресурсы. Вызов освобождения памяти приложением не доказывает немедленное снижение биллинга.

Проверяйте профиль «всплеск → простой → повторная работа»: пик, устойчивое потребление, время возврата памяти, стоимость полной сессии и задержку повторного доступа после освобождения. Если провайдер не раскрывает нужную величину, отмечайте её как неизвестную, а не подменяйте RSS. Предварительный запуск при открытии чата может уменьшить ожидание пользователя, но требует лимита времени жизни и учёта неиспользованных сессий. Это предлагаемые метрики книги, не результаты её собственных нагрузочных испытаний. Различие снимка старта и checkpoint — в [главе 16](../part-vii/chapter-16.md).
