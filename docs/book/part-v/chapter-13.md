...
### Структурное подтверждение достижимости

Google описывает уточнение этой воронки: локальная модель угроз строится из актуальных метаданных кодовой базы и зависимостей, а отдельный triage-агент проверяет находку через AST, граф вызовов и заранее индексированные доменные правила безопасности.[Google Cloud](https://cloud.google.com/blog/topics/systems/using-ai-agents-to-secure-google-infrastructure) Результат должен содержать проверяемый путь от доступной атакующему точки входа до опасной операции, условия доступа и фрагмент кода или тест, демонстрирующий проблему, а не только согласие второй LLM.

Для предлагаемого контракта книги связывай доказательство со снимком кода, конфигурацией сборки, версией модели угроз и ревизией графа. Путь в графе сам по себе не доказывает выполнимость условий или эксплуатацию: отражение, динамическая диспетчеризация и внешние зависимости ограничивают полноту анализа. Отсутствие пути в неполном или устаревшем графе означает «не подтверждено», а не «безопасно». Изменение кода или предпосылок требует повторной проверки. Разделяй правила, контекст и harness разработки, поиска и triage; структурные проверки дополняют независимого проверяющего, а не заменяют его.

Google сообщает о precision свыше 92% и времени triage менее минуты, а для некоторых локальных моделей угроз — о доле ложных срабатываний 3%. Это результаты авторов в их условиях: без общего набора и знаменателей их нельзя объединять в одну метрику, переносить на другую кодовую базу или считать гарантией полноты обнаружения. Поля и сценарии проверки — в [схеме оценок](../../appendix/eval-schema.md).

### Оценивать сжатие нужно на всей задаче

[GitHub Engineering, How we make AI coding more cost efficient without sacrificing task quality](https://github.blog/ai-and-ml/github-copilot/how-we-make-ai-coding-more-cost-efficient-without-sacrificing-task-quality/) показывает ловушку метрики «токены на tool call»: сокращенный ответ может вызвать перечитывания, повторные команды и дополнительные ходы. GitHub сообщает об отсутствии статистически значимой регрессии успешности в проверенных офлайн-задачах, где срабатывал финальный компрессор; это не гарантия эквивалентности на любой нагрузке.

Сравни контроль с полным разрешенным выводом и вариант с выборочным сжатием на одинаковых задачах и снимках репозитория, при фиксированных модели, harness, бюджете и verifier. Повторы или рандомизация порядка помогают оценить разброс; первичный результат — успешность всей задачи, общая стоимость и задержка, а не размер одного сообщения. Учитывай стоимость компрессора, хранения и чтения оригинала, повторных вызовов и неудачных попыток. Стоимость на успешную задачу считай как суммарную стоимость всех попыток, деленную на число успешных; при нуле успехов метрика не определена.

Отдельно измеряй долю сжатых outputs, для которых прочитан оригинал, повторные команды, повторное исследование и дополнительные ходы. Сам повтор не доказывает причинность: связывай событие с исходным output и причиной восстановления, а вывод о регрессии делай по сравнению групп. Показывай как общий эффект на наборе, так и диагностический срез задач, где сжатие сработало, не скрывая знаменатели. Заранее задай допустимую регрессию качества и критерий экономии, оцени неопределенность, затем проверь результат ограниченным онлайн-экспериментом. Поля предлагаемого артефакта — в [схеме оценок](../../appendix/eval-schema.md).

### Независимый эталон для системы, сгенерированной из спецификации

SMART сравнивает регенерированную библиотеку с вручную подготовленными моделями; документы также содержат точные примеры, по которым генерируются тесты. Источник: [Design Docs Are All You Need: An AI-native Machine-Learning Performance Tool](https://arxiv.org/html/2609.05364v1). Эти два вида проверки нельзя смешивать: код и тест, созданные из одной неверной предпосылки, могут согласованно повторить ошибку.

Для оценки такого подхода сохраняй независимые эталоны и происхождение ожидаемых результатов. Знакомые генератору примеры проверяют соответствие спецификации, но не служат удержанным тестовым набором. Дополняй их независимо подготовленными закрытыми для генератора случаями, инвариантами и проверками интерфейсов; сравнивай несколько чистых генераций при фиксированных условиях. Изменение эталона требует отдельного разбора, а не подгонки под новый код. Если заявляется физическая точность модели производительности, нужны измерения оборудования: совпадение с другой аналитической моделью этого не доказывает. Это рекомендации для оценки, не дополнительные эксперименты, проведенные авторами SMART.

### Оценка fork и isolated: экономия и независимость

Для продолжения работы сравни `fork` и `isolated` на одинаковых задачах, артефактах и разрешениях, фиксируя модель, harness и бюджет. Измеряй успешность, повторные чтения, вызовы инструментов, общую стоимость и задержку; отдельно показывай cached/uncached input и условия прогрева кэша. Снижение количества чтений не доказывает снижение полной стоимости. Источник постановки: [LangChain, Organizing Context in a Multi-Agent Harness](https://www.langchain.com/blog/organizing-context-in-a-multi-agent-harness).

Для verifier добавь отдельный тест: одинаковый diff с заранее известной ошибкой предъявляется с нейтральным пакетом свидетельств и с ошибочным уверенным объяснением координатора. Сравни обнаружение дефекта, ложные тревоги и принятие навязанного объяснения. Само имя `isolated` не доказывает независимость: смещение может попасть в поручение, общие файлы или выборочно переданные свидетельства; одинаковые модели также могут разделять ошибки. Эталон и критерии должны быть независимы от проверяемого решения. Это предложенный эксперимент, не результат измерений из статьи; поля учета — в [eval-schema](../../appendix/eval-schema.md).

### Проверять качество выбора схемы выполнения

Выбор `single`/`cascade`/`critique` — отдельный объект оценки, а не доказательство пользы multi-agent подхода. В [HydraFusion](https://github.blog/ai-and-ml/github-copilot/project-hydrafusion-frontier-quality-via-multi-model-orchestration/) политики неоднократно уточняли на бенчмарках; лучшие опубликованные конфигурации нельзя трактовать как результат на заведомо нетронутой контрольной выборке.

Для своей системы сравните адаптивную политику с фиксированными схемами на одинаковых задачах и снимках репозитория, с одинаковыми доступными моделями, правами, лимитами, ценами и независимым финальным grader. Разделите настройку и контроль по задачам/репозиториям, зафиксируйте router и пороги до контрольного запуска; не подавайте скрытые ответы или оценки в online-routing. Gate каскада — часть испытуемой системы, не независимый эталон её качества.

Измеряйте проверенную успешность, полную стоимость всех попыток, задержку (включая p95), частоты эскалации, fallback, отказа и ошибочного принятия gate. Покажите общий результат и срезы задач, а не только случаи, где сработала критика. Повторные запуски и интервалы неопределённости нужны для отделения улучшения от вариативности; допустимую потерю качества задают заранее. Пропуски и отмены учитывают по объявленному правилу, не удаляют ради экономии. После offline-проверки нужен ограниченный online rollout, особенно для многоходовых задач; поля доказательств — в [eval-schema](../../appendix/eval-schema.md).
...
!!! example "Открытый проект автора книги: Laconian как проверяемый навык"

    Laconian — открытый проект автора этой книги, а `if` — переносимый навык в его составе.[^laconian-case] Устанавливаемый артефакт состоит из одного Markdown-файла с поведенческими правилами, `skills/if/SKILL.md`: без скриптов, зависимостей, разрешений, ресурсов, сетевых вызовов и инструкций, привязанных к конкретным инструментам. Это открыто описанный пример автора, а не независимая сторонняя проверка.

    Контракт `if` не требует краткости любой ценой. Его порядок приоритетов таков: корректность и безопасность; требования пользователя, необходимая подробность, формат и тон; существенные факты, ограничения, предупреждения и неопределенность; практическая достаточность; ясность и естественный язык; и лишь затем краткость. Код, команды, сообщения об ошибках, числа, версии, URL, идентификаторы, схемы, обязательные ключи, порядок и формат сохраняются без изменений, когда их точная форма несет смысл. Сжатие прекращается, прежде чем следующее удаление ослабит корректность, безопасность, охват требований, полноту, полезность, ясность или тон.

    Оценка намеренно разделяет два вопроса: активацию и поведение ответа при намеренном применении инструкции. Набор проверок активации выясняет, должен ли хост агента выбрать `if` для данного запроса; набор проверок ответа обходит автоматический выбор и намеренно применяет каждый вариант сравнения. Четыре варианта — `baseline`, точная инструкция `Answer concisely.`, зафиксированный на уровне байтов тестовый образ Caveman и точные байты `skills/if/SKILL.md`. Между вариантами неизменными остаются модель, пользовательский запрос, настройки генерации, доступность инструментов и размещение инструкции; меняется только инструкция сравнения.

    Жесткий шлюз и необязательный слепой семантический шлюз выполняются до парных метрик краткости, поэтому неудачный ответ не может выиграть лишь за счет меньшей длины. Каждый запуск сохраняет сведения о версии исполнителя, полный хеш сценария, хеши промпта и инструкции, цепочку попыток и повторов, а также исходные артефакты в режиме только добавления; сведения об оценщике сохраняются только при семантической оценке. Благодаря этому гипотезу можно проверить и опровергнуть.

    На зафиксированной ревизии в репозитории пока нет публичных результатов бенчмарка. Дымовые проверки и сценарии повторного запуска проверяют путь оценки, но не доказывают экономию токенов, превосходство `if` над другим вариантом или одинаковое поведение на разных хостах агентов. Предмет этого примера — дисциплина контракта и оценки, а не благоприятный результат.
