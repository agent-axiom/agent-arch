...
Сильная цепочка доказательств не требует одного гигантского schema-файла. Но она требует устойчивого набора идентификаторов и ссылок между слоями.
...
Возьмем запуск разбора обращений поддержки, который умеет классифицировать входящее обращение, искать внутренние знания и создавать тикет только после подтверждения в высокорисковых случаях.
