...
- **P1:** closed for the named safe-agent upgrades and their schema spine. MCP threat model, A2A handoff trust contract, unified agent threat evidence, verifier verdict record, governance action record, and memory poisoning review fields are documented across RU/EN/ZH and reflected in trace/eval/memory schemas with docs-surface tests.
