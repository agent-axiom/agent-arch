...
def test_russian_chapter_9_prefers_reader_facing_mcp_contract_terms() -> None:
    ...
    expected_markers = (
        ...
        "`host` (`узел`) — это приложение или рантайм",
        "`client` (`клиент`) — это протокольный компонент",
        "`server` (`сервер`) — это граница",
        "один узел может одновременно держать несколько клиентов",
        "сразу с несколькими MCP-серверами",
    ...
    forbidden_markers = (
        ...
        "`server` - это граница",
        "сразу с несколькими MCP-серверы",
