...
_RUSSIAN_EXPECTED_ALTERNATIVES = {
    ...
    "A2A trust and delegation artifact": (
        ...
        "проверяемый контур передачи управления и доверия для A2A",
    ),
...
def test_russian_practical_pages_localize_deeper_reader_facing_prose() -> None:
    ...
    _assert_files_contain_all(
        ("docs/book/part-iv/practical-mcp-a2a.md",),
        (
            "взаимодействие агента с инструментом",
            "взаимодействие агента с другим агентом",
            "контур передачи управления A2A",
            "какой роли и какому действию",
            "Это почти всегда избыточное усложнение",
            "не дают сильного основания",
            "через какой набор политик",
            "проверяемый контур передачи управления и доверия для A2A",
