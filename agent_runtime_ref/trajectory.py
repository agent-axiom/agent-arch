...
_SAFE_REFERENCE_PATTERN = re.compile(r"[a-z][a-z0-9+.-]{0,15}://[a-z0-9][a-z0-9._~:/#-]*\Z")
_SAFE_TELEMETRY_PATTERN = re.compile(r"[a-z0-9][a-z0-9+._~:/#=;,\[\]-]*\Z")
...
def _safe_reference(value: object, *, field: str) -> str:
    normalized = _required_string(value, field=field)
    if len(normalized) > _MAX_REFERENCE_LENGTH or not _SAFE_REFERENCE_PATTERN.fullmatch(normalized):
...
def _safe_telemetry_value(value: object, *, field: str) -> str:
    normalized = _required_string(value, field=field)
    if len(normalized) > _MAX_TELEMETRY_VALUE_LENGTH or not _SAFE_TELEMETRY_PATTERN.fullmatch(
        normalized
    ):
...
def _typed_tuple(
    ...
    *,
    ...
    return cast(tuple[_TupleItem, ...], items)
...
def _require_bounded_collection(
    ...
    *,
    ...
    if len(items) > maximum:
        raise ValueError(f"Trajectory {label} must contain at most {maximum} entries")
...
TrajectoryRule: TypeAlias = (
    ValueBindingRule | CumulativeLimitRule | RequiredPredecessorRule | RequiredApprovalRule
)
_RULE_TYPES = (
    ValueBindingRule,
    CumulativeLimitRule,
...
def _trajectory_rules(value: object) -> tuple[TrajectoryRule, ...]:
    rules = _tuple(value, label="rules")
    if not rules:
    ...
    return cast(tuple[TrajectoryRule, ...], rules)
...
class TrajectoryPolicy:
    ...
    def __post_init__(self) -> None:
        ...
        rules = _trajectory_rules(self.rules)
...
class TrajectoryPolicyDecision:
    ...
    def to_event_payload(self) -> dict[str, str]:
        ...
        return {
            field: _safe_telemetry_value(value, field=field) for field, value in payload.items()
        }
...
def _fingerprint_summary(
    ...
    history = (
        {item.name: item.value for item in snapshot.fingerprints} if snapshot is not None else {}
    )
...
class _EvaluationContext(NamedTuple):
    ...
    policy: TrajectoryPolicy
    request_fingerprint: str
    counter_states: dict[str, str]


class _RuleValues(NamedTuple):
    current_fingerprints: dict[str, str]
    history_fingerprints: dict[str, str]
    current_counters: dict[str, Decimal]
    history_counters: dict[str, Decimal]
...
def _snapshot_decision(
    ...
    return TrajectoryPolicyDecision(
        ...
        fingerprints=_fingerprint_summary(
            request,
            snapshot,
            context.request_fingerprint,
        ),
        counters=_counter_summary(context.counter_states),
...
def _history_trust_decision(
    context: _EvaluationContext,
) -> TrajectoryPolicyDecision | None:
    snapshot = context.snapshot
    if snapshot.status == "missing" or snapshot.integrity == "missing":
        return _snapshot_decision(
            context,
            "deny",
...
def _trajectory_binding_decision(
    context: _EvaluationContext,
) -> TrajectoryPolicyDecision | None:
    request = context.request
    snapshot = context.snapshot
    ...
    if request.tenant_id != snapshot.tenant_id or request.subject_id != snapshot.subject_id:
        return _snapshot_decision(
            context,
            "deny",
            "trajectory.identity",
    ...
    if policy.policy_id != snapshot.policy_id or policy.policy_version != snapshot.policy_version:
        return _snapshot_decision(
            context,
            "deny",
            "trajectory.policy_identity",
...
def _snapshot_precondition_decision(
    context: _EvaluationContext,
) -> TrajectoryPolicyDecision | None:
    decision = _history_trust_decision(context)
    if decision is not None:
        return decision
    decision = _trajectory_binding_decision(context)
    if decision is not None:
        return decision
    return _window_decision(context)


def _rule_values(context: _EvaluationContext) -> _RuleValues:
    return _RuleValues(
        current_fingerprints={item.name: item.value for item in context.request.fingerprints},
        history_fingerprints={item.name: item.value for item in context.snapshot.fingerprints},
        current_counters={item.name: item.value for item in context.request.counters},
        history_counters={item.name: item.value for item in context.snapshot.counters},
    )


def _evaluate_value_binding_rule(
    context: _EvaluationContext,
    values: _RuleValues,
    rule: ValueBindingRule,
) -> TrajectoryPolicyDecision | None:
    current_value = values.current_fingerprints.get(rule.fingerprint_name)
    history_value = values.history_fingerprints.get(rule.fingerprint_name)
    if current_value is None:
        return _snapshot_decision(
            context,
            "deny",
            rule.rule_id,
            "current_fingerprint_missing",
        )
...
def _evaluate_cumulative_limit_rule(
    context: _EvaluationContext,
    values: _RuleValues,
    rule: CumulativeLimitRule,
) -> TrajectoryPolicyDecision | None:
    current_value = values.current_counters.get(rule.counter_name)
    history_value = values.history_counters.get(rule.counter_name)
    if current_value is None:
        return _snapshot_decision(
            context,
            "deny",
            rule.rule_id,
            "current_counter_missing",
        )
    if history_value is None:
        return _snapshot_decision(
            context,
            "deny",
            rule.rule_id,
            "history_counter_missing",
        )
    total = _add_counter_values(history_value, current_value)
    if total is None:
        context.counter_states[rule.counter_name] = "arithmetic_error"
        return _snapshot_decision(
            context,
            "deny",
            rule.rule_id,
            "counter_arithmetic_error",
        )
    if total > rule.limit:
        context.counter_states[rule.counter_name] = "limit_exceeded"
        return _snapshot_decision(
            context,
            "deny",
            rule.rule_id,
    ...
    context.counter_states[rule.counter_name] = "within_limit"
    return None


def _evaluate_required_predecessor_rule(
    context: _EvaluationContext,
    rule: RequiredPredecessorRule,
) -> TrajectoryPolicyDecision | None:
    if rule.predecessor not in context.snapshot.observed_sequence:
        return _snapshot_decision(
            context,
            "deny",
            rule.rule_id,
...
def _missing_bound_approval_decision(
    context: _EvaluationContext,
    rule: RequiredApprovalRule,
    scope_approvals: tuple[ApprovalRecord, ...],
) -> TrajectoryPolicyDecision:
    if scope_approvals:
        return _snapshot_decision(
            context,
    ...
    if context.snapshot.approval_state != "none":
        return _snapshot_decision(
            context,
            "deny",
            rule.rule_id,
...
def _bound_approval_decision(
    context: _EvaluationContext,
    rule: RequiredApprovalRule,
    bound_approvals: tuple[ApprovalRecord, ...],
) -> TrajectoryPolicyDecision | None:
    ...
    if bound_states & _INACTIVE_APPROVAL_STATES:
        return _snapshot_decision(
            ...
            "deny",
            rule.rule_id,
            "required_approval_inactive",
        )
    if bound_states == {"approved"}:
        if context.snapshot.approval_state != "approved":
            return _snapshot_decision(
                context,
                "deny",
                rule.rule_id,
    ...
    if bound_states == {"pending"}:
        if context.snapshot.approval_state != "pending":
            return _snapshot_decision(
                context,
                "deny",
                rule.rule_id,
...
def _evaluate_required_approval_rule(
    context: _EvaluationContext,
    rule: RequiredApprovalRule,
) -> TrajectoryPolicyDecision | None:
    scope_approvals = tuple(
        record
        for record in context.snapshot.approval_records
    ...
    if not bound_approvals:
        return _missing_bound_approval_decision(context, rule, scope_approvals)
    return _bound_approval_decision(context, rule, bound_approvals)
...
def _evaluate_rule(
    ...
    if isinstance(rule, ValueBindingRule):
        return _evaluate_value_binding_rule(context, values, rule)
    if isinstance(rule, CumulativeLimitRule):
        return _evaluate_cumulative_limit_rule(context, values, rule)
    if isinstance(rule, RequiredPredecessorRule):
        return _evaluate_required_predecessor_rule(context, rule)
    if isinstance(rule, RequiredApprovalRule):
        return _evaluate_required_approval_rule(context, rule)
    raise AssertionError("Unsupported trajectory rule")


def _evaluate_rules(context: _EvaluationContext) -> TrajectoryPolicyDecision:
    values = _rule_values(context)
    for rule in context.policy.rules:
        decision = _evaluate_rule(context, values, rule)
        if decision is not None:
            return decision
    return _snapshot_decision(
        context,
        "allow",
        "trajectory.all_rules",
        "all_rules_satisfied",
    )
...
def evaluate_trajectory_policy(
    ...
    precondition_decision = _snapshot_precondition_decision(context)
    if precondition_decision is not None:
        return precondition_decision
    return _evaluate_rules(context)
