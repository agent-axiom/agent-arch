...
That same line now also reaches the rollout/change side of the package: the demo `change.yaml` includes a `failed_run_drill_checked` gate signal so degraded paths are part of release review, not an afterthought. `inspect-lifecycle` now surfaces both failed-run gate signals and `failed_run_archive_targets` directly, adds `controls.failed_run_control_expectations` plus `controls.failed_run_control_domains` for the trace/provenance side of lifecycle review, `check-change` breaks out `missing_failed_run_signals`, `check-retirement` exposes the same `failed_run_archive_targets` that must survive shutdown, and `check-controls` now isolates failed-run-related control gaps through `failed_run_controls` while also naming `preserved_failed_run_controls` and a top-level `failed_run_controls_healthy` summary, so operators can see lifecycle and control-plane expectations without opening the YAML first.
