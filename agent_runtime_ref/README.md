...
That same line now also reaches the rollout/change side of the package: the demo `change.yaml` includes a `failed_run_drill_checked` gate signal so degraded paths are part of release review, not an afterthought. `inspect-lifecycle` now surfaces both failed-run gate signals and `failed_run_archive_targets` directly, `check-change` breaks out `missing_failed_run_signals`, and `check-retirement` exposes the same `failed_run_archive_targets` that must survive shutdown, so operators can see lifecycle expectations and specific degraded-path gaps without opening the YAML first.
